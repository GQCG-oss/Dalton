...
!     call getbytesize(maxrep, symmtilast, SizeInBytes)
     ...
     &                IPAR,   JSOP,   NCOS,  ICOS,   I2COSX 
      ...
      INTEGER LEN_SYMMTR
      PARAMETER (LEN_SYMMTR = 16) ! length of SYMMTR, used in MPI calls !Beyersays: Note that this distance is hardcoded. If for whatever reason the length of the block changes you can still use getbytesize(fmult, symmtrlast, SizeInBytes) to get the actual size of the common block.
