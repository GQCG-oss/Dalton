...
\item[\Key{PEQM}] \index{polarizable embedding}\index{PE}\index{environment model}\index{embedding model}\index{embedding}\index{QM/MM} Include environment effects by using the polarizable embedding (PE) model. This will embed the core molecular system, as defined in the \mol\ input file, in a polarizable embedding potential given in the \pot\ input file. See further input options in the \Sec{PEQM} input section (see subsection~\ref{subsec:peqm}). See Chapter~\ref{ch:embedding} for an introduction to polarizable embedding calculations.
...
\subsection{Polarizable embedding environment model: \Sec{PEQM}}
\label{subsec:peqm} \index{polarizable embedding}\index{PE}\index{environment model}\index{embedding model}\index{embedding}\index{QM/MM}
...
molecular system. The core system is defined by the \mol\ input
file. The environment can be described by multipole moment expansions
and anisotropic dipole-dipole polarizabilities provided in the \pot\
input file. See Chapter~\ref{ch:embedding} for an introduction to
calculations using the polarizable embedding model.
...
Not documented. Examples can be found by grepping for "QM3" in the "DALTON/test" directory in the distribution. See also \Key{PEQM} and \Sec{PEQM} for a more general and efficient implementation with many of the same capabilities.

\subsection{PCM environment model: \Sec{PCM}}
\label{subsec:pcm}

This input section, together with the \Sec{PCMCAV} input section, controls
the polarizable continuum model to describe the environment
effects. See Section~\ref{sec:pcm} for an introduction to calculations
using PCM in {\dalton}\index{PCM}.
...
\item[1] reading the sphere centers coordinates in the MOLECULE
  geometry file, as (X,Y,Z,R), with R the radius of the sphere. Be
  aware that this option is not working if molecular symmetry is
  detected.
...
\hline
\end{tabular}
It is also possible to define your own solvent by using the keywords
\Key{EPS}, \Key{EPSINF} and \Key{RSOLV} (see below)
...
\item[\Key{RET}]\verb| |\newline
\verb|READ (LUCMD, *) RET|\verb| |\newline
Defines the minimum radius of added spheres during the cavity
generation (default 0.2 au).
...
sphere. (default 0.4 au$^2$)

\item[\Key{INA}]\verb| |\newline
\verb|DO I = 1, NESFP|\newline
\verb|  READ (LUCMD, *) INA(I)|\verb| |\newline
\verb|END DO|\newline
List of which atoms to put a sphere on
...
contribution, and radii = RvdW for the cavitation.
