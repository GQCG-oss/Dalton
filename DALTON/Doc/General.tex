...
\item[\Key{PEQM}] Include environment effects by using the polarizable embedding (PE)
model~(See Ref.~\cite{pescf} for details on the PE-HF/DFT model, Ref.~\cite{pesoppa}
for PE-MP2/SOPPA and Ref.~\cite{pecc} for PE-CC)\index{polarizable embedding}\index{PE}\index{environment model}\index{embedding model}\index{embedding}\index{QM/MM}.
...
in the \Sec{PEQM} input section (see subsection~\ref{subsec:peqm}). See
...
\item[\Key{QFIT}] Invoke the calculation of potential derived
charges after the wave function has converged.
...
\index{molecular Hessian!initial!diagonal}
...
\subsection{Polarizable embedding model: \Sec{PEQM}}
...
This input section controls calculations using polarizable
embedding (PE) model which can include effects from a structured
...
calculations using the polarizable embedding model. See also Ref.~\cite{pescf} for details on the PE-HF/DFT model, Ref.~\cite{pesoppa} for PE-MP2/SOPPA, Ref.~\cite{pemcscf} for PE-MCSCF and Ref.~\cite{pecc} for PE-CC. Note, that the input options shown here cannot be used with PE-CC, because it is based on an older implementation (see Section~\ref{sec:pecc} for PE-CC input examples).
...
Not documented. Examples can be found by grepping for "QM3" in the "DALTON/test" directory in the distribution. See also \Key{PEQM} and \Sec{PEQM} 
...
Directives controlling the calculation of potential derived charges. Currently, the only supported grid is a Connolly-Merz grid where \verb|NSHELL| shells around the molecule are constructed from Van der Waal spheres placed on each nuclei of the molecule. The initial spheres are scaled by \verb|VDWSCL| and succesive spheres are scaled in increments of \verb|VDVINC|.
...
Sets the scaling factor for the Van der Waal radii of the nuclei when generating the surface. The default value is 1.4.

\item[\Key{VDWINC}]\verb| |\newline
\verb|READ (LUCMD,*) VDWINC|

Sets the increment of scaling for successive shells past the first. See option \verb|NSHELL|. The scaling factor for each Van der Waal sphere for shell $n$ can be calculated from $R = R_{vdw}\cdot (R_{scl}+(n-1)\cdot R_{inc})$ The default value is 0.2 per shell.
