...
\item[\Key{PEQM}] \index{polarizable embedding}\index{PE}\index{environment model}\index{embedding model}\index{embedding}\index{QM/MM} Include environment effects by using the polarizable embedding (PE) model. This will embed the core molecular system, as defined in the \molinp\ file, in a polarizable embedding potential given in the \potinp\ file. See further input options in the \Sec{PEQM} input section (see subsection~\ref{subsec:peqm}). See Chapter~\ref{ch:embedding} for an introduction to polarizable embedding calculations.
