...
Section~\ref{ref-mp2inp} \Sec{MP2 INPUT}, for \Key{MP2}
...
\item[\Key{SRFUN}] \ \\
  \kw{READ (LUINP,'(A80)') LINE} \\
  Specification of functional used for \quotekw{.HFSRDFT} and \quotekw{.MCSRDFT} calculations.
...
\item[\Key{STEX}]
  Static exchange calculation.\index{Static exchange}
...
\item[\Key{MP2}]
 \index{M{\o}ller-Plesset!second-order}\index{MP2}\
  ...
  Requires \quotekw{.HF}, \quotekw{.HFsrDFT},
...
\subsection{\label{ref-srdft}srDFT functionals}

In this sections the available srDFT functionals will be listed.
The functonals are actived by specifying them after the \quotekw{.SRFUN} keyword.
The most common functionals can be specied as a single word, whereas custom combinations of exchange and correlation can be specified by a two word key.
The exchange functional will be specified first and the correlation functional second.
For example, \verb|SRXPBEGWS SRCPBEGWS|
...
\item[HFEXCH] 100\% full-range Hartree-Fock like exchange.
