...
\index{polarizable embedding}\index{PE}\index{environment model}\index{embedding model}\index{embedding}\index{QM/MM}
...
and not the PE 
library. Note, that in the next release the current 
implementation will be replaced with newer code that takes advantage of the
new PE library (see Sec.~\ref{sec:pecc})}. 
...
the effective operator as induced dipoles that arise due to the total electric
...
The PE model is implemented for HF, DFT, MP2 and
...
in Sec.~\ref{sec:pecc}. Note, however, that this code will be replaced with
new code that takes advantage of the more general and efficient PE library
...
include environment effects using the polarizable embedding (PE) model it is 
necessary to define the core and environment part of the system: the \molinp\ 
file specifies the core molecular system and the \potinp\ file that contains 
the embedding potential which describes the environment. Moreover, additional 
...
**END OF DALTON
\end{verbatim}
To use non-default options a \Sec{PEQM} subsection is needed also in the 
...
The \potinp\ file is split into three sections: \Sec{*coordinates}, 
\Sec{*multipoles} and \Sec{*polarizabilities}. The format is perhaps best 
...
\noindent\textbf{\Sec*{polarizabilities}}\newline
...
line, i.e.\ \verb|0| or missing means no charges and \verb|text| means include 
charges) then the following integer is the nuclear charge. Following this is 
