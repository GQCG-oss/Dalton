...
\cite{soppaccsd,ekdspasjpca102,tejospastcan100,ctocd}.
Apart from the use of the CCSD amplitudes, the equations are essentially the
same as for SOPPA.  
Note that SOPPA(CCSD) is not a coupled cluster linear response function
and neither SOPPA nor SOPPA(CCSD) are the same as the CC2 model, and
although they all three are of at least second order in all terms they
differ in the terms included.
The SOPPA(CCSD) model is thus not implemented in the CC module but
is implemented in the RESPONSE and ABACUS
modules of the Dalton program, using basically the same code as SOPPA.
...
\item  Spin-Spin Coupling Constants : Thomas Enevoldsen, Jens Oddershede, 
