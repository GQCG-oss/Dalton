...
\item[\Key{FIELD}] \verb| |\newline
    ...
    (same input format as in \ref{ref-haminp}).
    ...
    models including triples excitations (most notably CCSD(T), CC3).
    For finite field calculations of orbital-relaxed energy 
    derivatives the field must be included in the SCF calculation.
    Use for this input section \Sec{HAMILTONIAN} (\ref{ref-haminp}).
    \index{finite field}
