...
\index{CTOCD-DZ} approach~\cite{paololazz1,paololazz2,ctocd}. In the present version
of DALTON the CTOCD-DZ method is implemented which can be invoked by the additional 
keyword \Key{CTOCD} in the \Sec{*PROPERTIES} input module.  More detailed information
...
be invoked by the additional keyword \Key{SOPPA} in the \Sec{*PROPERTIES} 
input module. This requires that the MP2 energy was calculated by specifying 
the keyword \Key{MP2} in the \Sec{*WAVE FUNCTIONS} input module. The use of 
...
For a SOPPA\index{SOPPA} calculation of molecular magnetizabilities the 
additional 
keyword \Key{SOPPA} has to be specified in the \Sec{*PROPERTIES} input module 
and an MP2 calculation has to be requested by the keyword \Key{MP2} in the 
\Sec{*WAVE FUNCTIONS} input module. This will also disable automatically
...
\Key{SOPPA} and \Key{CTOCD} could be used together to get SOPPA molecular
...
For a SOPPA\index{SOPPA} calculation of nuclear shieldings the additional 
keyword \Key{SOPPA} has to be specified in the \Sec{*PROPERTIES} input module 
and an MP2 calculation has to be requested by the keyword \Key{MP2} in the 
\Sec{*WAVE FUNCTIONS} input module. This will also disable automatically
the use of London orbitals.
...
additional keyword \Key{CTOCD} has to be specified in the \Sec{*PROPERTIES} 
input module. This will automatically disable the use of London orbitals.
\Key{SOPPA} and \Key{CTOCD} could be used together to get gauge origin independent
...
For a SOPPA\index{SOPPA} calculation of rotational  {\em g} tensors the 
additional 
keyword \Key{SOPPA} has to be specified in the \Sec{*PROPERTIES} input module 
and an MP2 calculation has to be requested by the keyword \Key{MP2} in the 
\Sec{*WAVE FUNCTIONS} input module. This will also disable automatically
...
For a SOPPA\index{SOPPA} calculation of nuclear spin--rotation constants the 
additional 
keyword \Key{SOPPA} has to be specified in the \Sec{*PROPERTIES} input module 
and an MP2 calculation has to be requested by the keyword \Key{MP2} in the 
\Sec{*WAVE FUNCTIONS} input module. This will also disable automatically
...
\item SOPPA: T.Enevoldsen, J.Oddershede, and S.P.A. Sauer.
...
For a SOPPA\index{SOPPA} calculation of indirect nuclear spin--spin coupling 
constants the additional 
keyword \Key{SOPPA} has to be specified in the \Sec{*PROPERTIES} input module 
and an MP2 calculation has to be requested by the keyword \Key{MP2} in the 
\Sec{*WAVE FUNCTIONS} input module. 
...
In SCF calculations the convergence towards the HF limit is slower 
...
\newblock {\em Chem.~Phys.~Lett.}, {\bf 220},\hspace{0.25em} 299 (1994)
\item P.~Lazzeretti
\newblock {\em Prog.~Nucl.~Mag.~Res.~Spec.}, {\bf 220},\hspace{0.25em} 1--88 (2000)
...
For \Sec{*RESPONSE} calculations of these properties the integrals needed in the \Sec{*INTEGRALS} 
