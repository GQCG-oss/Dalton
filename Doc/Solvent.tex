...
surrounding linear, homogeneous dielectric medium\index{dielectric
...
Self-Consistent Reaction Field (MCSCRF)\index{reaction
field}\index{MCSCRF}
...
solute is modelled by a multipole expansion\index{multipole
expansion}, in \siraba\ in principle to infinite order, but
...
extended to singlet linear, quadratic and cubic response, and  triplet linear response\index{linear response}\index{quadratic response}\index{cubic response}\index{triplet response} in the
\resp\ program, both using equilibrium and no-equilibrium solvation. A number of properties and excitation energies can
...
only through the general \Sec{WALK} module. 
...
Thus the input for an SCF geometry
