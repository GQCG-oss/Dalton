...
described in Ref.~\cite{klbaehkrthjopjtca90}, and Raman 
Optical Activity (ROA)\index{ROA}\index{Raman optical activity} as
described in Ref.~\cite{thkrklbpjjofd99}.  
...
results of this investigations indicate that the aug-cc-pVDZ basis
set, which is supplied with the \siraba\ basis set library, is
adequate for such calculations. 
...
The calculation of vibrational Raman intensities and Raman optical
...
same calculation, because the time used in the setup of the right-hand
...
is described in Ref.~\cite{thkrklbpjjofd99}. Thus we need to do a
geometry walk of  type numerical differentiation. In each geometry
...
the Cartesian directions\index{cartesian coordinate} and 
...
vibrational analysis for one species.

We note, that just as for Vibrational Circular Dichroism, a different
force field may be used in the estimation of the VROA intensity
paramaters. Indeed, a number of force fields can be used to estimate
the VROA paramaters obtained with a given basis set through the input:
...
Thus, it is evident that the calculation of Vibrational Raman Optical
...
currently most accurate way of calculating Raman optical activity. 
...
aug-cc-pVTZ seems to be necessary. However, this is far to large to
use in a calculation of ROA of a naturally optical active molecule.
