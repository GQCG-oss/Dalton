...
for locating minima and transition states\index{first-order
optimization}\index{second-order optimization}\index{geometry
optimization} (geometry optimization). Most of the Hessian
updating\index{Hessian update} schemes were taken from
ref.\cite{thkrprt95} and \cite{Fletcher}. The implementation of
redundant internal coordinates\index{redundant internal coordinates}
follows the work of Peng {\it et al.\/}~\cite{Peng}. In addition to
this, several keywords for VRML visualization are
included~\cite{VRML}.
...
combination of the two keywords \Key{BFGS} and \Key{REDINT}.

\item[\Key{2NDORD}]
Use default second-order method\index{second-order
optimization}. Molecular Hessians will be calculated at every
...
optimization} with the Broyden-Fletcher-Goldfarb-Shanno (BFGS)
update\index{BFGS update} formula for optimization. This is the
...
\item[\Key{BOFILL}]
Bofill's update\cite{veb003} is the default updating scheme for
...
\item[\Key{GDIIS}]
Use the Geometrical DIIS\cite{veb004} algorithm to control the
...
\item[\Key{INIMOD}]
Use a simple model Hessian\cite{veb005} diagonal in redundant internal
coordinates as the initial Hessian. All diagonal elements are
...
\item[\Key{RF}]
Use the rational function method~\cite{vb007} instead of level-shifted
Newton which is the default. The RF method is often slightly faster
than the level-shifted Newton, but also slightly less robust.

For saddle point optimizations there's a special partitioned rational
function method~\cite{vb008} (used automatically when both \Key{RF}
...
Hessian\index{Hessian index} is non-zero when the gradient norm
...
controlled by the \Key{DISPLA} keyword in the \Sec{OPTIMIZE} module.
