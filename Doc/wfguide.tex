...
the \\\Sec{*WAVE FUNCTIONS} input module}
...
there. The orbitals may then be read by \siraba\ using the \Key{MOSTART}
followed by \Key{FORM18} option on the next line (with this option
\siraba\ will, after having finished the \Sec{*WAVE FUNCTIONS}
...
Running \index{DFT} calculations is very similar to Hartree-Fock, except for
functional specification: The \verb|.DFT| keyword is followed by a
line describing the functional to be used. Simple input below will
...
**END OF DALTON INPUT
\end{verbatim}
Complete list of available functionals can be found in the Reference Guide.
