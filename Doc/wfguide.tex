...
Chapers~\ref{ec:ccexamples} and~\ref{cc:CC}.
...
\Key{RESTART} keyword under \Sec{*WAVE FUNCTION}. However, if the
...
wave functions, we refer to Chapter~\ref{Sec:ccexamples}.
...
!                    are active
.RAS1 SPACE
 1 0 0 0           | the opened core orbital (NOTE: always only this orbital)
.RAS1 ELECTRONS
...
solvation\index{non-equilibrium solvation}, where the molecule will be
enclosed in a spherical cavity. Usually one starts
with a calculation of a reference state (most often the ground
state) with equilibrium solvation, using keyword \Key{INERSF}. The
