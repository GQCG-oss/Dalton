...
theory\index{MP2}\index{M\o ller-Plesset} based on closed-shell
...
numerical gradients. \typeout{Sonia: Note: MP2 is also part of CC.
First order properties and geometry optimization are available.
The latter though is connected to Trygve's new integral
code....not distributed}
...
macro iteration in (MC)SCF, provided the "f21"\index{fort.21}
\typeout{Sonia: SIRIUS.RST} file was saved.  You will then at most
...
\Key{RESTART} keyword under \Sec{*WAVE FUNCTION}. If the correct
two-electron integrals over molecular orbitals are available, you
can skip the integral transformation\index{integral
transformation} in the beginning of the macro iteration by means
of the \Key{OLD TRANSFORMATION} keyword under
\Sec{TRANSFORMATION}. When resubmitting the job, you must make
sure that the correct "f21" \typeout{Sonia: SIRIUS.RST} file is
...
orbitals\index{molecular orbital} already available on the "f21"
\typeout{Sonia: SIRIUS.RST} file, for instance the converged
...
\Key{MOSTART} followed by \Key{FORM18} option  (with this option
\siraba\ will, after having finished the \Sec{*WAVE FUNCTION}
...
ller-Plesset} using the new feature of generating the HF
...
is used for restart in the input. \index{fort.21}\index{restart}
In this case the Hartree-Fock occupation will be then read from
the f21-file and used as initial Hartree-Fock occupation\index{HF
occupation}\index{Hartree-Fock occupation}. The HF occupation is
...
MCSCF-RAS\index{RASSCF} with starting orbitals. We note that and
MCSCF wave function by default will be optimized using
spin-adapted configurations (CSFs)\index{CSF}\index{configuration
state function}, unless the wave function optimization is followed
by a calculation of molecular properties or geometry optimization,
when determinants\index{determinants} will be used. The three
following examples illustrate calculations on excited
states\index{excited state}, on a core hole\index{core hole} state
...
energy\index{non-equilibrium solvation}.
...
from the MOs contained in the f21\index{fort.21} \typeout{Sonia:
SIRIUS.RST} file, as indicated in the following example:
...
\Key{STARTHDIAG} is therefore replaced by \Key{STARTOLDCI}, and
...
solvation\index{non-equilibrium solvation}. Usually one starts
with a calculation of a reference state (most often the ground
state) with equilibrium solvation, using keyword \Key{INESRF}. The
interface file is then used (without user interference) for a
non-equilibrium excited state calculation; keyword \Key{INERSI}.
