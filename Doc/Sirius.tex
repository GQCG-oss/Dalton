...
Functionals in \siraba\ can be divided into two groups: generic and
combined functionals. Combined functionals are a linear combination of
generic ones. One can always create own combined functionals by
using \verb|Combine| general functional. 

\subsubsection{Exchange Functionals}
\providecommand\exfn[1]{#1}
\begin{description}
\item[SLATER] Dirac-Slater exchange functional
\cite{dft:hohenberg,dft:kohn,dft:slater3}.\index{SLATER}

\item[BECKE] 1988 Becke exchange correction presented in reference
  \onlinecite{dft:becke88}. Observe that the full Becke88 exchange functional is
  sum \exfn{Slater} + \exfn{Becke}.\index{BECKE}

\item[BECKE35] Modified \exfn{Becke} exchange correction presented in reference
  \cite{dft:edf1} with respect to the EDF1 functional. The $\beta$ value
  of 0.0042 in \exfn{Becke} is modified to 0.0035. \index{BECKE35}\index{EDF1}
...
\item[B86mx] B86 exchange functional modified with a gradient correction 
  for large density gradients.\cite{dft:b96mgc}\index{B86MGC}

\item[DBx] Double-Becke exchange functional defined by Gill et al.\cite{dft:edf1,dft:edf2} 
  for use in the EDF1 functional. The full DBx functional is defined as
  sum 1.030952*\exfn{Slater} - 8.44793*\exfn{BECKE} + 10.4017*\exfn{BECKE35}
  \index{EDF1}\index{Double-Becke}\index{BECKE}

\item[DK87x] rational function GGA exchange functional (equation 7) from Ref. 
  \onlinecite{dft:dk87},\index{DK87} including Slater exchange.

\item[G96x] 1996 Gill exchange functional, including Slater exchange. \cite{dft:g96}\index{G96}

\item[LRC95x] GGA exchange functional with correct asymptotic behaviour. \cite{dft:lrc96}\index{LRC95}
    The LRC95x exchange functional includes the Slater exchange (eqn 6 from original reference).

\item[KTx] Keal and Tozer GGA exchange functional. Note that the gradient correction 
    pre-factor constant (-0.006 for KT1 and KT2) is not included in the KT 
    definition, but rather in the KT1, KT2 definitions. The full KT exchange
    is given by sum \exfn{Slater} + gamma*\exfn{KT} 
    (gamma=-0.006 for KT1,KT2 and -0.004 for KT3). \cite{dft:kt12}\index{KT}

\item[OPTX] exchange functional correction of Handy \cite{dft:optx}.\index{OPTX}
    The full OPTX exchange functional is given by 
    sum 1.05151*\exfn{Slater} - 1.43169*\exfn{OPTX}.

\item[PBEx] exchange functional.\cite{dft:pbe} \index{PBEx}

\item[revPBEx] revised PBEx exchange functional, with kappa of 1.245.\cite{dft:revpbe}
   \index{revPBE}\index{PBE}

\item[RPBEx] Hammer et al. revised PBEx exchange functional.\cite{dft:revpbe}\index{RPBE}\index{PBE}

\item[mPBEx] modified PBEx exchange functional of Adamo and Barone.\cite{dft:mpbe}\index{mPBE}\index{PBE}

\item[PW86x] Perdew and Wang 1986 exchange functional.\cite{dft:pw86x}.\index{PW86x}

\item[PW91x] Perdew and Wang 1991 exchange functional.\cite{dft:pw91}.\index{PW86x} This
  functional is also given in a separate parameterization in Refs.~\onlinecite{dft:g96,dft:mpw}.

\item[mPW] Adamo and Barone's modified PW91x exchange functional.\cite{dft:pw91,dft:mpw}.\index{PW86x}

\end{description}

\subsection{Correlation Functionals}
\providecommand\corfn[1]{#2}
\begin{description}

\item[VWN3] correlation functional of Vosko, Wilk and Nusair (number
  III) \cite{dft:vwn}.\index{VWN3}

\item[VWN5] correlation functional of Vosko, Wilk and Nusair (number
  V -- the recommended one). The VWN keyword is a synonym for VWN5.\cite{dft:vwn}.\index{VWN5}

\item[LYP] correlation functional by Lee, Yang and Parr
\cite{dft:lyp1,dft:lyp2}.\index{LYP}

\item[LYPr] Modified \corfn{LYP} functional, which is the re-parameterized EDF1 version 
  with modified parameters (0.055, 0.158, 0.25, 0.3505). 
  \cite{dft:lyp1,dft:lyp2,dft:edf1}.\index{LYPr}\index{EDF1}

\item[P86] non-local part of the correlation functional of Perdew
  ...
  sum \corfn{P86} + \corfn{PZ81}.\index{P86}\index{PZ81}

\item[PW91c] correlation functional of Perdew and Wang.
  \cite{dft:pw91}\index{PW91}

\item[PZ81] 1981 local correlation functional of Perdew and Zunger \cite{dft:pz81}.\index{PZ81}

\item[Wigner] spin-polarised correlation functional.\cite{dft:wigner}\index{Wigner}

\item[WL90c] 1990 non-local spin-dependent correlation functional (equation 15 from 
  Ref.~\onlinecite{dft:wl90}).\index{WL90}

\end{description}

\subsubsection{Standalone Functionals}
\providecommand\onefn[1]{#3}
\begin{description}

\item[LB94] assymptotically correct functional of Leeuwen and
Baerends\cite{dft:lb94}. This functional improves description of the
assymptotic density on the expense of core and inner
valence.\index{LB94}

\item[B97] Becke 1997 functional.\cite{dft:b97}\index{B97}

\item[B97-1] Re-parameterization of the B97 functional\cite{dft:b97} in a self-consistent procedure 
  by Hamprecht et al.\cite{dft:b97-1}\index{B97}\index{B97-1}

\item[B97-2] Modification of B97 functional\cite{dft:b97} by Wilson et al.\cite{dft:b97-2}\index{B97}\index{B97-2}

\item[HCTH93] Original HCTH functional, parameterized on a set of 93 training systems.\cite{dft:hcth93} 
  \index{HCTH}\index{HCTH93}

\item[HCTH120] HCTH functional, parameterized on a set of 120 training systems.\cite{dft:hcth120} 
  \index{HCTH}\index{HCTH120}

\item[HCTH147] HCTH functional, parameterized on a set of 147 training systems.\cite{dft:hcth:120}
  \index{HCTH}\index{HCTH147}

\item[HCTH407] HCTH functional, parameterized on a set of 407 training systems.\cite{dft:hcth407}
  \index{HCTH}\index{HCTH407}

\item[HCTH407p] HCTH407 functional re-parameterized on a set of 407 training systems and ammonia dimer
  to incorporate hydrogen bonding.\cite{dft:hcth407p}\index{HCTH}\index{HCTH407}\index{HCTH407p}
\end{description]
...
\item[Combine] is a universal keyword allowing users to manually
  ...
  functionals. Even\index{Combine} fractional Hartree--Fock exchange 
  can be specified. This keyword is to be followed by a string of 
  functionals with associated weights. The syntax is \verb|NAME=WEIGHT ...|. 
...
\item[BVWN] is a sum of Slater functional, Becke88 correction and VWN
...
\item[B3LYP] hybrid functional \cite{dft:b3lyp} equivalent to:
...
program.\index{B3LYPG} This functional can be explicitely set up by
...
\item[BPW91] Becke88 exchange functional and PW91 correlation
  ...
  \funexample{Combine slater=1 becke=1 pw91=1}
  \index{BPW91}

\item[B3PW91] variant of \verb|B3LYP| with VWN5 used for local
  correlation  and PW91 for the nonlocal part.
  \funexample{Combine hf=0.2 slater=0.8 becke=0.72 pw91=0.81 vwn5=1}
  \index{B3PW91}

\item[B3PW91g] variant of \verb|B3LYP| with VWN3 used for local
  correlation  and PW91 for the nonlocal part. 
  This is the form used by the Gaussian quantum chemistry program.
  \funexample{Combine hf=0.2 slater=0.8 becke=0.72 pw91=0.81 vwn3=1}
  \index{B3PW91}\index{B3PW91, Gaussian version}
...
\item[DBLYP] is a sum of the Double-Becke exchange functional and
  the LYP correlation functional.\cite{dft:becke,dft:edf1,dft:lyp}
 \funexample{Combine slater=1.030952 becke=-8.44793 becke35=10.4017 lyp=1}

\item[DBPW91] is a sum of the Double-Becke exchange functional and
  the PW91 correlation functional.\cite{dft:becke,dft:edf1,dft:pw91}
 \funexample{Combine slater=1.030952 becke=-8.44793 becke35=10.4017 pw91=1}

\item[DBP86] is the sum of the Double-Becke exchange functional and
  the P86 correlation functional.\cite{dft:becke,dft:edf1,dft:p86}
 \funexample{Combine slater=1.030952 becke=-8.44793 becke35=10.4017 p86c=1 pz81=1}
...
\item[EDF1] is a fitted functional of Adamson, Gill and Pople.\cite{dft:edf1}
  ...
  functional LYPr.
 \funexample{Combine slater=1.030952 becke=-8.44793 becke35=10.4017 lypr=1}

\item[EDF2] is a linear combination of the Hartree-Fock exchange and the Double-Becke exchange, Slater exchange,
  LYP correlation, revised LYPr correlation and VWN correlation functionals.\cite{dft:edf2}
 \funexample{Combine hf=0.1695 slater=0.2811 becke=0.6227 becke35=-0.0551 vwn=0.3029 lyp=0.5998 lypr=-0.0053}
...
\item[G96PW91] is the sum of the G96 exchange functional and the PW91
  correlation functional.
 \funexample{Combine slater=1 g96x=1 pw91=1}

\item[KT1] Slater-VWN5 functional with the KT GGA correction
...
\item[KT2] differs from KT1 only in that the weights of the Slater and
  VWN5 functionals are from an empirical fit
  \cite{dft:kt12,dft:kt12a}.\index{KT2}
  \funexample{Combine slater=1.07173 vwn=0.576727 kt=-0.006}

\item[KT3] a hybrid functional of Slater, OPTX and KT exhange with the
...
\item[OPW91] is the sum of the OPTX exchange functional with the
  ...
  \funexample{Combine slater=1.05151 optx=-1.43169 pw91=1}
  \index{OLYP}

\item[PBE0] a hybrid functional of Perdew, Burke and Ernzerhof with
  0.25 weight of exact exchange, 0.75 of \verb|PBEx| functional and
  \verb|PBEC| correlation functional \cite{dft:pbe0}.\index{PBE0}
  \funexample{Combine HF=0.25 PBEx=0.75 PBEc=1}

\item[PBE] same as above but with exchange estimated exclusively by
  \verb|PBEx| functional \cite{dft:pbe}.\index{PBE} This is the form
  used by eg. CADPAC and NWChem quantum chemistry programs.
  \funexample{Combine pbex=1 pbec=1}\\
  Observe that quantum chemistry program Molpro uses \verb|PW91c|
  correlation functional instead of \verb|PBEC| which is equivalent to
  following:
  \funexample{Combine PBEx=1 PW91c=1}.

\item[PW91] is the combination of PW91 exchange and correlation functionals.
   \cite{dft:pw91}
...
\item[PW91P86] is the combination of PW91 exchange and PW86 (with Perdew 1981 local) correlation functionals.
   \cite{dft:pw91,dft:pw86,dft:pz81}
  \funexample{Combine PW91x=1 PW86c=1 PZ81=1}.

\item[PW91PW91] is the combination of PW91 exchange and PW91 correlation functionals. Equivalent to 
  PW91.
   \cite{dft:pw91}
  \funexample{Combine PW91x=1 PW91c=1}.
...
\item[\Key{MOSTART}]
   ...
   \item[{\tt EWMO\ }] Start orbitals generated by projecting the EWMO
        ...
        Default inital guess for molecules in which all atoms have a nuclear charge
        ...
        This works a lot better than using a minimal basis set for EMWO.
