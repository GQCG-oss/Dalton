...
Functionals in dalton can be divided into two groups: generic and
...
using \verb|GGAKey| general functional. 

\subsubsection{Generic functionals}
\begin{description}
\item[BECKE] Becke exchange correction (Full Becke functional is BECKE +
  SLATER) from \cite{dft:becke88}.

\item[KT] Keal and Tozer GGA exchange functional \cite{dft:kt12}

\item[LB94] assymptotically correct functional of Leeuwen and
Baerends\cite{dft:lb94}.

\item[LYP] correlation functional by Lee, Yang and Parr
\cite{dft:lyp1,dft:lyp2}.

\item[OPTX] exchange functional of Handy \cite{dft:optx}.

\item[P86] correlation functional of Perdew \cite{dft:p86}.

\item[PBEx] exchange functional 

\item[PW86x]Perdew and Wang exchange functional.\cite{dft:pw86x}.

\item[PW91] correlation functional of Perdew and Wang.\cite{dft:pw91}

\item[PZ81] correlation functional of Perdew and Zunger \cite{dft:pz81}.

\item[SLATER] Dirac-Slater exchange functional
\cite{dft:hohenberg,dft:kohn,dft:slater3}.
...
\item[VWN5]  correlation functional Vosko, Wilk and Nusair (number
  V -- the recommended one) \cite{dft:vwn}.

\end{description}

\subsubsection{Combined functionals}
\begin{description}
\item[GGAKey] it is an universal keyword allowing to manually
  construct arbitrary linear combinations of other functionals. Even
...
\item[B3LYP] hybrid functional \cite{dft:b3lyp}.

\item[B3LYP-G] hybrid functional with VWN3 form used for correlation
  -- this is the form used by Gaussian quantum chemistry program.

\item[BP86] Becke88 exchange functional and Perdew86 correlation
  functional.


\item[B3P86] variant of \verb|B3LYP| with P86 used to get correlation.


\item[CAMB3LYP] Coulomb Attenuated Method Functional of Yanai and
Handy\cite{dft:camb3lyp}.

\item[KT1] Slater-VWN5 functional with the KT GGA correction
 \ref{dft:kt12,dft:kt12a}.

\item[KT2] differs from KT1 only in that the weights of the Slater and
 VWN5 functionals are from an empirical fit \ref{dft:kt12,dft:kt12a}.

\item[KT3] a hybrid functional of Slater, OPTX and KT exhange with the
 LYP correlation functional \ref{dft:kt3}. 

\item[OLYP] is the sum of the OPTX exchange functional with the
 LYP correlation functional \ref{dft:optx}. Mixed functionals such
 as OP86 and OPW91 may be constructed with \item[GGAKey].

\item[PBE0] a hybrid functional of Perdew, Burke and Ernzerhof with
  0.2 weight of exact exchange, 0.75 of \verb|PBEx| functional and
  \verb|PW91c| correlation functional \ref{dft:pbe0}.

\item[PBE] same as above but with exchange estimated exclusively by
  \verb|PBEx| functional \ref{dft:pbe}.

\item[BPW91] is a sum of Slater functional, Becke88 correction and
