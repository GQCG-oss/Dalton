...
In general, functionals in \siraba\ can be divided into two groups: 
...
\item[Becke35] 1998 modified \exfn{Becke} exchange correction presented in reference
  \cite{dft:edf1} for use in the EDF1 functional. The $\beta$ value
  of 0.0042 in \exfn{Becke} is changed to 0.0035.\index{Becke35}\index{EDF1}
...
\item[DBx] Double-Becke exchange functional defined in 1998 by 
  ...
  1.030952*\exfn{Slater} - 8.44793*\exfn{Becke} + 10.4017*\exfn{Becke35}
  \index{EDF1}\index{Double-Becke}\index{Becke}

\item[DK87x] DePristo and Kress' 1987 rational function GGA exchange functional 
  (equation 7) from Ref. \cite{dft:dk87},\index{DK87} which includes Slater exchange.

\item[G96x] Gill's 1996 exchange functional, including Slater exchange~\cite{dft:g96}.\index{G96}

\item[LRC95x] 1995 GGA exchange functional with correct asymptotic behaviour~\cite{dft:lrc95}.\index{LRC95}
...
\item[PW91x] Perdew and Wang 1991 exchange functional~\cite{dft:pw91}.\index{PW86x} This
  functional is also given in a separate parameterization in Refs.~\cite{dft:g96,dft:mpw}.
  The full PW91 exchange functional is given by 
  \exfn{Slater} + \exfn{PW91x}.

\item[mPW] Adamo and Barone's 1998 modified PW91x exchange functional~\cite{dft:pw91,dft:mpw}.\index{mPW}
...
\item[PW91c] correlation functional of Perdew and Wang, 1991~\cite{dft:pw91}\index{PW91}.

\item[PZ81] local correlation functional of Perdew and Zunger, 1981~\cite{dft:pz81}.\index{PZ81}

\item[Wigner] original 1938 spin-polarised correlation functional~\cite{dft:wigner}.\index{Wigner}
...
\item[LB94] assymptotically correct functional of Leeuwen and
  ...
  assymptotic density on the expense of core and inner valence.\index{LB94}

\item[B97] Becke 1997 functional~\cite{dft:b97}.\index{B97}

\item[B97-1] Hamprecht et al.'s 1998 self-consistent re-parameterization of the 
...
\item[BVWN] is a sum of \exfn{Slater} functional, \exfn{Becke} correction and 
...
\item[B1LYP] 1-parameter hybrid functional \cite{dft:b1lyp} equivalent to:
  \funexample{Combine HF=0.25 Slater=0.75 Becke=0.75 LYP=1}
  \index{B3LYP}
...
\item[B3PW91] variant of \verb|B3LYP| with VWN5 used for local
  and PW91 for the nonlocal part of the correlation functional.
  \funexample{Combine HF=0.2 Slater=0.8 Becke=0.72 PW91c=0.81 VWN5=1}
  \index{B3PW91}

\item[B3PW91g] variant of \verb|B3LYP| with VWN3 used for local
  correlation  and PW91 for the nonlocal part. 
  This is the form used by the Gaussian quantum chemistry program.
  \funexample{Combine HF=0.2 Slater=0.8 Becke=0.72 PW91c=0.81 VWN3=1}
...
\item[DBPW91] is a sum of the Double-Becke exchange functional and
 ...
 \funexample{Combine Slater=1.030952 Becke=-8.44793 Becke35=10.4017 PW91c=1}
...
\item[EDF2] is a linear combination of the Hartree-Fock exchange and the Double-Becke exchange, Slater exchange,
  LYP correlation, revised LYPr correlation and VWN correlation functionals \cite{dft:edf2}\index{EDF2}.
...
\item[PBE0] a hybrid functional of Perdew, Burke and Ernzerhof with
  ...
  \verb|PBEC| correlation functional \cite{dft:pbe0}.
  Alternatively called PBE1PBE or PBE0PBE functional.\index{PBE0}
...
\item[PBE] same as above but with exchange estimated exclusively by
  ...
  used by CADPAC and NWChem quantum chemistry programs.
  \funexample{Combine PBEx=1 PBEc=1}\\
  Note that the quantum chemistry program Molpro uses the \corfn{PW91c}
  correlation functional instead of \corfn{PBEc}, which is equivalent to
  the following:
...
\item[PW91P86] is the combination of PW91 exchange and PW86 (with Perdew 1981 local) 
...
\item[PW91PW91] is the combination of PW91 exchange and PW91 correlation functionals. 
  Equivalent to PW91 \cite{dft:pw91}.
  \funexample{Combine PW91x=1 PW91c=1}.
