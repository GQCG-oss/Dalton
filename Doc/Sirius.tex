...
\item[PW91x] Perdew and Wang 1991 exchange functional (the pwGGA-II functional)
  \cite{dft:pw91}.\index{PW86x} This functional is also given in a separate parameterization 
  in Refs.~\cite{dft:g96,dft:mpw}. The full PW91 exchange functional is given by 
  \exfn{Slater} + \exfn{PW91x}.

\item[mPW] Adamo and Barone's 1998 modified PW91x exchange functional~\cite{dft:pw91,dft:mpw}.
  \index{mPW}
...
\item[B3PW91] 3-parameter Becke-PW91 functional, with PW91c non-local
  and PW92c local correlation functional.
...
\funexample{Combine VWN=1 LYP=1} combines the VWN local and LYP non-local 
correlation functional, whereas \funexample{Combine VWN=1 P86=1} 
...
is possible with the Combine keyword.
