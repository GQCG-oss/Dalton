...
\item[\Key{RESTART}]
  ...
  the SIRIUS restart file (\verb|SIRIUS.RST|) must be available
...
calculation,only  one of them can be specified.
...
\item[\Key{HF OCCUPATION}]
  ...
  calculations\index{HF}\index{SCF}\index{Hartree-Fock}\index{MP2}\index{M\o
  ller-Plesset}. This keyword is required is Hartree--Fock or MP2 is
  part of a multistep calculation which includes an MCSCF wave
  function. 
...
added and the core orbital will be relaxed\index{relaxed core
hole}.  When CORERELAX is specified it is assumed that the
...
To direct MP2 calculation\index{MP2}\index{M\o ller-Plesset}. Note
that MP2 energies as well as properties also are available through the
Coupled Cluster module, see Chapter~\ref{ch:CC}.
...
orbital} and to control the
use of supersymmetry\index{supersymmetry}, frozen
...
\item[\Key{MOSTART}]
   ...
   \item[{\tt H1DIAG\ }] Start orbitals generated by projecting the
   Huckel eigenvector in a minimal basis onto the present basis set
   (default for molecules in which all atoms have a nuclear less than
   or equal to 36).
...
unless explicitly enforced with \Key{SUPSYM}. \fi The initial
...
\item[\Key{INERSFINAL}]
  ...
  polarization}. This keyword should also be used to specify the
...
the \Sec{ORBITAL INPUT} section, then SIRIUS will attempt to find
