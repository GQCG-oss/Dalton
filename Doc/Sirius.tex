...
\item[BECKE] Becke exchange correction presented in reference
  \cite{dft:becke88}. Observe that full Becke88 exchange functional is
...
%\item[B86MGC] B86 with modified gradient correction for large density gradients. 
%\cite{dft:b96mgc}\index{B86MGC}
...
\item[G96x] Gill 1996 exchange functional. \cite{dft:g96}\index{G96}
    The full G96 exchange functional is given by 
    sum \exfn{Slater} + \exfn{G96x}.

\item[KTx] Keal and Tozer GGA exchange functional. Note that
    the gradient correction pre-factor constant (-0.006 for 
    KT1 and KT2) is not included in the KT definition, but
    rather in the KT1, KT2 definitions. The full KT exchange
    is given by sum \exfn{Slater} + gamma*\exfn{KT} (gamma=-0.006 for KT1,KT2 and -0.004 for KT3).
...
\item[PBEx] exchange functional \index{PBEx}
...
\item[VWN3] correlation functional Vosko, Wilk and Nusair (number
  III) \cite{dft:vwn}.\index{VWN3}

\item[VWN5]  correlation functional Vosko, Wilk and Nusair (number
  V -- the recommended one) The VWN keyword is synonymous with VWN5. \cite{dft:vwn}.\index{VWN5}
...
\item[P86] non-local part of the correlation functional of Perdew
  ...
  functional. \index{P86}\index{PZ81}
...
\item[Wigner] spin-polarised correlation functional.\cite{dft:wigner,dft:bw}
...
\item[HCTH407p] HCTH407 functional re-parameterized on a set of 407 training systems and ammonia dimer
  to incorporate hydrogen bonding.\cite{dft:hcth407p}
  \index{HCTH}\index{HCTH407}\index{HCTH407p}
\end{description]
...
\item[Combine] it is an universal keyword allowing users to manually
  construct arbitrary linear combinations of other functionals. Even\index{Combine}
  fractional Hartree--Fock exchange can be specified. This keyword is
  to be followed by a string of functionals with associated
  weights. The syntax is \verb|NAME=WEIGHT ...|. Example:
\begin{verbatim}
.DFT
 Combine hf=0.2 slater=0.8 becke=0.72 lyp=0.81 vwn3=0.19
\end{verbatim}

\item[SVWN5] is a sum of Slater functional and VWN5 correlation
  functional.  It is equivalent to
  \funexample{Combine slater=1 vwn5=1}
  \index{SVWN5}

\item[SVWN3] is a sum of Slater functional and VWN3 correlation
