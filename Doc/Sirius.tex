...
using this module described here.
...
\item[\Key{AUTOCCUPATION}]
  ...
  Allow the Hartree--Fock occupation\index{Hartree--Fock occupation}\index{HF occupation} to
...
\item[\Key{ELECTRONS}]
  ...
  keyword is incompatible with the keywords \quotekw{.DOUBLY OCCUPIED}
  and \quotekw{.OPEN SHELL}.
...
Number of singly occupied orbitals in each irreducible representation
...
\item[\Key{NOQCSCF}]
  No quadratically convergent SCF\index{SCF!quadratic convergent} iterations
...
Value of level-shift parameter. The default value is 0.0D0 (no level
  shift).
...
Specify that the orbitals should be localized.
   \kw{READ (LUINP,'(1X,A6)') REWORD} \\
Two options for localization of the orbitals are available:
\begin{description}
\item[{\tt BOYLOC\ }] Use the Boys localization scheme~\cite{}.
...
The use of "super symmetry" can  be activated with the
\quotekw{\Key{SUPSYM}} keyword.
%, for example in finite field
...
more digits in the nuclear coordinates, to change \kw{THRSSY}.
%, or
%to disable super symmetry with \quotekw{.NOSUPSYM}.  
The value of
...
\item[\Key{INERSINITIAL}]
  ...
  polarization}. Note that the optical dielectric constant in this
  only will take effect in case there is a calculation of response
  properties, in which this is an alternative input to the use of the
