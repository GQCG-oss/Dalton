...
such as NMR-parameters (Chapter~\ref{ch:magnetic}). These properties
are in part
...
expectation-value of $A$, due to perturbations $B$, $C$, and $D$, each of
...
x_i;x_j\rangle\!\rangle_\omega$, given a few selected frequencies (in
...
Another special case of residue of quadratic response function 
...
The final ${\cal{B}}(0\to f)$ must be obtained from combination of the individual
components. 
...
all excited specified in {\tt .ROOTS}. Note that the diagonal contributions
...
We mentioned above (Sec.~\ref{subsec:quadrsp}) that the "diagonal" 
