...
in the \Sec{*GENERAL} input section, which ensures that {\eri}
...
\item[\Key{AUXBAS}] An auxiliary basis is used. Basis sets must be identified as
orbital basis and auxiliary basis in the \verb|MOLECULE.INP| file (line 5).
...
\item[\Key{DPTOVL}] DPT integrals: Small-component one-electron
overlap integrals.
...
\item[\Key{DPTPOT}] DPT integrals: Small-component one-electron
...
\item Property labels: \verb|NEF ab  |, where \verb|ab| is the number
...
where $K$ is the nucleus of interest.
\item Property label: \verb|NST abcd|, where \verb|abc| is the number
of the symmetry-adapted nuclear magnetic moment coordinate, and
\verb|cd| refers to the x, y, or z component of the magnetic field.
\end{list}
...
where $K$ is the nucleus of interest.
\item Property label: \verb|NSCOabcd|, where \verb|abc| is the number
of the symmetry-adapted nuclear magnetic moment coordinate, and
...
l}_K}{r_{K}^3}\right|\chi_{\nu}\right>$
where $K$ is the nucleus of interest.
\item Property labels: \verb|NSLOabcd|, where \verb|abc| is the number
of the symmetry-adapted nuclear magnetic moment coordinate, and
...
where $K$ is the nucleus of interest.
\item Property label: \verb|NSNLabcd|, where \verb|abc| is the number
of the symmetry-adapted nuclear magnetic moment coordinate, and
\verb|d| refers to the x, y, or z component of the magnetic field.
\end{list}
...
component (x/y/z) of the third moment tensor.
...
(i.e. \Key{SECMOM}, note: it is NOT the gradient of the \Key{QUADRU} integrals).

\begin{list}{}{}
\item Integral: $\frac{\partial}{\partial {\mathbf
R}_K}\left<\chi_{\mu}\left|{\mathbf r}{\mathbf r}^T\right|\chi_{\nu}\right>$ 
\item Property labels: \verb|abQDG cd|
\end{list}
where \verb|ab| is the symmetry adapted nuclear coordinate, and \verb|cd| the
component (x/y/z) of the second moment tensor.
...
\item Property label: \verb|SOMMabcd|, where \verb|abc| is the number
of the symmetry-adapted nuclear magnetic moment coordinate, and
\verb|d| refers to the x, y, or z component of the magnetic field.
\end{list}
...
moment!traceless} integrals as defined by Buckingham~\cite{adbacp12}.
...
\item[\Key{THIRDM}] Third-moment integrals\index{multipole
integral}\index{third-moment}.
...
\Key{RUNERI} keyword in the \Sec{*GENERAL} input section. Note that
...
\item[\Key{NCLERI}] Calculate only integrals with nonclassiscal
contributions.
