...
\item[\Key{DNS-KE}] Kinetic-energy correction to diamagnetic
...
\item[\Key{DPTPXP}] DPT integrals: Small-component dipole length
...
\item[\Key{FC-KE}] Kinetic energy correction to
  Fermi-contact\index{Fermi contact} integrals, see Ref.~\cite{pmpljvkrjcp119}.
...
\item[\Key{OZ-KE}] Calculates the kinetic energy correction to the
  orbital Zeeman operator, see Ref.~\ref{pmpljvkrjcp119}.
...
operator in finite-perturbation calculations involving and external
...
\item Property labels: \verb|ab-QDB X|, \verb|ab-QDB Y|,
  ...
  gradient operator read in in the variable FIELD3.
\end{list}
...
\item[\Key{SD-KE}] Kinetic energy correction to spin--dipole
  integrals\index{spin-dipole}, see Ref.~\cite{pmpljvkrjcp119}.
...
\item[\Key{WRTLIN}] Write out the lines read in during the input
  ...
  purposes or for analysing input errors in the \verb|MOLECULE.INP|
  file.
...
\item[\Key{DISTRI}] Use distributions for electron 1. Must be used in
  connection with the keywork \Key{SELCT1}.
...
Read in the maximimum number of integral distribution calculated in
each call to the integral code. Default value is 40.

\item[\Key{MXBCH}]\verb| |\newline
\verb|READ (LUCMD,*) MXBCH|

Read in the maximum number of integral batches to treat
