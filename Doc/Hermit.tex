...
indicated by the keyword \Sec{*INTEGRALS}, and the section may be ended
with \Sec{END OF} or any keyword starting with two stars (like
e.g. \Sec{*WAVE FUNCTIONS}). 
The intermediate input is divided into two sections: one general input
section describing what molecular integrals is to be evaluated, and
then a set of modules controling the different parts of the
calculation of undifferentiated atomic integrals and the (possible)
formation of a supermatrix as defined in for
instance~\cite{pemsjaahborjcp74}.
...
the magnetic moment\index{magnetic moment} using London
orbitalsindex{London orbitals} arising from the differentiation on
the London phase factors, see Ref.~\cite{thpjjcp95}. 
...
ytensor integrals without using London atomic orbitals\index{London orbitals}. Note that the
gauge origin is controled by the keyword \Key{GAUGEO}.
...
shielding tensor\index{diamagnetic nuclear shielding} from the differentiation of the Hamiltonian alone\index{London orbitals}, see Ref.~\cite{thpjjcp95}.
...
where $K$ is the nucleus of interest.
\item Property label: \verb|POT.E ab|, where \verb|ab| is the two
...
\item[\Key{S2MAG}] Calculate the second derivate of the overlap
...
\item[\Key{MOLPRI}] Requests that a copy of a standarized molecular
...
generated with the keyword \Key{SORT I}, the keyword \Key{PRESORTED}
has to used in the \Sec{TRANSFORMATION} input module of the \Sec{*WAVE
FUNCTION} input section. For standard calculation you will in general
not need to change any of the deafult settings.

\begin{description}
\item[\Key{INTSYM}]\verb| |\newline
\verb|READ (LUCMD, *) ISNTSYM|

Symmetry of the two-electron integrals that are to
be sorted. Default is totally-symmetric two-electron integrals (INTSYM
= 1).
