...
\item{\Key{NODOIT}} Do not perform the linear transformation of the
  response equation in a direct manner, which is the default. Not
  recommend to use and primarily for debugging purposes.
...
\item{\Key{ANTTES}} Test the antisymmetry of the response
  vector. Mainly for debugging purposes.
...
\verb|READ (LUCMD,*) (NPPSTV(J),J=1,NSYM)|\\
 The number of start vectors to be used in the
  ...
  requested through the keyword \Key{ROOTS}. Mainly for debugging purposes.

\item{\Key{NSIMUL}}\\
\verb|READ (LUCMD,*) (NPPSIM(J),J=1,NSYM)|\\
The number of eigenvectors to solve simultaneously in each
symmetry. Normally decided automatically be the program depending on
available memory and size of eigenvectors. Mainly for debugging purposes.
...
calculation so that no fbyurther response input is required except \Key{ROOTS}; the
