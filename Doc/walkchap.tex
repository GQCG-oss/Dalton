\chapter{Potential energy surfaces}\index{geometry walk}\label{ch:geometrywalks}
...
second-order trust-region \index{second-order optimization}\index{trust region}
optimizations~\cite{tuhjahjajpjjcp84} or a 
variety of first-order\index{first-order optimization}
...
specific mode~\cite{hjajpjthjcp85}\index{mode following}. These
...
(IRC)~\cite{pjhjajthtca73}\index{IRC}\index{intrinsic reaction coordinate}, or 
solve Newtons equations for the nuclei under the potential put up by
the electronic cloud \cite{theuhjajcpl173}\index{dynamics}. Whereas
...
The keyword \Key{MINIMIZE} signalizes a search for a minimum, that is,
...
the final, optimized geometry the input in the \Sec{*FINAL} section requests
...
routines have to be used, indicated by the keyword \Key{WALK}. We also
...
optimization\index{first-order optimization}
...
coordinates\index{cartesian coordinate} and redundant internal
coordinates\index{redundant internal coordinates}, the latter being 
...
convergence criteria\index{convergence criteria}. These will be set
...
gradient\index{norm of gradient} and the norm of the step\index{step
norm}. The threshold for convergence are 
...
In case of non-variational\index{non-variational wave functions} wave
functions, where a numerical gradient\index{numerical gradient} 
is used, the threshold for convergence is reset to 1.0D-4, due to
possible numerical instabilities occuring in the finite
difference\index{finite difference} methods employed to determine the
molecular gradient\index{molecular gradient}. 
...
preoptimization\index{preoptimization}. Note that they will be used in
...
\index{transition state}\index{image surface}
...
image\index{image surface,trust region}
...
second-order surfaces, and are controled by the keywork \Key{WALK} in
the general input section, and the request for a trust-region image
minimzation is achieved  by adding the keyword \Key{IMAGE} in the
\Sec{WALK} input module and the input file will then
look like:
\begin{verbatim}
**DALTON INPUT
.WALK
...
inverted through the keyword \Key{MODE}. However, one 
...
keyword \Key{GRDEXT}. In this example we have chosen to follow the
second-lowest mode, as specified by the keyword \Key{MODE}.
...
can be requested throguh the use of the keyword \Key{INDEX}.
...
coordinate} system. In \siraba , the 
...
The theory behind the ``direct dynamics'' as implemented in \siraba\ is
...
are to be used in the calculation. Thus, the input \Key{ISOTOP} is
...
reduced by the keyword \Key{TRUST}. In the \siraba\ output one will
...
CI\index{MP2}\index{M\o ller-Plesset}\index{CI}\index{configuration
interaction}. However, in order to 
...
numerical gradient can be controled through the keyword \Key{DISPLA}
in the \Sec{MINIMI} input module. Default value is $1.0\cdot 10^{-3}$
...
gradients\index{numerical gradient}. Note that
