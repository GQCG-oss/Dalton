...
The keyword \Key{OPTIMIZE} signals a search for a minimum, that is,
...
step\index{step norm}. The threshold for convergence are for all
...
\index{transition state}\index{image surface} The simplest way of
...
and are controlled by the keyword \Key{WALK} in the general input
section, and the request for a trust-region image minimization is
achieved  by adding the keyword \Key{IMAGE} in the \Sec{WALK}
...
\Key{DISPLA} in the \Sec{MINIMI} input module. Default value is
