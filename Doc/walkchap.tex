\chapter{Potential energy surfaces}\index{geometry
walk}\label{ch:geometrywalks}
...
second-order trust-region \index{second-order
optimization}\index{trust region}
optimizations~\cite{tuhjahjajpjjcp84} (energies, gradients and
Hessians are calculated) or a variety of first-order\index{first-order
optimization} methods~\cite{Fletcher} (only energies and gradients are
...
modules for exploring potential energy surfaces: \Sec{WALK} which is
...
the recommended module is usually \Sec{OPTIMIZE}.

One of the strengths of the \dalton\ program package is the
stable algorithms for locating first-order transition
states\index{transition state}. As described below,
this can done by one of three algorithms in \Sec{WALK}: by trust-region
second-order image surface minimization~\cite{thcpl182}\index{image
surface}, by gradient extremal
walks~\cite{pjhjajthtca73}\index{gradient extremal}, or by following a
specific mode~\cite{hjajpjthjcp85}\index{mode following} (all
...
well as a partitioned rational function method~\cite{vb001}.
...
(IRC)~\cite{pjhjajthtca73}\index{IRC}\index{intrinsic reaction
coordinate}, or solve Newtons equations for the nuclei under the
potential put up by the electronic cloud
\cite{theuhjajcpl173}\index{dynamics}. Whereas the first option
...
\item 1.st-order methods: V.Bakken, and T.Helgaker, To be published.
...
(using a second-order optimization\index{second-order optimization}
...
described in Ref.~\cite{tuhjahjajpjjcp84}. With the \Sec{OPTIMIZE}
module the second-order method has to be explicitly requested
(\Key{2NDORD}), as first-order methods are the default. At the final,
...
symmetry\index{symmetry breaking} of the molecule, if needed in order
to reach a minimum
...
analysis\index{population analysis}, dipole gradients\index{dipole
gradient} or vibrational circular dichroism (VCD)\index{vibrational
circular dichroism}\index{VCD} will be ignored as all the necessary
...
optimization\index{first-order optimization} routines as
...
coordinates~\cite{gfxfzpwtppjacs114,ppgfjcp96,vbth}\index{redundant
internal coordinates} using the BFGS\index{BFGS}\index{Hessian update}
...
Hessian updating scheme\index{BFGS}\index{Hessian update}. Though other
...
trust-region\index{trust region}
...
coordinates\index{cartesian coordinate} and redundant
internal coordinates\index{redundant internal coordinates},
...
The \Sec{OPTIMIZE} decides when a minimum is reached through a set of
convergence criteria\index{convergence criteria}. These will be
...
step\index{step norm}. The convergence threshold for the change
...
criterias, is to use the keyword \Key{BAKER} enforcing the convergence
criterias of Baker\cite{Baker} (see the Reference Manual for
...
wave functions, where a numerical gradient\index{numerical
gradient} is used, the threshold for convergence is reset to
...
We also note that for optimization of excited states\index{excited
state} where the state of interest is not the lowest of its
...
preoptimization\index{preoptimization}, that is,
...
preoptimization\index{preoptimization}. Note that they will be
...
\Sec{*FINAL}. The \Sec{OPTIMIZE} and \Sec{WALK} is
...
\index{transition state}Transition states are found as saddle points
on the potential energy surface. \index{image surface} The simplest
way of locating a first-order transition state, which are the
chemically most interesting ones, is to use the trust-region
image\index{image surface,trust region} minimization method
...
it using Bofill's update\cite{vb002}\index{Bofill's update}\index{Hessian
update} (the BFGS update is not suitable since it tends towards a
positive definite Hessian). As for minimizations, redundant internal
coordinates are used by default for first-order methods.
...
coordinates\index{mass-weighted coordinate}, it is recommended to
...
The input needed for doing a level-shifted mode following\index{mode
following} is very
similar to the input for following a gradient extremal\index{gradient
extremal}, and the
...
molecular coordinates\index{mass-weighted coordinate} as default, and
...
approach~\cite{kfacr14,pmjcp88}\index{IRC}\index{intrinsic reaction
coordinate}. The IRC is calculated by taking small steps along the
negative gradient in a mass-weighted coordinate\index{mass-weighted
...
coordinates\index{mass-weighted coordinate}, care
has to be taken in order to specify the correct isotopic substitution
of the molecule through the keyword \Key{ISOTOP} in the
\Sec{WALK} input module. In this example we have assumed that the
...
Chem.Phys.Lett.}, {\bf 173},\hspace{0.25em}145, (1993).
\end{list}
}}
\end{center}
...
That the walk is to
be a dynamic walk is specified through the keyword \Key{DYNAMI}. The
...
\Key{ISOTOP} is redundant for this calculation, as the given
...
basis set superposition errors\index{BSSE}\index{basis set superposition error} and
vibrational\index{vibrational excitation} and rotational
...
CI\index{CI}\index{configuration interaction}. However, in order to
...
\Key{DISPLA} in the \Sec{OPTIIMI} input module. Default value is
