...
linear response\index{frequency}\index{linear response}\index{response!linear}
calculations. At present this only  
includes the frequency dependent polarizability\index{polarizability} as well as directives
controlling the calculation of Raman related
properties\index{Raman intensity}\index{Raman optical activity}\index{ROA}. Combined with the 
keyword \Key{SOPPA} it invokes a SOPPA\index{SOPPA} calculation of the 
...
undifferentiated solvent integrals in the {\her} section.
...
automatically the .NOLOND option since the CTOCD-DZ formalism is already truely gauge 
independent (for Nuclear Magnetic Shieldings). The default gauge origin is choosen to be 
the center of mass.  \Key{CTOCD} and \Key{SOPPA} can be combined to perform CTOCD calculations 
at the SOPPA level.\index{SOPPA}
...
moments\index{transition moment}\index{rotatory strength} and 
rotatory strengths. Combined with the 
keyword \Key{SOPPA} it invokes a SOPPA\index{SOPPA} calculation of electronic
...
\item[\Key{INPTES}] Checks  only the input and then stops.
...
and walks using mass-weighted normal\index{mass-weighted coordinates} 
...
Combined with the keyword \Key{SOPPA} it invokes a SOPPA\index{SOPPA} 
calculation of the magnetizability and the rotational {\em g} tensor. London 
orbitals are automatically disabled in SOPPA\index{SOPPA} calculations.
...
wave function, the program will give wrong results unless the symmetry
of the electronic wave function is reduced to that of the vibrational
part.
Combined with the keyword \Key{SOPPA} it invokes a SOPPA\index{SOPPA} 
calculation of the magnetizability and the rotational {\em g} tensor. London 
orbitals are automatically disabled in SOPPA\index{SOPPA} calculations.
...
Combined with the keyword \Key{SOPPA} it invokes a SOPPA\index{SOPPA} 
calculation of the Nuclear Magnetic Shieldings. London 
orbitals are automatically disabled in SOPPA\index{SOPPA} calculations. Gauge origin
independent SOPPA calcualtions of Nuclear Magnetic Shieldings can be carried
out with the CTOCD-DZ method\index{CTOCD-DZ} (Ref.~\cite{paololazz1,paololazz2,ctocd}) 
(keyword \Key{CTOCD}).
...
approach~\cite{mjpekdtehjajjojcp}.\index{SOPPA} This requires that the MP2 energy and wave
...
wave functions (or MP2 wave functions used together with the keyword
\verb|.SOPPA|) in order to get reliable results, it is recommended to 
...
response\index{frequency}\index{linear response}\index{response!linear}
functions. At present these directives only affect the 
...
\begin{description}
\item[\Key{ALFA}] Calculates the frequency-dependent
...
\begin{description}
\item[\Key{AOMAT}]  Print in AO as well as SO basis dipole
...
energies. This will give wrong results for ECD and first-order
...
\item[\Key{TRIPLE}]
\index{excitation energy!triplet}
...
\begin{description}
\item[\Key{ALL CO}] Indicates that all components of the nuclear
...
and dihedral angles appear in the \verb|*GEOANA| section. By default all
...
Set the natural abundance\index{abundance} threshold for discarding
couplings between 
...
which includes both proton and $^{13}$C nuclei.
...
spin-spin couplings by only solving one instead of ten response
