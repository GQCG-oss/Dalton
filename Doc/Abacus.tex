...
intensity}\index{Raman optical activity}\index{ROA}. Combined with the 
...
calculation. By default this is chosen to be the center of
...
Tells the program to use the Hellman--Feynman approximation when calculating
the molecular gradients and
Hessians~\cite{hhbook,rpfpr56,vbthwkkrmp96} that is, all 
contributions to 
the molecular gradient and Hessians from differentiation of the orbitals are
ignored. Requires large basis sets in order to give reliable results, but does
not require any differentiated two-electron integrals.
...
\item[\Key{OPTORB}] Use optimal orbital trial vectors\index{optimal
orbital trial vector} in the
iterative solution of the eigenvalue equations.~\index{linear response}
...
\item[\Key{TRIPLE}]
Indicates that it is triplet excitation energies\index{excitation energy,
triplet} that is to be calculated. 
\end{description}
...
Set the natural abundance\index{abundance} threshold for discarding couplings between
