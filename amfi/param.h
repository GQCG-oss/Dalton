...
cbs   ################ parameter block ##############################################
cbs   ###############################################################################
      include 'para.h'  ! the dimension-parameters 
      parameter (MxcontL4=MxcontL*MxcontL*MxcontL*MxcontL) 
      parameter (MxprimL4=MxprimL*Mxpriml*MxprimL*MxprimL)                       
      parameter (Lanz=Lmax+1)   ! number of angular momenta                       
      parameter (intblmx=(Lanz+1)*(Lanz+1)*Lanz*Lanz/4+1)   ! max number of integral blocks (l1,l2,l3,l4)     
cbs   surely, this number can be reduced by using more properly 
cbs   symmetry,parity conservation and selection rules
cbs   ###############################################################################
cbs   ################ parameter block ##############################################
cbs   ###############################################################################
...
cbs   the numbers of primitive and contracted functions for each l-value   
      common /dims/ nprimit(0:Lmax),ncontrac(0:Lmax) 
cbs   scratch should explain itself ...........
      common /scratch/ scratch1(MxprimL4),scratch2(MxprimL4),
     *primints(MxprimL4),scratch4(4*MxprimL*MxprimL),
     *mcombina(2,-Lmax:Lmax,-Lmax:Lmax,-Lmax:Lmax,-Lmax:Lmax),
     *cntscrtch(MxprimL,MxcontL,0:Lmax) 
      common /contint/ cont4SO(icont4),cont4OO(icont4),
     *Lfirst(4,intblmx),
     *Llast(4,intblmx),Lblocks(4,intblmx),Lstarter(4,intblmx),
     *nblock,Lvalues(4,intblmx)
...
cbs   
      common/spherint/ angintSO(mxangint),angintOO(mxangint),
     *iangfirst(intblmx)
...
cbs   common block with the cartesian integrals 
      common /cartint/ carteXSO(mxicart),carteYSO(mxicart),
     *carteZSO(mxicart),
     *carteXOO(mxicart),carteYOO(mxicart),carteZOO(mxicart),
     *mcombcart(2,-Lmax:Lmax,-Lmax:Lmax,-Lmax:Lmax,-Lmax:Lmax,intblmx),
     *istartX(intblmx),istartY(intblmx),istartZ(intblmx),
     *isignM(-Lmax:Lmax)
...
cbs   4. index: L-value  
     ...
     *Xireds(Mxcart*Mxcart),
     *Yireds(Mxcart*Mxcart),Zireds(Mxcart*Mxcart)
