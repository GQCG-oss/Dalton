...
#define ENERGY_FUNCTION(FUN) FUN##_energy

template<enum xc_parameters fun, class T>
T (*get_energy_fun(void))(const densvars<T> &d); 

template<enum xc_parameters fun, class T>
struct energy_functions
{
  static T (*f)(const densvars<T> &d);
};

template<int fun>
void setup_functional(functional &f);

#define SETUP_FUNCTION(FUN) template<> void setup_functional<FUN>

#define SETUP_LDA(FUN) \
  template<> taylor<ireal_t,1,0> (*get_energy_fun<FUN,taylor<ireal_t,1,0> >(void))(const densvars<taylor<ireal_t,1,0> > &d) { return ENERGY_FUNCTION(FUN)<taylor<ireal_t,1,0> >; }\
template<> void setup_functional<FUN>
