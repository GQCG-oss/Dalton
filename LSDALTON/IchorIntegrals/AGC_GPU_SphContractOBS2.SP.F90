module SPAGC_GPU_OBS_Sphcontract2Mod
!Automatic Generated Code (AGC) by runSphContractOBS2.f90 in tools directory
use IchorPrecisionMod
  
 CONTAINS
  
 subroutine SPSphericalContractOBS2_GPU_maxAngQ2_maxAngC2(nlmP,nContPasses,IN,OUT,iASync)
  implicit none
  integer,intent(in)        :: nlmP,nContPasses
  real(reals),intent(in)    :: IN(nContPasses*nlmP,  6)
  real(reals),intent(inout) :: OUT(nContPasses*nlmP,  5)
...
!$ACC PARALLEL LOOP PRIVATE(iP) PRESENT(IN,OUT) ASYNC(iASync)
  ...
  ENDDO
end subroutine SPSphericalContractOBS2_GPU_maxAngQ2_maxAngC2 
  
  
 subroutine SPSphericalContractOBS2_GPU_maxAngQ2_maxAngC0(nlmP,nContPasses,IN,OUT,iASync)
  implicit none
  integer,intent(in)        :: nlmP,nContPasses
  real(reals),intent(in)    :: IN(nContPasses*nlmP,  6)
  real(reals),intent(inout) :: OUT(nContPasses*nlmP,  5)
...
!$ACC PARALLEL LOOP PRIVATE(iP) PRESENT(IN,OUT) ASYNC(iASync)
  DO iP=1,nContPasses*nlmP
    ...
    OUT(iP,5) = IN(iP,1)*SPHMAT1_5 + IN(iP,4)*SPHMAT4_5
  ENDDO
end subroutine SPSphericalContractOBS2_GPU_maxAngQ2_maxAngC0 
  
  
 subroutine SPSphericalContractOBS2_GPU_maxAngQ3_maxAngC2(nlmP,nContPasses,IN,OUT,iASync)
  implicit none
  integer,intent(in)        :: nlmP,nContPasses
  real(reals),intent(in)    :: IN(nContPasses*nlmP, 18)
  real(reals),intent(inout) :: OUT(nContPasses*nlmP, 15)
  integer(kind=acckind),intent(in) :: iASync
  integer :: iPass,ijkP,iP
  real(reals),parameter :: SPHMAT1_3      =   -2.8867513459481292E-01_reals
  real(reals),parameter :: SPHMAT1_5      =    5.0000000000000000E-01_reals
...
!$ACC PARALLEL LOOP PRIVATE(iP) PRESENT(IN,OUT) ASYNC(iASync)
  ...
  ENDDO
end subroutine SPSphericalContractOBS2_GPU_maxAngQ3_maxAngC2 
  
  
 subroutine SPSphericalContractOBS2_GPU_maxAngQ3_maxAngC1(nlmP,nContPasses,IN,OUT,iASync)
  implicit none
  integer,intent(in)        :: nlmP,nContPasses
  real(reals),intent(in)    :: IN(nContPasses*nlmP, 18)
  real(reals),intent(inout) :: OUT(nContPasses*nlmP, 15)
...
!$ACC PARALLEL LOOP PRIVATE(iP) PRESENT(IN,OUT) ASYNC(iASync)
  ...
  ENDDO
end subroutine SPSphericalContractOBS2_GPU_maxAngQ3_maxAngC1 
  
  
 subroutine SPSphericalContractOBS2_GPU_maxAngQ4_maxAngC2(nlmP,nContPasses,IN,OUT,iASync)
  implicit none
  integer,intent(in)        :: nlmP,nContPasses
  real(reals),intent(in)    :: IN(nContPasses*nlmP, 36)
  real(reals),intent(inout) :: OUT(nContPasses*nlmP, 25)
  integer(kind=acckind),intent(in) :: iASync
  integer :: iPass,ijkP,iP
...
!$ACC PARALLEL LOOP PRIVATE(iP) PRESENT(IN,OUT) ASYNC(iASync)
  DO iP=1,nContPasses*nlmP
    ...
    OUT(iP,25) = IN(iP,1)*SPHMAT1_25 + IN(iP,4)*SPHMAT4_25 + IN(iP,19)*SPHMAT4_25 &
               & + IN(iP,22)*SPHMAT1_25
  ENDDO
end subroutine SPSphericalContractOBS2_GPU_maxAngQ4_maxAngC2 
  
  
END module SPAGC_GPU_OBS_Sphcontract2Mod
