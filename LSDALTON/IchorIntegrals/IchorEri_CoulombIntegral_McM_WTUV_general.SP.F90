...
!> \author T. Kjaergaard
!> \date 2013 
module SPIchorEriCoulombintegralCPUMcMGeneralWTUVMod
...
CONTAINS
  subroutine SPIchorwtuvRecurrenceCurrent(OLD,CUR,J,JMAX,nPrimQ,nPrimP,nPasses,&
       & ntuv,Rpq,RJ000)
    implicit none 
    integer,intent(in)     :: J,nPrimQ,nPrimP,nPasses,ntuv,JMAX
    real(reals),intent(in) :: Rpq(nPrimQ*nPrimP*nPasses,3)
    real(reals),intent(in) :: RJ000(0:JMAX,nPrimQ*nPrimP*nPasses)
    Real(reals),intent(in) :: OLD(nPrimQ*nPrimP*nPasses,ntuv) 
    Real(reals),intent(inout) :: CUR(nPrimQ*nPrimP*nPasses,ntuv) 
    !
    !local variables
    ...
    real(reals) :: Xtemp,Ytemp,Ztemp,WJ,WJ1,WJ2,WJ3
    real(reals) :: W100,W010,W001,W200,W110,W101,W020,W011,W002
    ...
    DO n=1,nPrimQ*nPrimP*nPasses
       ...
       CUR(n,11) = 2.0E0_reals*W100 + Xtemp*W200 !300
       ...
       CUR(n,17) = 2.0E0_reals*W010 + Ytemp*W020 !030
       CUR(n,18) =       W001 + Ytemp*W011       !021
       CUR(n,19) =       W010 + Ztemp*W011       !012
       CUR(n,20) = 2.0E0_reals*W001 + Ztemp*W002 !003
    ENDDO
    !$OMP END DO
    ituv = 20
    DO k=4,JMAX-j
...
!$OMP END DO
       ...
       ENDDO
    ENDDO
  end subroutine SPIchorwtuvRecurrenceCurrent
...
!    Version that use the cheaper calculation
  ...
  subroutine SPIchorwtuvRecurrenceJMIN0JMAX0(WJ000,WTUV,nPrim)
    implicit none
    Integer,intent(in)           :: nPrim
    Real(reals),intent(in)       :: WJ000(0:0,nPrim)
    Real(reals),intent(inout)    :: WTUV(nPrim,1)
    !
    integer :: n
!$OMP DO PRIVATE(n)
    DO n=1,nPrim
    ...
    ENDDO
!$OMP END DO
  END subroutine SPICHORWTUVRECURRENCEJMIN0JMAX0

  subroutine SPichorwtuvrecurrenceJMIN0JMAX1(WJ000,WTUV,Rpq,nPrim)
    implicit none
    Integer,intent(in)           :: nPrim
    Real(reals),intent(in)       :: WJ000(0:1,nPrim),Rpq(nPrim,3)
    Real(reals),intent(inout)    :: WTUV(nPrim,4)
    !
    integer :: n
!$OMP DO PRIVATE(n)
    DO n=1,nPrim
    ...
    ENDDO
!$OMP END DO
  END subroutine SPICHORWTUVRECURRENCEJMIN0JMAX1
  
subroutine SPichorwtuvRecurrenceJMIN0JMAX2(WJ000,WTUV,Rpq,nPrim)
  implicit none
  Integer,intent(in)           :: nPrim
  Real(reals),intent(in)       :: WJ000(0:2,nPrim),Rpq(nPrim,3)
  Real(reals),intent(inout)    :: WTUV(nPrim,10)
  !
  integer :: n
  !000
!$OMP DO PRIVATE(n)
  ...
  ENDDO
!$OMP END DO
end subroutine SPichorwtuvRecurrenceJMIN0JMAX2

subroutine SPichorwtuvRecurrenceJMIN0JMAX3(WJ000,WTUV,Rpq,nPrim)
  implicit none
  Integer,intent(in)           :: nPrim
  Real(reals),intent(in)       :: WJ000(0:3,nPrim),Rpq(nPrim,3)
  Real(reals),intent(inout)    :: WTUV(nPrim,20)
  !
  Real(reals) :: WTUVTEMP_21,WTUVTEMP_22
  Real(reals) :: WTUVTEMP_31,WTUVTEMP_32
  Real(reals) :: WTUVTEMP_41,WTUVTEMP_42
  Real(reals) :: WTUVTEMP25,WTUVTEMP28
  Real(reals) :: WTUVTEMP210,WTUVTEMP29
  integer :: n
!$OMP DO PRIVATE(n)
  DO n=1,nPrim
     WTUV(n,1) = WJ000(0,n)
  ...
  ENDDO
!$OMP END DO
END subroutine SPichorWTUVRECURRENCEJMIN0JMAX3

subroutine SPichorwtuvRecurrenceJMIN0JMAX3J(WJ000,WTUV,Rpq,nPrim,maxJ,J)
  implicit none
  Integer,intent(in)           :: nPrim,maxJ,J
  Real(reals),intent(in)       :: WJ000(0:maxJ,nPrim),Rpq(nPrim,3)
  Real(reals),intent(inout)    :: WTUV(nPrim,20)
  !
  Real(reals) :: WTUVTEMP_21,WTUVTEMP_22
  Real(reals) :: WTUVTEMP_31,WTUVTEMP_32
...
!$OMP DO PRIVATE(n)
  ...
  ENDDO
!$OMP END DO
END subroutine SPichorwtuvRecurrenceJMIN0JMAX3J

! WARNING CHANGE ORDER OF WTUV 
!!$
!!$subroutine SPichorwtuvRecurrenceJMIN0JMAX4(WJ000,WTUV,Rpq,nPrim)
!!$  implicit none
!!$  Integer,intent(in)           :: nPrim
!!$  Real(reals),intent(in)       :: WJ000(0:4,nPrim),Rpq(nPrim,3)
!!$  Real(reals),intent(inout)    :: WTUV(35,nPrim)
!!$  !
!!$  Integer     :: n,M
!!$  Real(reals) :: WTUVTEMP(2:4,3)
!!$  Real(reals) :: WTUVTEMP2(5:10,2)
!!$  Real(reals) :: WTUVTEMP3(11:20)
!!$  !000
!!$  DO n=1,nPrim
!!$     WTUV(1,n) = WJ000(0,n)
!!$  ENDDO
...
!!$  ENDDO
!!$END subroutine SPichorwtuvRecurrenceJMIN0JMAX4
!!$
!!$subroutine SPichorwtuvRecurrenceJMIN0JMAX5(WJ000,WTUV,Rpq,nPrim)
!!$  implicit none
!!$  Integer,intent(in)           :: nPrim
!!$  Real(reals),intent(in)       :: WJ000(0:5,nPrim),Rpq(nPrim,3)
!!$  Real(reals),intent(inout)    :: WTUV(nPrim,56)
!!$  !
!!$  Integer     :: n,M
!!$  Real(reals) :: WTUVTEMP(2:4,4)
!!$  Real(reals) :: WTUVTEMP2(5:10,3)
!!$  Real(reals) :: WTUVTEMP3(11:20,2)
!!$  Real(reals) :: WTUVTEMP4(21:35)
!!$  !000
!!$  DO n=1,nPrim
!!$     WTUV(n,1) = WJ000(0,n)
!!$  ENDDO
...
!!$  ENDDO
!!$END subroutine SPichorwtuvRecurrenceJMIN0JMAX5
!!$
!!$subroutine SPichorwtuvRecurrenceJMIN0JMAX6(WJ000,WTUV,Rpq,nPrim)
!!$  implicit none
!!$  Integer,intent(in)           :: nPrim
!!$  Real(reals),intent(in)       :: WJ000(0:6,nPrim),Rpq(nPrim,3)
!!$  Real(reals),intent(inout)    :: WTUV(nPrim,84)
!!$  !
!!$  Integer     :: n,M
!!$  Real(reals) :: WTUVTEMP(2:4,5)
!!$  Real(reals) :: WTUVTEMP2(5:10,4)
!!$  Real(reals) :: WTUVTEMP3(11:20,3)
!!$  Real(reals) :: WTUVTEMP4(21:35,2)
!!$  Real(reals) :: WTUVTEMP5(36:56,1)
!!$  !000
!!$  !ERROR IN THIS subroutine SPSOMEWHERE
!!$  DO n=1,nPrim
!!$     WTUV(n,1) = WJ000(0,n)
!!$  ENDDO
!!$  !it can reuse registers with this collection of this in 1 loop
...
!!$  ENDDO
!!$END subroutine SPichorwtuvRecurrenceJMIN0JMAX6
!!$
!!$subroutine SPichorwtuvRecurrenceJMIN0JMAX7(WJ000,WTUV,Rpq,nPrim)
!!$  implicit none
!!$  Integer,intent(in)           :: nPrim
!!$  Real(reals),intent(in)       :: WJ000(0:7,nPrim),Rpq(nPrim,3)
!!$  Real(reals),intent(inout)    :: WTUV(nPrim,120)
!!$  !
!!$  Integer     :: n,M
!!$  Real(reals) :: WTUVTEMP(2:4,6)
!!$  Real(reals) :: WTUVTEMP2(5:10,5)
!!$  Real(reals) :: WTUVTEMP3(11:20,4)
!!$  Real(reals) :: WTUVTEMP4(21:35,3)
!!$  Real(reals) :: WTUVTEMP5(36:56,2)
!!$  Real(reals) :: WTUVTEMP6(57:84,1)
!!$  !000
!!$  DO n=1,nPrim
!!$     WTUV(n,1) = WJ000(0,n)
!!$  ENDDO
...
!!$  ENDDO
!!$END subroutine SPichorwtuvRecurrenceJMIN0JMAX7
!!$
!!$subroutine SPichorwtuvRecurrenceJMIN0JMAX8(WJ000,WTUV,Rpq,nPrim)
!!$  implicit none
!!$  Integer,intent(in)           :: nPrim
!!$  Real(reals),intent(in)       :: WJ000(0:8,nPrim),Rpq(nPrim,3)
!!$  Real(reals),intent(inout)    :: WTUV(nPrim,165)
!!$  !
!!$  Integer     :: n,M
!!$  Real(reals) :: WTUVTEMP(2:4,7)
!!$  Real(reals) :: WTUVTEMP2(5:10,6)
!!$  Real(reals) :: WTUVTEMP3(11:20,5)
!!$  Real(reals) :: WTUVTEMP4(21:35,4)
!!$  Real(reals) :: WTUVTEMP5(36:56,3)
!!$  Real(reals) :: WTUVTEMP6(57:84,2)
!!$  Real(reals) :: WTUVTEMP7(85:120,1)
!!$  !000
!!$  DO n=1,nPrim
!!$     WTUV(n,1) = WJ000(0,n)
!!$  ENDDO
...
!!$  ENDDO
!!$END subroutine SPichorwtuvRecurrenceJMIN0JMAX8

subroutine SPPrintWTUV(WTUV,AngmomPQ,nPrimQP,nPasses,nTUV,lupri)
  implicit none
  integer,intent(in) :: AngmomPQ,nPrimQP,nPasses,nTUV,lupri
  real(reals),intent(in) :: WTUV(nPrimQP,nPasses,nTUV) 
  !
  ...
  DO iPass = 1,nPasses
     ...
     ENDDO
  ENDDO
end subroutine SPPrintWTUV

end module SPIchorEriCoulombintegralCPUMcMGeneralWTUVMod
