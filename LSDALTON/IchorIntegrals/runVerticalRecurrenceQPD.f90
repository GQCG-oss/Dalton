...
!    logical :: CREATED(-2:8,-2:8,-2:8)
    ...
    WRITE(*,'(A)')'subroutine VerticalRecurrence1D(nPasses,nPrimP,nPrimQ,reducedExponents,&'
    WRITE(*,'(A)')'         & TABFJW,Qexp,Dcenter,Pcent,Qcent,integralPrefactor,PpreExpFac,&'
...
!    WRITE(*,'(A)')'!  REAL(REALK),intent(in) :: RJ000(0:1,nPrimQ,nPrimP,nPasses)'
    ...
    WRITE(*,'(A)')'  real(realk) :: Dx,Dy,Dz,Pexpfac,PX,PY,PZ,Xqd,Yqd,Zqd,RJ000(0:1)'
    ...
    WRITE(*,'(A)')'   Dx = -Dcenter(1,iPassQ)'
    WRITE(*,'(A)')'   Dy = -Dcenter(2,iPassQ)'
    WRITE(*,'(A)')'   Dz = -Dcenter(3,iPassQ)'
    ...
    DO JMAX=2,MaxAngmomQP
       ...
       WRITE(*,'(A)')''
       WRITE(*,'(A,I1,A)')'subroutine VerticalRecurrence',JMAX,'D(nPasses,nPrimP,nPrimQ,reducedExponents,&'
       WRITE(*,'(A)')'         & TABFJW,Qexp,Dcenter,Pcent,Qcent,integralPrefactor,PpreExpFac,QpreExpFac,&'
       WRITE(*,'(A)')'         & AUXarray)'
       WRITE(*,'(A)')'  implicit none'
       WRITE(*,'(A)')'  integer,intent(in) :: nPasses,nPrimP,nPrimQ'
       WRITE(*,'(A)')'  real(realk),intent(in) :: reducedExponents(nPrimQ,nPrimP),Qexp(nPrimQ)'
       ...
       WRITE(*,'(A)')'  real(realk),intent(in) :: Pcent(3,nPrimP),Dcenter(3,nPasses),Qcent(3,nPrimQ,nPasses)'
       WRITE(*,'(A,I2,A)')'  REAL(REALK),intent(in) :: TABFJW(0:',JMAX+3,',0:1200)'
       WRITE(*,'(A)')'  !Local variables'
       WRITE(*,'(A)')'  integer :: iPassQ,iPrimP,iPrimQ,IPNT,IP'
       WRITE(*,'(A)')'  real(realk) :: Dx,Dy,Dz,Pexpfac,invexpQ,inv2expQ,PX,PY,PZ,Xqd,Yqd,Zqd,WVAL'
