MODULE AGC_OBS_VERTICALRECURRENCEMODC
 use IchorPrecisionModule
  
 CONTAINS
subroutine VerticalRecurrence1C(nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD,&
  ...
  real(realk),intent(in) :: Pcent(3,nPrimP),Qcent(3,nPrimQ,nPasses),Ccenter(3,nAtomsC)
  real(realk),intent(inout) :: AUXarray(4,nPrimQ,nPrimP,nPasses)
  !local variables
  integer :: iPassQ,iPrimP,iPrimQ,ipnt,iAtomC
  real(realk) :: Cx,Cy,Cz,Pexpfac,mPX,mPY,mPZ,Xqc,Yqc,Zqc,RJ000(0:1)
  ...
  DO iPassQ = 1,nPasses
   ...
   enddo
  enddo
end subroutine VerticalRecurrence1C

subroutine VerticalRecurrence2C(nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD,&
...
!  REAL(REALK),intent(in) :: RJ000(0:2,nPrimQ*nPrimP*nPasses)
  ...
  real(realk),intent(in) :: Pcent(3,nPrimP),Ccenter(3,nAtomsC),Qcent(3,nPrimQ,nPasses)
  REAL(REALK),intent(in) :: TABFJW(0: 5,0:1200)
  !Local variables
  integer :: iPassQ,iPrimP,iPrimQ,IPNT,IP,iAtomC
  real(realk) :: Cx,Cy,Cz,Pexpfac,invexpQ,inv2expQ,mPX,mPY,mPZ,Xqc,Yqc,Zqc,WVAL
  ...
  REAL(REALK),PARAMETER :: SQRPIH = SQRTPI/D2
  REAL(REALK),PARAMETER :: PID4 = PI/D4, PID4I = D4/PI
  !TUV(T,0,0,N) = Xqc*TUV(T-1,0,0,N) + (alpha/p)*Xpq*TUV(T-1,0,0,N+1)
...
!      RJ000(2)=  2.0000000000000001E-01_realk
   ...
   ENDDO
  ENDDO
end subroutine VerticalRecurrence2C

subroutine VerticalRecurrence3C(nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD,&
         & reducedExponents,TABFJW,Qexp,Ccenter,Pcent,Qcent,integralPrefactor,&
         & PpreExpFac,QpreExpFac,AUXarray)
  implicit none
  integer,intent(in) :: nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD
...
!      RJ000(3)=  1.4285714285714285E-01_realk
   ...
   ENDDO
  ENDDO
end subroutine VerticalRecurrence3C

subroutine VerticalRecurrence4C(nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD,&
         & reducedExponents,TABFJW,Qexp,Ccenter,Pcent,Qcent,integralPrefactor,&
         & PpreExpFac,QpreExpFac,AUXarray)
  implicit none
  integer,intent(in) :: nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD
...
!      RJ000(4)=  1.1111111111111110E-01_realk
     ...
     TwoTerms(  2) = inv2expQ*(AuxArray(  6,IP) + alphaQ*TmpArray3(  6,2))
     TwoTerms(  3) = inv2expQ*(AuxArray(  7,IP) + alphaQ*TmpArray3(  7,2))
     TwoTerms(  4) = inv2expQ*(AuxArray(  8,IP) + alphaQ*TmpArray3(  8,2))
     TwoTerms(  5) = inv2expQ*(AuxArray(  9,IP) + alphaQ*TmpArray3(  9,2))
     TwoTerms(  6) = inv2expQ*(AuxArray( 10,IP) + alphaQ*TmpArray3( 10,2))
     ...
     AuxArray(  24,IP) = Xqc*AuxArray(  14,IP) + alphaXpq*TmpArray4(  14,2) + TwoTerms(  4)
     AuxArray(  25,IP) = Yqc*AuxArray(  13,IP) + alphaYpq*TmpArray4(  13,2)
     AuxArray(  26,IP) = Xqc*AuxArray(  16,IP) + alphaXpq*TmpArray4(  16,2) + TwoTerms(  6)
     ...
     AuxArray(  31,IP) = Yqc*AuxArray(  17,IP) + alphaYpq*TmpArray4(  17,2) +  3*TwoTerms(  4)
     AuxArray(  32,IP) = Zqc*AuxArray(  17,IP) + alphaZpq*TmpArray4(  17,2)
     AuxArray(  33,IP) = Yqc*AuxArray(  19,IP) + alphaYpq*TmpArray4(  19,2) + TwoTerms(  6)
     AuxArray(  34,IP) = Yqc*AuxArray(  20,IP) + alphaYpq*TmpArray4(  20,2)
     AuxArray(  35,IP) = Zqc*AuxArray(  20,IP) + alphaZpq*TmpArray4(  20,2) +  3*TwoTerms(  6)
    ENDDO
   ENDDO
  ENDDO
end subroutine VerticalRecurrence4C

subroutine VerticalRecurrence5C(nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD,&
         & reducedExponents,TABFJW,Qexp,Ccenter,Pcent,Qcent,integralPrefactor,&
         & PpreExpFac,QpreExpFac,AUXarray)
  implicit none
  integer,intent(in) :: nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD
...
!      RJ000(5)=  9.0909090909090912E-02_realk
     ...
     TwoTerms(  2) = inv2expQ*(TmpArray3(  6,  2) + alphaQ*TmpArray3(  6,  3))
     TwoTerms(  3) = inv2expQ*(TmpArray3(  7,  2) + alphaQ*TmpArray3(  7,  3))
     TwoTerms(  4) = inv2expQ*(TmpArray3(  8,  2) + alphaQ*TmpArray3(  8,  3))
     TwoTerms(  5) = inv2expQ*(TmpArray3(  9,  2) + alphaQ*TmpArray3(  9,  3))
     TwoTerms(  6) = inv2expQ*(TmpArray3( 10,  2) + alphaQ*TmpArray3( 10,  3))
     ...
     tmpArray5(  24, 2) = Xqc*tmpArray4(  14, 2) + alphaXpq*TmpArray4(  14, 3) + TwoTerms(  4)
     tmpArray5(  25, 2) = Yqc*tmpArray4(  13, 2) + alphaYpq*TmpArray4(  13, 3)
     tmpArray5(  26, 2) = Xqc*tmpArray4(  16, 2) + alphaXpq*TmpArray4(  16, 3) + TwoTerms(  6)
     ...
     tmpArray5(  31, 2) = Yqc*tmpArray4(  17, 2) + alphaYpq*TmpArray4(  17, 3) +  3*TwoTerms(  4)
     tmpArray5(  32, 2) = Zqc*tmpArray4(  17, 2) + alphaZpq*TmpArray4(  17, 3)
     tmpArray5(  33, 2) = Yqc*tmpArray4(  19, 2) + alphaYpq*TmpArray4(  19, 3) + TwoTerms(  6)
     tmpArray5(  34, 2) = Yqc*tmpArray4(  20, 2) + alphaYpq*TmpArray4(  20, 3)
     tmpArray5(  35, 2) = Zqc*tmpArray4(  20, 2) + alphaZpq*TmpArray4(  20, 3) +  3*TwoTerms(  6)
     TwoTerms(  1) = inv2expQ*(AuxArray( 11,IP) + alphaQ*TmpArray4( 11,2))
     TwoTerms(  2) = inv2expQ*(AuxArray( 12,IP) + alphaQ*TmpArray4( 12,2))
     TwoTerms(  3) = inv2expQ*(AuxArray( 13,IP) + alphaQ*TmpArray4( 13,2))
     TwoTerms(  4) = inv2expQ*(AuxArray( 14,IP) + alphaQ*TmpArray4( 14,2))
     TwoTerms(  5) = inv2expQ*(AuxArray( 15,IP) + alphaQ*TmpArray4( 15,2))
     TwoTerms(  6) = inv2expQ*(AuxArray( 16,IP) + alphaQ*TmpArray4( 16,2))
     TwoTerms(  7) = inv2expQ*(AuxArray( 17,IP) + alphaQ*TmpArray4( 17,2))
     TwoTerms(  8) = inv2expQ*(AuxArray( 18,IP) + alphaQ*TmpArray4( 18,2))
     TwoTerms(  9) = inv2expQ*(AuxArray( 19,IP) + alphaQ*TmpArray4( 19,2))
     TwoTerms( 10) = inv2expQ*(AuxArray( 20,IP) + alphaQ*TmpArray4( 20,2))
     ...
     AuxArray(  39,IP) = Xqc*AuxArray(  24,IP) + alphaXpq*TmpArray5(  24,2) +  2*TwoTerms(  4)
     AuxArray(  40,IP) = Yqc*AuxArray(  23,IP) + alphaYpq*TmpArray5(  23,2)
     AuxArray(  41,IP) = Xqc*AuxArray(  26,IP) + alphaXpq*TmpArray5(  26,2) +  2*TwoTerms(  6)
     AuxArray(  42,IP) = Xqc*AuxArray(  27,IP) + alphaXpq*TmpArray5(  27,2) + TwoTerms(  7)
     AuxArray(  43,IP) = Zqc*AuxArray(  24,IP) + alphaZpq*TmpArray5(  24,2)
     AuxArray(  44,IP) = Yqc*AuxArray(  26,IP) + alphaYpq*TmpArray5(  26,2)
     AuxArray(  45,IP) = Xqc*AuxArray(  30,IP) + alphaXpq*TmpArray5(  30,2) + TwoTerms( 10)
     ...
     AuxArray(  51,IP) = Yqc*AuxArray(  31,IP) + alphaYpq*TmpArray5(  31,2) +  4*TwoTerms(  7)
     AuxArray(  52,IP) = Zqc*AuxArray(  31,IP) + alphaZpq*TmpArray5(  31,2)
     AuxArray(  53,IP) = Yqc*AuxArray(  33,IP) + alphaYpq*TmpArray5(  33,2) +  2*TwoTerms(  9)
     AuxArray(  54,IP) = Yqc*AuxArray(  34,IP) + alphaYpq*TmpArray5(  34,2) + TwoTerms( 10)
     AuxArray(  55,IP) = Yqc*AuxArray(  35,IP) + alphaYpq*TmpArray5(  35,2)
     AuxArray(  56,IP) = Zqc*AuxArray(  35,IP) + alphaZpq*TmpArray5(  35,2) +  4*TwoTerms( 10)
    ENDDO
   ENDDO
  ENDDO
end subroutine VerticalRecurrence5C

subroutine VerticalRecurrence6C(nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD,&
         & reducedExponents,TABFJW,Qexp,Ccenter,Pcent,Qcent,integralPrefactor,&
         & PpreExpFac,QpreExpFac,AUXarray)
  implicit none
  integer,intent(in) :: nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD
...
!      RJ000(6)=  7.6923076923076927E-02_realk
     ...
     TwoTerms(  2) = inv2expQ*(TmpArray3(  6,  3) + alphaQ*TmpArray3(  6,  4))
     TwoTerms(  3) = inv2expQ*(TmpArray3(  7,  3) + alphaQ*TmpArray3(  7,  4))
     TwoTerms(  4) = inv2expQ*(TmpArray3(  8,  3) + alphaQ*TmpArray3(  8,  4))
     TwoTerms(  5) = inv2expQ*(TmpArray3(  9,  3) + alphaQ*TmpArray3(  9,  4))
     TwoTerms(  6) = inv2expQ*(TmpArray3( 10,  3) + alphaQ*TmpArray3( 10,  4))
     ...
     tmpArray5(  24, 3) = Xqc*tmpArray4(  14, 3) + alphaXpq*TmpArray4(  14, 4) + TwoTerms(  4)
     tmpArray5(  25, 3) = Yqc*tmpArray4(  13, 3) + alphaYpq*TmpArray4(  13, 4)
     tmpArray5(  26, 3) = Xqc*tmpArray4(  16, 3) + alphaXpq*TmpArray4(  16, 4) + TwoTerms(  6)
     ...
     tmpArray5(  31, 3) = Yqc*tmpArray4(  17, 3) + alphaYpq*TmpArray4(  17, 4) +  3*TwoTerms(  4)
     tmpArray5(  32, 3) = Zqc*tmpArray4(  17, 3) + alphaZpq*TmpArray4(  17, 4)
     tmpArray5(  33, 3) = Yqc*tmpArray4(  19, 3) + alphaYpq*TmpArray4(  19, 4) + TwoTerms(  6)
     tmpArray5(  34, 3) = Yqc*tmpArray4(  20, 3) + alphaYpq*TmpArray4(  20, 4)
     tmpArray5(  35, 3) = Zqc*tmpArray4(  20, 3) + alphaZpq*TmpArray4(  20, 4) +  3*TwoTerms(  6)
     TwoTerms(  1) = inv2expQ*(AuxArray( 11,IP) + alphaQ*TmpArray4( 11,2))
     TwoTerms(  2) = inv2expQ*(AuxArray( 12,IP) + alphaQ*TmpArray4( 12,2))
     TwoTerms(  3) = inv2expQ*(AuxArray( 13,IP) + alphaQ*TmpArray4( 13,2))
     TwoTerms(  4) = inv2expQ*(AuxArray( 14,IP) + alphaQ*TmpArray4( 14,2))
     ...
     TwoTerms(  2) = inv2expQ*(TmpArray4( 12,  2) + alphaQ*TmpArray4( 12,  3))
     TwoTerms(  3) = inv2expQ*(TmpArray4( 13,  2) + alphaQ*TmpArray4( 13,  3))
     TwoTerms(  4) = inv2expQ*(TmpArray4( 14,  2) + alphaQ*TmpArray4( 14,  3))
     TwoTerms(  5) = inv2expQ*(TmpArray4( 15,  2) + alphaQ*TmpArray4( 15,  3))
     TwoTerms(  6) = inv2expQ*(TmpArray4( 16,  2) + alphaQ*TmpArray4( 16,  3))
     TwoTerms(  7) = inv2expQ*(TmpArray4( 17,  2) + alphaQ*TmpArray4( 17,  3))
     TwoTerms(  8) = inv2expQ*(TmpArray4( 18,  2) + alphaQ*TmpArray4( 18,  3))
     TwoTerms(  9) = inv2expQ*(TmpArray4( 19,  2) + alphaQ*TmpArray4( 19,  3))
     TwoTerms( 10) = inv2expQ*(TmpArray4( 20,  2) + alphaQ*TmpArray4( 20,  3))
     ...
     tmpArray6(  39, 2) = Xqc*tmpArray5(  24, 2) + alphaXpq*TmpArray5(  24, 3) +  2*TwoTerms(  4)
     tmpArray6(  40, 2) = Yqc*tmpArray5(  23, 2) + alphaYpq*TmpArray5(  23, 3)
     tmpArray6(  41, 2) = Xqc*tmpArray5(  26, 2) + alphaXpq*TmpArray5(  26, 3) +  2*TwoTerms(  6)
     tmpArray6(  42, 2) = Xqc*tmpArray5(  27, 2) + alphaXpq*TmpArray5(  27, 3) + TwoTerms(  7)
     tmpArray6(  43, 2) = Zqc*tmpArray5(  24, 2) + alphaZpq*TmpArray5(  24, 3)
     tmpArray6(  44, 2) = Yqc*tmpArray5(  26, 2) + alphaYpq*TmpArray5(  26, 3)
     tmpArray6(  45, 2) = Xqc*tmpArray5(  30, 2) + alphaXpq*TmpArray5(  30, 3) + TwoTerms( 10)
     ...
     tmpArray6(  51, 2) = Yqc*tmpArray5(  31, 2) + alphaYpq*TmpArray5(  31, 3) +  4*TwoTerms(  7)
     tmpArray6(  52, 2) = Zqc*tmpArray5(  31, 2) + alphaZpq*TmpArray5(  31, 3)
     tmpArray6(  53, 2) = Yqc*tmpArray5(  33, 2) + alphaYpq*TmpArray5(  33, 3) +  2*TwoTerms(  9)
     tmpArray6(  54, 2) = Yqc*tmpArray5(  34, 2) + alphaYpq*TmpArray5(  34, 3) + TwoTerms( 10)
     tmpArray6(  55, 2) = Yqc*tmpArray5(  35, 2) + alphaYpq*TmpArray5(  35, 3)
     tmpArray6(  56, 2) = Zqc*tmpArray5(  35, 2) + alphaZpq*TmpArray5(  35, 3) +  4*TwoTerms( 10)
     TwoTerms(  1) = inv2expQ*(AuxArray( 21,IP) + alphaQ*TmpArray5( 21,2))
     TwoTerms(  2) = inv2expQ*(AuxArray( 22,IP) + alphaQ*TmpArray5( 22,2))
     TwoTerms(  3) = inv2expQ*(AuxArray( 23,IP) + alphaQ*TmpArray5( 23,2))
     TwoTerms(  4) = inv2expQ*(AuxArray( 24,IP) + alphaQ*TmpArray5( 24,2))
     TwoTerms(  5) = inv2expQ*(AuxArray( 25,IP) + alphaQ*TmpArray5( 25,2))
     TwoTerms(  6) = inv2expQ*(AuxArray( 26,IP) + alphaQ*TmpArray5( 26,2))
     TwoTerms(  7) = inv2expQ*(AuxArray( 27,IP) + alphaQ*TmpArray5( 27,2))
     TwoTerms(  8) = inv2expQ*(AuxArray( 28,IP) + alphaQ*TmpArray5( 28,2))
     TwoTerms(  9) = inv2expQ*(AuxArray( 29,IP) + alphaQ*TmpArray5( 29,2))
     TwoTerms( 10) = inv2expQ*(AuxArray( 30,IP) + alphaQ*TmpArray5( 30,2))
     TwoTerms( 11) = inv2expQ*(AuxArray( 31,IP) + alphaQ*TmpArray5( 31,2))
     TwoTerms( 12) = inv2expQ*(AuxArray( 32,IP) + alphaQ*TmpArray5( 32,2))
     TwoTerms( 13) = inv2expQ*(AuxArray( 33,IP) + alphaQ*TmpArray5( 33,2))
     TwoTerms( 14) = inv2expQ*(AuxArray( 34,IP) + alphaQ*TmpArray5( 34,2))
     TwoTerms( 15) = inv2expQ*(AuxArray( 35,IP) + alphaQ*TmpArray5( 35,2))
     ...
     AuxArray(  60,IP) = Xqc*AuxArray(  39,IP) + alphaXpq*TmpArray6(  39,2) +  3*TwoTerms(  4)
     AuxArray(  61,IP) = Yqc*AuxArray(  38,IP) + alphaYpq*TmpArray6(  38,2)
     AuxArray(  62,IP) = Xqc*AuxArray(  41,IP) + alphaXpq*TmpArray6(  41,2) +  3*TwoTerms(  6)
     AuxArray(  63,IP) = Xqc*AuxArray(  42,IP) + alphaXpq*TmpArray6(  42,2) +  2*TwoTerms(  7)
     AuxArray(  64,IP) = Zqc*AuxArray(  39,IP) + alphaZpq*TmpArray6(  39,2)
     AuxArray(  65,IP) = Yqc*AuxArray(  41,IP) + alphaYpq*TmpArray6(  41,2)
     AuxArray(  66,IP) = Xqc*AuxArray(  45,IP) + alphaXpq*TmpArray6(  45,2) +  2*TwoTerms( 10)
     AuxArray(  67,IP) = Xqc*AuxArray(  46,IP) + alphaXpq*TmpArray6(  46,2) + TwoTerms( 11)
     AuxArray(  68,IP) = Zqc*AuxArray(  42,IP) + alphaZpq*TmpArray6(  42,2)
     AuxArray(  69,IP) = Xqc*AuxArray(  48,IP) + alphaXpq*TmpArray6(  48,2) + TwoTerms( 13)
     AuxArray(  70,IP) = Yqc*AuxArray(  45,IP) + alphaYpq*TmpArray6(  45,2)
     AuxArray(  71,IP) = Xqc*AuxArray(  50,IP) + alphaXpq*TmpArray6(  50,2) + TwoTerms( 15)
     ...
     AuxArray(  78,IP) = Yqc*AuxArray(  51,IP) + alphaYpq*TmpArray6(  51,2) +  5*TwoTerms( 11)
     AuxArray(  79,IP) = Zqc*AuxArray(  51,IP) + alphaZpq*TmpArray6(  51,2)
     AuxArray(  80,IP) = Yqc*AuxArray(  53,IP) + alphaYpq*TmpArray6(  53,2) +  3*TwoTerms( 13)
     AuxArray(  81,IP) = Yqc*AuxArray(  54,IP) + alphaYpq*TmpArray6(  54,2) +  2*TwoTerms( 14)
     AuxArray(  82,IP) = Yqc*AuxArray(  55,IP) + alphaYpq*TmpArray6(  55,2) + TwoTerms( 15)
     AuxArray(  83,IP) = Yqc*AuxArray(  56,IP) + alphaYpq*TmpArray6(  56,2)
     AuxArray(  84,IP) = Zqc*AuxArray(  56,IP) + alphaZpq*TmpArray6(  56,2) +  5*TwoTerms( 15)
    ENDDO
   ENDDO
  ENDDO
end subroutine VerticalRecurrence6C

subroutine VerticalRecurrence7C(nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD,&
         & reducedExponents,TABFJW,Qexp,Ccenter,Pcent,Qcent,integralPrefactor,&
         & PpreExpFac,QpreExpFac,AUXarray)
  implicit none
  integer,intent(in) :: nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD
...
!      RJ000(7)=  6.6666666666666666E-02_realk
     ...
     TwoTerms(  2) = inv2expQ*(TmpArray3(  6,  4) + alphaQ*TmpArray3(  6,  5))
     TwoTerms(  3) = inv2expQ*(TmpArray3(  7,  4) + alphaQ*TmpArray3(  7,  5))
     TwoTerms(  4) = inv2expQ*(TmpArray3(  8,  4) + alphaQ*TmpArray3(  8,  5))
     TwoTerms(  5) = inv2expQ*(TmpArray3(  9,  4) + alphaQ*TmpArray3(  9,  5))
     TwoTerms(  6) = inv2expQ*(TmpArray3( 10,  4) + alphaQ*TmpArray3( 10,  5))
     ...
     tmpArray5(  24, 4) = Xqc*tmpArray4(  14, 4) + alphaXpq*TmpArray4(  14, 5) + TwoTerms(  4)
     tmpArray5(  25, 4) = Yqc*tmpArray4(  13, 4) + alphaYpq*TmpArray4(  13, 5)
     tmpArray5(  26, 4) = Xqc*tmpArray4(  16, 4) + alphaXpq*TmpArray4(  16, 5) + TwoTerms(  6)
     ...
     tmpArray5(  31, 4) = Yqc*tmpArray4(  17, 4) + alphaYpq*TmpArray4(  17, 5) +  3*TwoTerms(  4)
     tmpArray5(  32, 4) = Zqc*tmpArray4(  17, 4) + alphaZpq*TmpArray4(  17, 5)
     tmpArray5(  33, 4) = Yqc*tmpArray4(  19, 4) + alphaYpq*TmpArray4(  19, 5) + TwoTerms(  6)
     tmpArray5(  34, 4) = Yqc*tmpArray4(  20, 4) + alphaYpq*TmpArray4(  20, 5)
     tmpArray5(  35, 4) = Zqc*tmpArray4(  20, 4) + alphaZpq*TmpArray4(  20, 5) +  3*TwoTerms(  6)
     TwoTerms(  1) = inv2expQ*(AuxArray( 11,IP) + alphaQ*TmpArray4( 11,2))
     TwoTerms(  2) = inv2expQ*(AuxArray( 12,IP) + alphaQ*TmpArray4( 12,2))
     TwoTerms(  3) = inv2expQ*(AuxArray( 13,IP) + alphaQ*TmpArray4( 13,2))
     TwoTerms(  4) = inv2expQ*(AuxArray( 14,IP) + alphaQ*TmpArray4( 14,2))
     ...
     TwoTerms(  2) = inv2expQ*(TmpArray4( 12,  3) + alphaQ*TmpArray4( 12,  4))
     TwoTerms(  3) = inv2expQ*(TmpArray4( 13,  3) + alphaQ*TmpArray4( 13,  4))
     TwoTerms(  4) = inv2expQ*(TmpArray4( 14,  3) + alphaQ*TmpArray4( 14,  4))
     TwoTerms(  5) = inv2expQ*(TmpArray4( 15,  3) + alphaQ*TmpArray4( 15,  4))
     TwoTerms(  6) = inv2expQ*(TmpArray4( 16,  3) + alphaQ*TmpArray4( 16,  4))
     TwoTerms(  7) = inv2expQ*(TmpArray4( 17,  3) + alphaQ*TmpArray4( 17,  4))
     TwoTerms(  8) = inv2expQ*(TmpArray4( 18,  3) + alphaQ*TmpArray4( 18,  4))
     TwoTerms(  9) = inv2expQ*(TmpArray4( 19,  3) + alphaQ*TmpArray4( 19,  4))
     TwoTerms( 10) = inv2expQ*(TmpArray4( 20,  3) + alphaQ*TmpArray4( 20,  4))
     ...
     tmpArray6(  39, 3) = Xqc*tmpArray5(  24, 3) + alphaXpq*TmpArray5(  24, 4) +  2*TwoTerms(  4)
     tmpArray6(  40, 3) = Yqc*tmpArray5(  23, 3) + alphaYpq*TmpArray5(  23, 4)
     tmpArray6(  41, 3) = Xqc*tmpArray5(  26, 3) + alphaXpq*TmpArray5(  26, 4) +  2*TwoTerms(  6)
     tmpArray6(  42, 3) = Xqc*tmpArray5(  27, 3) + alphaXpq*TmpArray5(  27, 4) + TwoTerms(  7)
     tmpArray6(  43, 3) = Zqc*tmpArray5(  24, 3) + alphaZpq*TmpArray5(  24, 4)
     tmpArray6(  44, 3) = Yqc*tmpArray5(  26, 3) + alphaYpq*TmpArray5(  26, 4)
     tmpArray6(  45, 3) = Xqc*tmpArray5(  30, 3) + alphaXpq*TmpArray5(  30, 4) + TwoTerms( 10)
     ...
     tmpArray6(  51, 3) = Yqc*tmpArray5(  31, 3) + alphaYpq*TmpArray5(  31, 4) +  4*TwoTerms(  7)
     tmpArray6(  52, 3) = Zqc*tmpArray5(  31, 3) + alphaZpq*TmpArray5(  31, 4)
     tmpArray6(  53, 3) = Yqc*tmpArray5(  33, 3) + alphaYpq*TmpArray5(  33, 4) +  2*TwoTerms(  9)
     tmpArray6(  54, 3) = Yqc*tmpArray5(  34, 3) + alphaYpq*TmpArray5(  34, 4) + TwoTerms( 10)
     tmpArray6(  55, 3) = Yqc*tmpArray5(  35, 3) + alphaYpq*TmpArray5(  35, 4)
     tmpArray6(  56, 3) = Zqc*tmpArray5(  35, 3) + alphaZpq*TmpArray5(  35, 4) +  4*TwoTerms( 10)
     TwoTerms(  1) = inv2expQ*(AuxArray( 21,IP) + alphaQ*TmpArray5( 21,2))
     TwoTerms(  2) = inv2expQ*(AuxArray( 22,IP) + alphaQ*TmpArray5( 22,2))
     TwoTerms(  3) = inv2expQ*(AuxArray( 23,IP) + alphaQ*TmpArray5( 23,2))
     TwoTerms(  4) = inv2expQ*(AuxArray( 24,IP) + alphaQ*TmpArray5( 24,2))
     ...
     TwoTerms(  2) = inv2expQ*(TmpArray5( 22,  2) + alphaQ*TmpArray5( 22,  3))
     TwoTerms(  3) = inv2expQ*(TmpArray5( 23,  2) + alphaQ*TmpArray5( 23,  3))
     TwoTerms(  4) = inv2expQ*(TmpArray5( 24,  2) + alphaQ*TmpArray5( 24,  3))
     TwoTerms(  5) = inv2expQ*(TmpArray5( 25,  2) + alphaQ*TmpArray5( 25,  3))
     TwoTerms(  6) = inv2expQ*(TmpArray5( 26,  2) + alphaQ*TmpArray5( 26,  3))
     TwoTerms(  7) = inv2expQ*(TmpArray5( 27,  2) + alphaQ*TmpArray5( 27,  3))
     TwoTerms(  8) = inv2expQ*(TmpArray5( 28,  2) + alphaQ*TmpArray5( 28,  3))
     TwoTerms(  9) = inv2expQ*(TmpArray5( 29,  2) + alphaQ*TmpArray5( 29,  3))
     TwoTerms( 10) = inv2expQ*(TmpArray5( 30,  2) + alphaQ*TmpArray5( 30,  3))
     TwoTerms( 11) = inv2expQ*(TmpArray5( 31,  2) + alphaQ*TmpArray5( 31,  3))
     TwoTerms( 12) = inv2expQ*(TmpArray5( 32,  2) + alphaQ*TmpArray5( 32,  3))
     TwoTerms( 13) = inv2expQ*(TmpArray5( 33,  2) + alphaQ*TmpArray5( 33,  3))
     TwoTerms( 14) = inv2expQ*(TmpArray5( 34,  2) + alphaQ*TmpArray5( 34,  3))
     TwoTerms( 15) = inv2expQ*(TmpArray5( 35,  2) + alphaQ*TmpArray5( 35,  3))
     ...
     tmpArray7(  60, 2) = Xqc*tmpArray6(  39, 2) + alphaXpq*TmpArray6(  39, 3) +  3*TwoTerms(  4)
     tmpArray7(  61, 2) = Yqc*tmpArray6(  38, 2) + alphaYpq*TmpArray6(  38, 3)
     tmpArray7(  62, 2) = Xqc*tmpArray6(  41, 2) + alphaXpq*TmpArray6(  41, 3) +  3*TwoTerms(  6)
     tmpArray7(  63, 2) = Xqc*tmpArray6(  42, 2) + alphaXpq*TmpArray6(  42, 3) +  2*TwoTerms(  7)
     tmpArray7(  64, 2) = Zqc*tmpArray6(  39, 2) + alphaZpq*TmpArray6(  39, 3)
     tmpArray7(  65, 2) = Yqc*tmpArray6(  41, 2) + alphaYpq*TmpArray6(  41, 3)
     tmpArray7(  66, 2) = Xqc*tmpArray6(  45, 2) + alphaXpq*TmpArray6(  45, 3) +  2*TwoTerms( 10)
     tmpArray7(  67, 2) = Xqc*tmpArray6(  46, 2) + alphaXpq*TmpArray6(  46, 3) + TwoTerms( 11)
     tmpArray7(  68, 2) = Zqc*tmpArray6(  42, 2) + alphaZpq*TmpArray6(  42, 3)
     tmpArray7(  69, 2) = Xqc*tmpArray6(  48, 2) + alphaXpq*TmpArray6(  48, 3) + TwoTerms( 13)
     tmpArray7(  70, 2) = Yqc*tmpArray6(  45, 2) + alphaYpq*TmpArray6(  45, 3)
     tmpArray7(  71, 2) = Xqc*tmpArray6(  50, 2) + alphaXpq*TmpArray6(  50, 3) + TwoTerms( 15)
     ...
     tmpArray7(  78, 2) = Yqc*tmpArray6(  51, 2) + alphaYpq*TmpArray6(  51, 3) +  5*TwoTerms( 11)
     tmpArray7(  79, 2) = Zqc*tmpArray6(  51, 2) + alphaZpq*TmpArray6(  51, 3)
     tmpArray7(  80, 2) = Yqc*tmpArray6(  53, 2) + alphaYpq*TmpArray6(  53, 3) +  3*TwoTerms( 13)
     tmpArray7(  81, 2) = Yqc*tmpArray6(  54, 2) + alphaYpq*TmpArray6(  54, 3) +  2*TwoTerms( 14)
     tmpArray7(  82, 2) = Yqc*tmpArray6(  55, 2) + alphaYpq*TmpArray6(  55, 3) + TwoTerms( 15)
     tmpArray7(  83, 2) = Yqc*tmpArray6(  56, 2) + alphaYpq*TmpArray6(  56, 3)
     tmpArray7(  84, 2) = Zqc*tmpArray6(  56, 2) + alphaZpq*TmpArray6(  56, 3) +  5*TwoTerms( 15)
     TwoTerms(  1) = inv2expQ*(AuxArray( 36,IP) + alphaQ*TmpArray6( 36,2))
     TwoTerms(  2) = inv2expQ*(AuxArray( 37,IP) + alphaQ*TmpArray6( 37,2))
     TwoTerms(  3) = inv2expQ*(AuxArray( 38,IP) + alphaQ*TmpArray6( 38,2))
     TwoTerms(  4) = inv2expQ*(AuxArray( 39,IP) + alphaQ*TmpArray6( 39,2))
     TwoTerms(  5) = inv2expQ*(AuxArray( 40,IP) + alphaQ*TmpArray6( 40,2))
     TwoTerms(  6) = inv2expQ*(AuxArray( 41,IP) + alphaQ*TmpArray6( 41,2))
     TwoTerms(  7) = inv2expQ*(AuxArray( 42,IP) + alphaQ*TmpArray6( 42,2))
     TwoTerms(  8) = inv2expQ*(AuxArray( 43,IP) + alphaQ*TmpArray6( 43,2))
     TwoTerms(  9) = inv2expQ*(AuxArray( 44,IP) + alphaQ*TmpArray6( 44,2))
     TwoTerms( 10) = inv2expQ*(AuxArray( 45,IP) + alphaQ*TmpArray6( 45,2))
     TwoTerms( 11) = inv2expQ*(AuxArray( 46,IP) + alphaQ*TmpArray6( 46,2))
     TwoTerms( 12) = inv2expQ*(AuxArray( 47,IP) + alphaQ*TmpArray6( 47,2))
     TwoTerms( 13) = inv2expQ*(AuxArray( 48,IP) + alphaQ*TmpArray6( 48,2))
     TwoTerms( 14) = inv2expQ*(AuxArray( 49,IP) + alphaQ*TmpArray6( 49,2))
     TwoTerms( 15) = inv2expQ*(AuxArray( 50,IP) + alphaQ*TmpArray6( 50,2))
     TwoTerms( 16) = inv2expQ*(AuxArray( 51,IP) + alphaQ*TmpArray6( 51,2))
     TwoTerms( 17) = inv2expQ*(AuxArray( 52,IP) + alphaQ*TmpArray6( 52,2))
     TwoTerms( 18) = inv2expQ*(AuxArray( 53,IP) + alphaQ*TmpArray6( 53,2))
     TwoTerms( 19) = inv2expQ*(AuxArray( 54,IP) + alphaQ*TmpArray6( 54,2))
     TwoTerms( 20) = inv2expQ*(AuxArray( 55,IP) + alphaQ*TmpArray6( 55,2))
     TwoTerms( 21) = inv2expQ*(AuxArray( 56,IP) + alphaQ*TmpArray6( 56,2))
     ...
     AuxArray(  88,IP) = Xqc*AuxArray(  60,IP) + alphaXpq*TmpArray7(  60,2) +  4*TwoTerms(  4)
     AuxArray(  89,IP) = Yqc*AuxArray(  59,IP) + alphaYpq*TmpArray7(  59,2)
     AuxArray(  90,IP) = Xqc*AuxArray(  62,IP) + alphaXpq*TmpArray7(  62,2) +  4*TwoTerms(  6)
     AuxArray(  91,IP) = Xqc*AuxArray(  63,IP) + alphaXpq*TmpArray7(  63,2) +  3*TwoTerms(  7)
     AuxArray(  92,IP) = Zqc*AuxArray(  60,IP) + alphaZpq*TmpArray7(  60,2)
     AuxArray(  93,IP) = Yqc*AuxArray(  62,IP) + alphaYpq*TmpArray7(  62,2)
     AuxArray(  94,IP) = Xqc*AuxArray(  66,IP) + alphaXpq*TmpArray7(  66,2) +  3*TwoTerms( 10)
     AuxArray(  95,IP) = Xqc*AuxArray(  67,IP) + alphaXpq*TmpArray7(  67,2) +  2*TwoTerms( 11)
     AuxArray(  96,IP) = Zqc*AuxArray(  63,IP) + alphaZpq*TmpArray7(  63,2)
     AuxArray(  97,IP) = Xqc*AuxArray(  69,IP) + alphaXpq*TmpArray7(  69,2) +  2*TwoTerms( 13)
     AuxArray(  98,IP) = Yqc*AuxArray(  66,IP) + alphaYpq*TmpArray7(  66,2)
     AuxArray(  99,IP) = Xqc*AuxArray(  71,IP) + alphaXpq*TmpArray7(  71,2) +  2*TwoTerms( 15)
     AuxArray( 100,IP) = Xqc*AuxArray(  72,IP) + alphaXpq*TmpArray7(  72,2) + TwoTerms( 16)
     AuxArray( 101,IP) = Zqc*AuxArray(  67,IP) + alphaZpq*TmpArray7(  67,2)
     AuxArray( 102,IP) = Xqc*AuxArray(  74,IP) + alphaXpq*TmpArray7(  74,2) + TwoTerms( 18)
     AuxArray( 103,IP) = Xqc*AuxArray(  75,IP) + alphaXpq*TmpArray7(  75,2) + TwoTerms( 19)
     AuxArray( 104,IP) = Yqc*AuxArray(  71,IP) + alphaYpq*TmpArray7(  71,2)
     AuxArray( 105,IP) = Xqc*AuxArray(  77,IP) + alphaXpq*TmpArray7(  77,2) + TwoTerms( 21)
     ...
     AuxArray( 113,IP) = Yqc*AuxArray(  78,IP) + alphaYpq*TmpArray7(  78,2) +  6*TwoTerms( 16)
     AuxArray( 114,IP) = Zqc*AuxArray(  78,IP) + alphaZpq*TmpArray7(  78,2)
     AuxArray( 115,IP) = Yqc*AuxArray(  80,IP) + alphaYpq*TmpArray7(  80,2) +  4*TwoTerms( 18)
     AuxArray( 116,IP) = Yqc*AuxArray(  81,IP) + alphaYpq*TmpArray7(  81,2) +  3*TwoTerms( 19)
     AuxArray( 117,IP) = Yqc*AuxArray(  82,IP) + alphaYpq*TmpArray7(  82,2) +  2*TwoTerms( 20)
     AuxArray( 118,IP) = Yqc*AuxArray(  83,IP) + alphaYpq*TmpArray7(  83,2) + TwoTerms( 21)
     AuxArray( 119,IP) = Yqc*AuxArray(  84,IP) + alphaYpq*TmpArray7(  84,2)
     AuxArray( 120,IP) = Zqc*AuxArray(  84,IP) + alphaZpq*TmpArray7(  84,2) +  6*TwoTerms( 21)
    ENDDO
   ENDDO
  ENDDO
end subroutine VerticalRecurrence7C

subroutine VerticalRecurrence8C(nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD,&
         & reducedExponents,TABFJW,Qexp,Ccenter,Pcent,Qcent,integralPrefactor,&
         & PpreExpFac,QpreExpFac,AUXarray)
  implicit none
  integer,intent(in) :: nPasses,nPrimP,nPrimQ,nAtomsC,nAtomsD
...
!      RJ000(8)=  5.8823529411764705E-02_realk
     ...
     TwoTerms(  2) = inv2expQ*(TmpArray3(  6,  5) + alphaQ*TmpArray3(  6,  6))
     TwoTerms(  3) = inv2expQ*(TmpArray3(  7,  5) + alphaQ*TmpArray3(  7,  6))
     TwoTerms(  4) = inv2expQ*(TmpArray3(  8,  5) + alphaQ*TmpArray3(  8,  6))
     TwoTerms(  5) = inv2expQ*(TmpArray3(  9,  5) + alphaQ*TmpArray3(  9,  6))
     TwoTerms(  6) = inv2expQ*(TmpArray3( 10,  5) + alphaQ*TmpArray3( 10,  6))
     ...
     tmpArray5(  24, 5) = Xqc*tmpArray4(  14, 5) + alphaXpq*TmpArray4(  14, 6) + TwoTerms(  4)
     tmpArray5(  25, 5) = Yqc*tmpArray4(  13, 5) + alphaYpq*TmpArray4(  13, 6)
     tmpArray5(  26, 5) = Xqc*tmpArray4(  16, 5) + alphaXpq*TmpArray4(  16, 6) + TwoTerms(  6)
     ...
     tmpArray5(  31, 5) = Yqc*tmpArray4(  17, 5) + alphaYpq*TmpArray4(  17, 6) +  3*TwoTerms(  4)
     tmpArray5(  32, 5) = Zqc*tmpArray4(  17, 5) + alphaZpq*TmpArray4(  17, 6)
     tmpArray5(  33, 5) = Yqc*tmpArray4(  19, 5) + alphaYpq*TmpArray4(  19, 6) + TwoTerms(  6)
     tmpArray5(  34, 5) = Yqc*tmpArray4(  20, 5) + alphaYpq*TmpArray4(  20, 6)
     tmpArray5(  35, 5) = Zqc*tmpArray4(  20, 5) + alphaZpq*TmpArray4(  20, 6) +  3*TwoTerms(  6)
     TwoTerms(  1) = inv2expQ*(AuxArray( 11,IP) + alphaQ*TmpArray4( 11,2))
     TwoTerms(  2) = inv2expQ*(AuxArray( 12,IP) + alphaQ*TmpArray4( 12,2))
     TwoTerms(  3) = inv2expQ*(AuxArray( 13,IP) + alphaQ*TmpArray4( 13,2))
     TwoTerms(  4) = inv2expQ*(AuxArray( 14,IP) + alphaQ*TmpArray4( 14,2))
     ...
     TwoTerms(  2) = inv2expQ*(TmpArray4( 12,  4) + alphaQ*TmpArray4( 12,  5))
     TwoTerms(  3) = inv2expQ*(TmpArray4( 13,  4) + alphaQ*TmpArray4( 13,  5))
     TwoTerms(  4) = inv2expQ*(TmpArray4( 14,  4) + alphaQ*TmpArray4( 14,  5))
     TwoTerms(  5) = inv2expQ*(TmpArray4( 15,  4) + alphaQ*TmpArray4( 15,  5))
     TwoTerms(  6) = inv2expQ*(TmpArray4( 16,  4) + alphaQ*TmpArray4( 16,  5))
     TwoTerms(  7) = inv2expQ*(TmpArray4( 17,  4) + alphaQ*TmpArray4( 17,  5))
     TwoTerms(  8) = inv2expQ*(TmpArray4( 18,  4) + alphaQ*TmpArray4( 18,  5))
     TwoTerms(  9) = inv2expQ*(TmpArray4( 19,  4) + alphaQ*TmpArray4( 19,  5))
     TwoTerms( 10) = inv2expQ*(TmpArray4( 20,  4) + alphaQ*TmpArray4( 20,  5))
     ...
     tmpArray6(  39, 4) = Xqc*tmpArray5(  24, 4) + alphaXpq*TmpArray5(  24, 5) +  2*TwoTerms(  4)
     tmpArray6(  40, 4) = Yqc*tmpArray5(  23, 4) + alphaYpq*TmpArray5(  23, 5)
     tmpArray6(  41, 4) = Xqc*tmpArray5(  26, 4) + alphaXpq*TmpArray5(  26, 5) +  2*TwoTerms(  6)
     tmpArray6(  42, 4) = Xqc*tmpArray5(  27, 4) + alphaXpq*TmpArray5(  27, 5) + TwoTerms(  7)
     tmpArray6(  43, 4) = Zqc*tmpArray5(  24, 4) + alphaZpq*TmpArray5(  24, 5)
     tmpArray6(  44, 4) = Yqc*tmpArray5(  26, 4) + alphaYpq*TmpArray5(  26, 5)
     tmpArray6(  45, 4) = Xqc*tmpArray5(  30, 4) + alphaXpq*TmpArray5(  30, 5) + TwoTerms( 10)
     ...
     tmpArray6(  51, 4) = Yqc*tmpArray5(  31, 4) + alphaYpq*TmpArray5(  31, 5) +  4*TwoTerms(  7)
     tmpArray6(  52, 4) = Zqc*tmpArray5(  31, 4) + alphaZpq*TmpArray5(  31, 5)
     tmpArray6(  53, 4) = Yqc*tmpArray5(  33, 4) + alphaYpq*TmpArray5(  33, 5) +  2*TwoTerms(  9)
     tmpArray6(  54, 4) = Yqc*tmpArray5(  34, 4) + alphaYpq*TmpArray5(  34, 5) + TwoTerms( 10)
     tmpArray6(  55, 4) = Yqc*tmpArray5(  35, 4) + alphaYpq*TmpArray5(  35, 5)
     tmpArray6(  56, 4) = Zqc*tmpArray5(  35, 4) + alphaZpq*TmpArray5(  35, 5) +  4*TwoTerms( 10)
     TwoTerms(  1) = inv2expQ*(AuxArray( 21,IP) + alphaQ*TmpArray5( 21,2))
     TwoTerms(  2) = inv2expQ*(AuxArray( 22,IP) + alphaQ*TmpArray5( 22,2))
     TwoTerms(  3) = inv2expQ*(AuxArray( 23,IP) + alphaQ*TmpArray5( 23,2))
     TwoTerms(  4) = inv2expQ*(AuxArray( 24,IP) + alphaQ*TmpArray5( 24,2))
     ...
     TwoTerms(  2) = inv2expQ*(TmpArray5( 22,  3) + alphaQ*TmpArray5( 22,  4))
     TwoTerms(  3) = inv2expQ*(TmpArray5( 23,  3) + alphaQ*TmpArray5( 23,  4))
     TwoTerms(  4) = inv2expQ*(TmpArray5( 24,  3) + alphaQ*TmpArray5( 24,  4))
     TwoTerms(  5) = inv2expQ*(TmpArray5( 25,  3) + alphaQ*TmpArray5( 25,  4))
     TwoTerms(  6) = inv2expQ*(TmpArray5( 26,  3) + alphaQ*TmpArray5( 26,  4))
     TwoTerms(  7) = inv2expQ*(TmpArray5( 27,  3) + alphaQ*TmpArray5( 27,  4))
     TwoTerms(  8) = inv2expQ*(TmpArray5( 28,  3) + alphaQ*TmpArray5( 28,  4))
     TwoTerms(  9) = inv2expQ*(TmpArray5( 29,  3) + alphaQ*TmpArray5( 29,  4))
     TwoTerms( 10) = inv2expQ*(TmpArray5( 30,  3) + alphaQ*TmpArray5( 30,  4))
     TwoTerms( 11) = inv2expQ*(TmpArray5( 31,  3) + alphaQ*TmpArray5( 31,  4))
     TwoTerms( 12) = inv2expQ*(TmpArray5( 32,  3) + alphaQ*TmpArray5( 32,  4))
     TwoTerms( 13) = inv2expQ*(TmpArray5( 33,  3) + alphaQ*TmpArray5( 33,  4))
     TwoTerms( 14) = inv2expQ*(TmpArray5( 34,  3) + alphaQ*TmpArray5( 34,  4))
     TwoTerms( 15) = inv2expQ*(TmpArray5( 35,  3) + alphaQ*TmpArray5( 35,  4))
     ...
     tmpArray7(  60, 3) = Xqc*tmpArray6(  39, 3) + alphaXpq*TmpArray6(  39, 4) +  3*TwoTerms(  4)
     tmpArray7(  61, 3) = Yqc*tmpArray6(  38, 3) + alphaYpq*TmpArray6(  38, 4)
     tmpArray7(  62, 3) = Xqc*tmpArray6(  41, 3) + alphaXpq*TmpArray6(  41, 4) +  3*TwoTerms(  6)
     tmpArray7(  63, 3) = Xqc*tmpArray6(  42, 3) + alphaXpq*TmpArray6(  42, 4) +  2*TwoTerms(  7)
     tmpArray7(  64, 3) = Zqc*tmpArray6(  39, 3) + alphaZpq*TmpArray6(  39, 4)
     tmpArray7(  65, 3) = Yqc*tmpArray6(  41, 3) + alphaYpq*TmpArray6(  41, 4)
     tmpArray7(  66, 3) = Xqc*tmpArray6(  45, 3) + alphaXpq*TmpArray6(  45, 4) +  2*TwoTerms( 10)
     tmpArray7(  67, 3) = Xqc*tmpArray6(  46, 3) + alphaXpq*TmpArray6(  46, 4) + TwoTerms( 11)
     tmpArray7(  68, 3) = Zqc*tmpArray6(  42, 3) + alphaZpq*TmpArray6(  42, 4)
     tmpArray7(  69, 3) = Xqc*tmpArray6(  48, 3) + alphaXpq*TmpArray6(  48, 4) + TwoTerms( 13)
     tmpArray7(  70, 3) = Yqc*tmpArray6(  45, 3) + alphaYpq*TmpArray6(  45, 4)
     tmpArray7(  71, 3) = Xqc*tmpArray6(  50, 3) + alphaXpq*TmpArray6(  50, 4) + TwoTerms( 15)
     ...
     tmpArray7(  78, 3) = Yqc*tmpArray6(  51, 3) + alphaYpq*TmpArray6(  51, 4) +  5*TwoTerms( 11)
     tmpArray7(  79, 3) = Zqc*tmpArray6(  51, 3) + alphaZpq*TmpArray6(  51, 4)
     tmpArray7(  80, 3) = Yqc*tmpArray6(  53, 3) + alphaYpq*TmpArray6(  53, 4) +  3*TwoTerms( 13)
     tmpArray7(  81, 3) = Yqc*tmpArray6(  54, 3) + alphaYpq*TmpArray6(  54, 4) +  2*TwoTerms( 14)
     tmpArray7(  82, 3) = Yqc*tmpArray6(  55, 3) + alphaYpq*TmpArray6(  55, 4) + TwoTerms( 15)
     tmpArray7(  83, 3) = Yqc*tmpArray6(  56, 3) + alphaYpq*TmpArray6(  56, 4)
     tmpArray7(  84, 3) = Zqc*tmpArray6(  56, 3) + alphaZpq*TmpArray6(  56, 4) +  5*TwoTerms( 15)
     TwoTerms(  1) = inv2expQ*(AuxArray( 36,IP) + alphaQ*TmpArray6( 36,2))
     TwoTerms(  2) = inv2expQ*(AuxArray( 37,IP) + alphaQ*TmpArray6( 37,2))
     TwoTerms(  3) = inv2expQ*(AuxArray( 38,IP) + alphaQ*TmpArray6( 38,2))
     TwoTerms(  4) = inv2expQ*(AuxArray( 39,IP) + alphaQ*TmpArray6( 39,2))
     ...
     TwoTerms(  2) = inv2expQ*(TmpArray6( 37,  2) + alphaQ*TmpArray6( 37,  3))
     TwoTerms(  3) = inv2expQ*(TmpArray6( 38,  2) + alphaQ*TmpArray6( 38,  3))
     TwoTerms(  4) = inv2expQ*(TmpArray6( 39,  2) + alphaQ*TmpArray6( 39,  3))
     TwoTerms(  5) = inv2expQ*(TmpArray6( 40,  2) + alphaQ*TmpArray6( 40,  3))
     TwoTerms(  6) = inv2expQ*(TmpArray6( 41,  2) + alphaQ*TmpArray6( 41,  3))
     TwoTerms(  7) = inv2expQ*(TmpArray6( 42,  2) + alphaQ*TmpArray6( 42,  3))
     TwoTerms(  8) = inv2expQ*(TmpArray6( 43,  2) + alphaQ*TmpArray6( 43,  3))
     TwoTerms(  9) = inv2expQ*(TmpArray6( 44,  2) + alphaQ*TmpArray6( 44,  3))
     TwoTerms( 10) = inv2expQ*(TmpArray6( 45,  2) + alphaQ*TmpArray6( 45,  3))
     TwoTerms( 11) = inv2expQ*(TmpArray6( 46,  2) + alphaQ*TmpArray6( 46,  3))
     TwoTerms( 12) = inv2expQ*(TmpArray6( 47,  2) + alphaQ*TmpArray6( 47,  3))
     TwoTerms( 13) = inv2expQ*(TmpArray6( 48,  2) + alphaQ*TmpArray6( 48,  3))
     TwoTerms( 14) = inv2expQ*(TmpArray6( 49,  2) + alphaQ*TmpArray6( 49,  3))
     TwoTerms( 15) = inv2expQ*(TmpArray6( 50,  2) + alphaQ*TmpArray6( 50,  3))
     TwoTerms( 16) = inv2expQ*(TmpArray6( 51,  2) + alphaQ*TmpArray6( 51,  3))
     TwoTerms( 17) = inv2expQ*(TmpArray6( 52,  2) + alphaQ*TmpArray6( 52,  3))
     TwoTerms( 18) = inv2expQ*(TmpArray6( 53,  2) + alphaQ*TmpArray6( 53,  3))
     TwoTerms( 19) = inv2expQ*(TmpArray6( 54,  2) + alphaQ*TmpArray6( 54,  3))
     TwoTerms( 20) = inv2expQ*(TmpArray6( 55,  2) + alphaQ*TmpArray6( 55,  3))
     TwoTerms( 21) = inv2expQ*(TmpArray6( 56,  2) + alphaQ*TmpArray6( 56,  3))
     ...
     tmpArray8(  88, 2) = Xqc*tmpArray7(  60, 2) + alphaXpq*TmpArray7(  60, 3) +  4*TwoTerms(  4)
     tmpArray8(  89, 2) = Yqc*tmpArray7(  59, 2) + alphaYpq*TmpArray7(  59, 3)
     tmpArray8(  90, 2) = Xqc*tmpArray7(  62, 2) + alphaXpq*TmpArray7(  62, 3) +  4*TwoTerms(  6)
     tmpArray8(  91, 2) = Xqc*tmpArray7(  63, 2) + alphaXpq*TmpArray7(  63, 3) +  3*TwoTerms(  7)
     tmpArray8(  92, 2) = Zqc*tmpArray7(  60, 2) + alphaZpq*TmpArray7(  60, 3)
     tmpArray8(  93, 2) = Yqc*tmpArray7(  62, 2) + alphaYpq*TmpArray7(  62, 3)
     tmpArray8(  94, 2) = Xqc*tmpArray7(  66, 2) + alphaXpq*TmpArray7(  66, 3) +  3*TwoTerms( 10)
     tmpArray8(  95, 2) = Xqc*tmpArray7(  67, 2) + alphaXpq*TmpArray7(  67, 3) +  2*TwoTerms( 11)
     tmpArray8(  96, 2) = Zqc*tmpArray7(  63, 2) + alphaZpq*TmpArray7(  63, 3)
     tmpArray8(  97, 2) = Xqc*tmpArray7(  69, 2) + alphaXpq*TmpArray7(  69, 3) +  2*TwoTerms( 13)
     tmpArray8(  98, 2) = Yqc*tmpArray7(  66, 2) + alphaYpq*TmpArray7(  66, 3)
     tmpArray8(  99, 2) = Xqc*tmpArray7(  71, 2) + alphaXpq*TmpArray7(  71, 3) +  2*TwoTerms( 15)
     tmpArray8( 100, 2) = Xqc*tmpArray7(  72, 2) + alphaXpq*TmpArray7(  72, 3) + TwoTerms( 16)
     tmpArray8( 101, 2) = Zqc*tmpArray7(  67, 2) + alphaZpq*TmpArray7(  67, 3)
     tmpArray8( 102, 2) = Xqc*tmpArray7(  74, 2) + alphaXpq*TmpArray7(  74, 3) + TwoTerms( 18)
     tmpArray8( 103, 2) = Xqc*tmpArray7(  75, 2) + alphaXpq*TmpArray7(  75, 3) + TwoTerms( 19)
     tmpArray8( 104, 2) = Yqc*tmpArray7(  71, 2) + alphaYpq*TmpArray7(  71, 3)
     tmpArray8( 105, 2) = Xqc*tmpArray7(  77, 2) + alphaXpq*TmpArray7(  77, 3) + TwoTerms( 21)
     ...
     tmpArray8( 113, 2) = Yqc*tmpArray7(  78, 2) + alphaYpq*TmpArray7(  78, 3) +  6*TwoTerms( 16)
     tmpArray8( 114, 2) = Zqc*tmpArray7(  78, 2) + alphaZpq*TmpArray7(  78, 3)
     tmpArray8( 115, 2) = Yqc*tmpArray7(  80, 2) + alphaYpq*TmpArray7(  80, 3) +  4*TwoTerms( 18)
     tmpArray8( 116, 2) = Yqc*tmpArray7(  81, 2) + alphaYpq*TmpArray7(  81, 3) +  3*TwoTerms( 19)
     tmpArray8( 117, 2) = Yqc*tmpArray7(  82, 2) + alphaYpq*TmpArray7(  82, 3) +  2*TwoTerms( 20)
     tmpArray8( 118, 2) = Yqc*tmpArray7(  83, 2) + alphaYpq*TmpArray7(  83, 3) + TwoTerms( 21)
     tmpArray8( 119, 2) = Yqc*tmpArray7(  84, 2) + alphaYpq*TmpArray7(  84, 3)
     tmpArray8( 120, 2) = Zqc*tmpArray7(  84, 2) + alphaZpq*TmpArray7(  84, 3) +  6*TwoTerms( 21)
     TwoTerms(  1) = inv2expQ*(AuxArray( 57,IP) + alphaQ*TmpArray7( 57,2))
     TwoTerms(  2) = inv2expQ*(AuxArray( 58,IP) + alphaQ*TmpArray7( 58,2))
     TwoTerms(  3) = inv2expQ*(AuxArray( 59,IP) + alphaQ*TmpArray7( 59,2))
     TwoTerms(  4) = inv2expQ*(AuxArray( 60,IP) + alphaQ*TmpArray7( 60,2))
     TwoTerms(  5) = inv2expQ*(AuxArray( 61,IP) + alphaQ*TmpArray7( 61,2))
     TwoTerms(  6) = inv2expQ*(AuxArray( 62,IP) + alphaQ*TmpArray7( 62,2))
     TwoTerms(  7) = inv2expQ*(AuxArray( 63,IP) + alphaQ*TmpArray7( 63,2))
     TwoTerms(  8) = inv2expQ*(AuxArray( 64,IP) + alphaQ*TmpArray7( 64,2))
     TwoTerms(  9) = inv2expQ*(AuxArray( 65,IP) + alphaQ*TmpArray7( 65,2))
     TwoTerms( 10) = inv2expQ*(AuxArray( 66,IP) + alphaQ*TmpArray7( 66,2))
     TwoTerms( 11) = inv2expQ*(AuxArray( 67,IP) + alphaQ*TmpArray7( 67,2))
     TwoTerms( 12) = inv2expQ*(AuxArray( 68,IP) + alphaQ*TmpArray7( 68,2))
     TwoTerms( 13) = inv2expQ*(AuxArray( 69,IP) + alphaQ*TmpArray7( 69,2))
     TwoTerms( 14) = inv2expQ*(AuxArray( 70,IP) + alphaQ*TmpArray7( 70,2))
     TwoTerms( 15) = inv2expQ*(AuxArray( 71,IP) + alphaQ*TmpArray7( 71,2))
     TwoTerms( 16) = inv2expQ*(AuxArray( 72,IP) + alphaQ*TmpArray7( 72,2))
     TwoTerms( 17) = inv2expQ*(AuxArray( 73,IP) + alphaQ*TmpArray7( 73,2))
     TwoTerms( 18) = inv2expQ*(AuxArray( 74,IP) + alphaQ*TmpArray7( 74,2))
     TwoTerms( 19) = inv2expQ*(AuxArray( 75,IP) + alphaQ*TmpArray7( 75,2))
     TwoTerms( 20) = inv2expQ*(AuxArray( 76,IP) + alphaQ*TmpArray7( 76,2))
     TwoTerms( 21) = inv2expQ*(AuxArray( 77,IP) + alphaQ*TmpArray7( 77,2))
     TwoTerms( 22) = inv2expQ*(AuxArray( 78,IP) + alphaQ*TmpArray7( 78,2))
     TwoTerms( 23) = inv2expQ*(AuxArray( 79,IP) + alphaQ*TmpArray7( 79,2))
     TwoTerms( 24) = inv2expQ*(AuxArray( 80,IP) + alphaQ*TmpArray7( 80,2))
     TwoTerms( 25) = inv2expQ*(AuxArray( 81,IP) + alphaQ*TmpArray7( 81,2))
     TwoTerms( 26) = inv2expQ*(AuxArray( 82,IP) + alphaQ*TmpArray7( 82,2))
     TwoTerms( 27) = inv2expQ*(AuxArray( 83,IP) + alphaQ*TmpArray7( 83,2))
     TwoTerms( 28) = inv2expQ*(AuxArray( 84,IP) + alphaQ*TmpArray7( 84,2))
     ...
     AuxArray( 124,IP) = Xqc*AuxArray(  88,IP) + alphaXpq*TmpArray8(  88,2) +  5*TwoTerms(  4)
     AuxArray( 125,IP) = Yqc*AuxArray(  87,IP) + alphaYpq*TmpArray8(  87,2)
     AuxArray( 126,IP) = Xqc*AuxArray(  90,IP) + alphaXpq*TmpArray8(  90,2) +  5*TwoTerms(  6)
     AuxArray( 127,IP) = Xqc*AuxArray(  91,IP) + alphaXpq*TmpArray8(  91,2) +  4*TwoTerms(  7)
     AuxArray( 128,IP) = Zqc*AuxArray(  88,IP) + alphaZpq*TmpArray8(  88,2)
     AuxArray( 129,IP) = Yqc*AuxArray(  90,IP) + alphaYpq*TmpArray8(  90,2)
     AuxArray( 130,IP) = Xqc*AuxArray(  94,IP) + alphaXpq*TmpArray8(  94,2) +  4*TwoTerms( 10)
     AuxArray( 131,IP) = Xqc*AuxArray(  95,IP) + alphaXpq*TmpArray8(  95,2) +  3*TwoTerms( 11)
     AuxArray( 132,IP) = Zqc*AuxArray(  91,IP) + alphaZpq*TmpArray8(  91,2)
     AuxArray( 133,IP) = Xqc*AuxArray(  97,IP) + alphaXpq*TmpArray8(  97,2) +  3*TwoTerms( 13)
     AuxArray( 134,IP) = Yqc*AuxArray(  94,IP) + alphaYpq*TmpArray8(  94,2)
     AuxArray( 135,IP) = Xqc*AuxArray(  99,IP) + alphaXpq*TmpArray8(  99,2) +  3*TwoTerms( 15)
     AuxArray( 136,IP) = Xqc*AuxArray( 100,IP) + alphaXpq*TmpArray8( 100,2) +  2*TwoTerms( 16)
     AuxArray( 137,IP) = Zqc*AuxArray(  95,IP) + alphaZpq*TmpArray8(  95,2)
     AuxArray( 138,IP) = Xqc*AuxArray( 102,IP) + alphaXpq*TmpArray8( 102,2) +  2*TwoTerms( 18)
     AuxArray( 139,IP) = Xqc*AuxArray( 103,IP) + alphaXpq*TmpArray8( 103,2) +  2*TwoTerms( 19)
     AuxArray( 140,IP) = Yqc*AuxArray(  99,IP) + alphaYpq*TmpArray8(  99,2)
     AuxArray( 141,IP) = Xqc*AuxArray( 105,IP) + alphaXpq*TmpArray8( 105,2) +  2*TwoTerms( 21)
     AuxArray( 142,IP) = Xqc*AuxArray( 106,IP) + alphaXpq*TmpArray8( 106,2) + TwoTerms( 22)
     AuxArray( 143,IP) = Zqc*AuxArray( 100,IP) + alphaZpq*TmpArray8( 100,2)
     AuxArray( 144,IP) = Xqc*AuxArray( 108,IP) + alphaXpq*TmpArray8( 108,2) + TwoTerms( 24)
     AuxArray( 145,IP) = Xqc*AuxArray( 109,IP) + alphaXpq*TmpArray8( 109,2) + TwoTerms( 25)
     AuxArray( 146,IP) = Xqc*AuxArray( 110,IP) + alphaXpq*TmpArray8( 110,2) + TwoTerms( 26)
     AuxArray( 147,IP) = Yqc*AuxArray( 105,IP) + alphaYpq*TmpArray8( 105,2)
     AuxArray( 148,IP) = Xqc*AuxArray( 112,IP) + alphaXpq*TmpArray8( 112,2) + TwoTerms( 28)
     ...
     AuxArray( 157,IP) = Yqc*AuxArray( 113,IP) + alphaYpq*TmpArray8( 113,2) +  7*TwoTerms( 22)
     AuxArray( 158,IP) = Zqc*AuxArray( 113,IP) + alphaZpq*TmpArray8( 113,2)
     AuxArray( 159,IP) = Yqc*AuxArray( 115,IP) + alphaYpq*TmpArray8( 115,2) +  5*TwoTerms( 24)
     AuxArray( 160,IP) = Yqc*AuxArray( 116,IP) + alphaYpq*TmpArray8( 116,2) +  4*TwoTerms( 25)
     AuxArray( 161,IP) = Yqc*AuxArray( 117,IP) + alphaYpq*TmpArray8( 117,2) +  3*TwoTerms( 26)
     AuxArray( 162,IP) = Yqc*AuxArray( 118,IP) + alphaYpq*TmpArray8( 118,2) +  2*TwoTerms( 27)
     AuxArray( 163,IP) = Yqc*AuxArray( 119,IP) + alphaYpq*TmpArray8( 119,2) + TwoTerms( 28)
     AuxArray( 164,IP) = Yqc*AuxArray( 120,IP) + alphaYpq*TmpArray8( 120,2)
     AuxArray( 165,IP) = Zqc*AuxArray( 120,IP) + alphaZpq*TmpArray8( 120,2) +  7*TwoTerms( 28)
    ENDDO
   ENDDO
  ENDDO
end subroutine VerticalRecurrence8C
end module
