module SPAGC_GPU_PrimContractSegQMod
...
CONTAINS
   ...
   subroutine SPPrimitiveContractionAGPUSegQ(AUXarray2,AUXarrayCont,nPrimP,nPrimQ,nPasses,&
    ...
    real(reals),intent(in) :: AUXarray2(nPrimA,nPrimB*nPasses*nTUVP*nTUVQ)
    real(reals),intent(inout) :: AUXarrayCont(nContA,nPrimB*nPasses*nTUVP*nTUVQ)
...
!$ACC         ACC,AUXarrayCont,AUXarray2) ASYNC(iASync)
    do iP=1,nPasses*nTUVP*nTUVQ*nPrimB
      ...
      enddo
    enddo
   end subroutine SPPrimitiveContractionAGPUSegQ


   subroutine SPPrimitiveContractionBGPUSegQ(AUXarray2,AUXarrayCont,nPrimP,nPrimQ,nPasses,&
    ...
    real(reals),intent(in) :: AUXarray2(nContA,nPrimB,nPasses*nTUVP*nTUVQ)
    real(reals),intent(inout) :: AUXarrayCont(nContA,nContB,nPasses*nTUVP*nTUVQ)
    integer(kind=acckind),intent(in) :: iASync
    !
...
!$ACC         BCC,AUXarrayCont,AUXarray2) ASYNC(iASync)
    do iP=1,nPasses*nTUVP*nTUVQ
     do iContB=1,nContB
      do iContA=1,nContA
       AUXarrayCont(iContA,iContB,iP) = 0.0E0_reals
      enddo
      do iPrimB=1,nPrimB
     ...
     enddo
    enddo
   end subroutine SPPrimitiveContractionBGPUSegQ

END module SPAGC_GPU_PrimContractSegQMod
