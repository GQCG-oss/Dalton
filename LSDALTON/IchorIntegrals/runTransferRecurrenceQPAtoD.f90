  ...
  subroutine PASSsub
    ...
    DO JMAX=2,MaxAngmomQP
       DO JP = 1, JMAX
          ...
          !The Equation A to D looks like
          !A to D
          ...
          WRITE(*,'(A)')'  real(realk),intent(in) :: Bexp(nPrimB),Cexp(nPrimD)'
          ...
          WRITE(*,'(A)')'  integer :: iPassQ,iPrimP,iPrimQ,IP,iTUVP,iPrimB,iPrimC'
...
!          WRITE(*,'(A)')'     iPrimD = (iPrimQ-1)/nPrimC+1'
          ...
          WRITE(*,'(A)')'     facX = -(expBX-Cexp(iPrimC)*Xcd)*invexpQ'
          WRITE(*,'(A)')'     facY = -(expBY-Cexp(iPrimC)*Ycd)*invexpQ'
          WRITE(*,'(A)')'     facZ = -(expBZ-Cexp(iPrimC)*Zcd)*invexpQ'
