...
!$OMP         iPrimP,iPrimQ,&
!$OMP         Bx,By,Bz,Xpb,Ypb,Zpb,alphaP,invexpP,&
  ...
  DO iPassP = 1,nPasses
   ...
   DO iPrimP=1, nPrimP
    ...
    DO iPrimQ=1, nPrimQ
     ...
     AUXarray(2,iPrimQ,iPrimP,iPassP) = Xpb*TMP1 + alphaXpq*TMP2
     AUXarray(3,iPrimQ,iPrimP,iPassP) = Ypb*TMP1 + alphaYpq*TMP2
     AUXarray(4,iPrimQ,iPrimP,iPassP) = Zpb*TMP1 + alphaZpq*TMP2
    enddo
   enddo
  enddo
!$OMP END DO
...
subroutine VerticalRecurrenceCPU2B(nPasses,nPrimP,nPrimQ,reducedExponents,&
         & RJ000Array,Pexp,Bcenter,Pcent,Qcent,integralPrefactor,PpreExpFac,QpreExpFac,&
         & IatomApass,IatomBpass,MaxPasses,nAtomsA,nAtomsB,&
         & AUXarray)
  implicit none
  integer,intent(in) :: nPasses,nPrimP,nPrimQ
...
!$OMP PRIVATE(iPassP,iAtomA,iAtomB,Xpq,Ypq,Zpq,&
!$OMP         iPrimP,iPrimQ,&
!$OMP         Bx,By,Bz,Xpb,Ypb,Zpb,alphaP,invexpP,inv2expP,&
