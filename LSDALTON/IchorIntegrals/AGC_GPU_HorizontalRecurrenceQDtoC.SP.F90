module SPAGC_GPU_OBS_HorizontalRecurrenceRHSModDtoC
 use IchorPrecisionMod
  
 CONTAINS

!Transfer angmom from D to C
subroutine SPHorizontalRR_GPU_RHS_Q1C0D1DtoC(nContPQ,nPasses,nlmP,&
  ...
  real(reals),intent(inout) :: ThetaP(nContPQ*nPasses,nlmP,1,    2:    4)
...
!$ACC PRESENT(nPasses,ThetaP,ThetaP2) ASYNC(iASync)
  DO iP = 1,nContPQ*nPasses
    ...
    ENDDO
   ENDDO
end subroutine SPHorizontalRR_GPU_RHS_Q1C0D1DtoC

!Transfer angmom from D to C
subroutine SPHorizontalRR_GPU_RHS_Q2C0D2DtoC(nContPQ,nPasses,nlmP,&
         & Qdistance12,ThetaP2,ThetaP,lupri,iASync)
  implicit none
  integer,intent(in) :: nContPQ,nPasses,nlmP,lupri
  real(reals),intent(in) :: Qdistance12(3)
  real(reals),intent(in) :: ThetaP2(nContPQ*nPasses,nlmP,   10)
  real(reals),intent(inout) :: ThetaP(nContPQ*nPasses,nlmP,1,    5:   10)
  integer(kind=acckind),intent(in) :: iASync
  !Local variables
  integer :: iP,ilmP,iTUVD
!$ACC PARALLEL LOOP &
...
!$ACC PRESENT(nPasses,ThetaP,ThetaP2) ASYNC(iASync)
  DO iP = 1,nContPQ*nPasses
    ...
    ENDDO
   ENDDO
end subroutine SPHorizontalRR_GPU_RHS_Q2C0D2DtoC

!Transfer angmom from D to C
subroutine SPHorizontalRR_GPU_RHS_Q3C1D2DtoC(nContPQ,nPasses,nlmP,&
         & Qdistance12,ThetaP2,ThetaP,lupri,iASync)
  implicit none
  integer,intent(in) :: nContPQ,nPasses,nlmP,lupri
  real(reals),intent(in) :: Qdistance12(3)
  real(reals),intent(in) :: ThetaP2(nContPQ*nPasses,nlmP,   20)
  real(reals),intent(inout) :: ThetaP(nContPQ*nPasses,nlmP,    2:    4,    5:   10)
  integer(kind=acckind),intent(in) :: iASync
  !Local variables
...
!$ACC PRESENT(nPasses,Qdistance12,ThetaP,ThetaP2) ASYNC(iASync)
  DO iP = 1,nContPQ*nPasses
   ...
   Zcd = -Qdistance12(3)
    ...
    ENDDO
   ENDDO
end subroutine SPHorizontalRR_GPU_RHS_Q3C1D2DtoC
end module
