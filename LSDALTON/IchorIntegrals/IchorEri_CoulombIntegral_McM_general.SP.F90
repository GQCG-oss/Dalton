...
!> \author T. Kjaergaard
!> \date 2013 
module SPIchorEriCoulombintegralCPUMcMGeneralMod
...
type vector
real(reals),pointer :: elms(:)
end type vector
type(vector),allocatable :: SPH_MAT(:)
integer :: nTmpArray3,nTmpArray4
real(reals),allocatable :: TmpArray3(:),TmpArray4(:)

CONTAINS
  subroutine SPDetermineSizeTmpArray34(nTUVQ,nCartOrbCompQ,nPrimQ,nTUVP,nCartOrbCompP,nPrimP,nPasses,&
  ...
  end subroutine SPDetermineSizeTmpArray34

  subroutine SPICI_CPU_McM_general(nPrimA,nPrimB,nPrimC,nPrimD,&
    ...
    real(reals),intent(in) :: pcent(3*nPrimP*nAtomsA*nAtomsB) !pcent(3,nPrimP,nPasses)
    ...
    real(reals),intent(inout) :: CDAB(nLocalIntPass)    
    real(reals),intent(in) :: integralPrefactor(nPrimQP)!integralPrefactor(nPrimQ,nPrimP)    
    real(reals),intent(in) :: reducedExponents(nPrimQP) !reducedExponents(nPrimQ,nPrimP)
    real(reals),intent(in) :: Qdistance12(3)
    real(reals),intent(in) :: Pdistance12(3*nAtomsA*nAtomsB) !full set nAtomsA*nAtomsB
    !local variables
!    real(reals),allocatable :: squaredDistance(:),Rpq(:)
...
!    integer :: nCartOrbCompP,nOrbCompP,nCartOrbCompQ,nOrbCompQ
    ...
    call SPbuild_Rpq(nPrimQ,nPasses,nPrimP,Qcent,Pcent,TmpArray3,IatomApass,IatomBpass,MaxPasses,nAtomsA,nAtomsB)
    ...
    IF(.NOT.GGemOperatorCalc)THEN
       call SPbuildRJ000_general(nPasses,nPrimQ,nPrimP,nTABFJW1,nTABFJW2,reducedExponents,&
    ...
    ELSE
       IF(GGemOperatorSpec.EQ.GGemOperator.OR.GGemOperatorSpec.EQ.GGemSqOperator)THEN
          call SPbuildGJ000_general(nPasses,nPrimQ,nPrimP,reducedExponents,&
               & TmpArray4,AngmomPQ,integralPrefactor,TmpArray3)
       ELSEIF(GGemOperatorSpec.EQ.GGemGrdOperator)THEN
          call SPbuildGJ000Grad_general(nPasses,nPrimQ,nPrimP,reducedExponents,&
               & TmpArray4,AngmomPQ,integralPrefactor,TmpArray3)
       ELSEIF(GGemOperatorSpec.EQ.GGemCouOperator)THEN
          call SPbuildGJ000Coulomb_general(nPasses,nPrimQ,nPrimP,nTABFJW1,nTABFJW2,reducedExponents,&
               & TABFJW,TmpArray4,AngmomPQ,integralPrefactor,TmpArray3)
       ENDIF
    ENDIF
    IF (INTPRINT .GE. 10) call SPPrintRJ000(TmpArray4,AngmomPQ,nPrimQ*nPrimP,nPasses,lupri)    
    !
...
#endif
       call SPIchorwtuvRecurrenceJMIN0JMAX0(TmpArray4,TmpArray1,nPrimQP*nPasses)
    ELSEIF (AngmomPQ.EQ. 1) THEN
#ifdef VAR_DEBUGICHOR
       IF(TMParray1maxsize.LT.nPrimQP*nPasses*ntuv)call ichorquit('IchorTmp1G3B',-1)
#endif
       call SPIchorwtuvRecurrenceJMIN0JMAX1(TmpArray4,TmpArray1,TmpArray3,nPrimQP*nPasses)
    ELSEIF (AngmomPQ.EQ. 2) THEN
#ifdef VAR_DEBUGICHOR
       IF(TMParray1maxsize.LT.nPrimQP*nPasses*ntuv)call ichorquit('IchorTmp1G3C',-1)
#endif
       call SPIchorwtuvRecurrenceJMIN0JMAX2(TmpArray4,TmpArray1,TmpArray3,nPrimQP*nPasses)
    ELSEIF (AngmomPQ.EQ. 3) THEN
#ifdef VAR_DEBUGICHOR
       IF(TMParray1maxsize.LT.nPrimQP*nPasses*ntuv)call ichorquit('IchorTmp1G3D',-1)
#endif
       call SPIchorwtuvRecurrenceJMIN0JMAX3(TmpArray4,TmpArray1,TmpArray3,nPrimQP*nPasses)
    ELSE !AngmomPQ > 3
#ifdef VAR_DEBUGICHOR
       IF(TMParray1maxsize.LT.nPrimQP*nPasses*ntuv)call ichorquit('IchorTmp1G3a',-1)
...
#endif
       J = AngmomPQ-3
       call SPIchorwtuvRecurrenceJMIN0JMAX3J(TmpArray4,TmpArray1,TmpArray3,nPrimQP*nPasses,AngmomPQ,J)
       TMP1 = .TRUE.
       !J minimum goes from 0 to 0. For  AngmomPQ=6 J takes the values 2,1,0 
       DO j=AngmomPQ-4,0,-1 
          IF(TMP1)THEN
             call SPIchorwtuvRecurrenceCurrent(TmpArray1,TmpArray2,J,AngmomPQ,nPrimQ,nPrimP,nPasses,&
                  & ntuv,TmpArray3,TmpArray4)
          ELSE
             call SPIchorwtuvRecurrenceCurrent(TmpArray2,TmpArray1,J,AngmomPQ,nPrimQ,nPrimP,nPasses,&
                  & ntuv,TmpArray3,TmpArray4)
          ENDIF
          TMP1 = .NOT.TMP1
       ENDDO
    ...
    IF (IntPrint .GE. 25)THEN
       IF(TMP1)THEN
          call SPPrintWTUV(TmpArray1,AngmomPQ,nPrimQP,nPasses,nTUV,lupri)
       ELSE
          call SPPrintWTUV(TmpArray2,AngmomPQ,nPrimQP,nPasses,nTUV,lupri)
       ENDIF
    ENDIF
...
#endif
       call SPIchorbuild_Ecoeff_RHS(nPrimQ,nPrimC,nPrimD,AngmomQ,AngmomC,AngmomD,nTUVQ,&
    ...
    IF (IntPrint .GE. 25)call SPprintEcoeff(TmpArray3,nTUVQ,nCartOrbCompQ,nPrimQ,nPassQ,lupri)
    ...
    IF(TMP1)THEN !current intermediate WTUV reside in TmpArray1
       ...
       call SPDirectcontractEQgen(TmpArray1,TmpArray2,nPrimQP,nPrimP*nPasses,nPrimQ,nTUV,ntuvP,ntuvQ,&
            & TmpArray3,nCartOrbCompQ,AngmomA,AngmomB,AngmomC,AngmomD,AngmomPQ)
       IF (IntPrint .GE. 25)call SPPrintIchorTensorRE(TmpArray2,nPrimQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ,lupri)
...
#endif
       !builds RE(nPrimQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ)
       call SPDirectcontractEQgen(TmpArray2,TmpArray1,nPrimQP,nPrimP*nPasses,nPrimQ,nTUV,ntuvP,ntuvQ,&
            & TmpArray3,nCartOrbCompQ,AngmomA,AngmomB,AngmomC,AngmomD,AngmomPQ)
       IF (IntPrint .GE. 25)call SPPrintIchorTensorRE(TmpArray1,nPrimQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ,lupri)
    ENDIF
    TMP1 = .NOT.TMP1
    ...
    IF(TMP1)THEN !current intermediate RE(nPrimQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ) reside in TmpArray1
       ...
       IF(Qsegmented)THEN
          call SPcontractBasisSegQ(TmpArray1,TmpArray2,nPrimC,nPrimD,nPrimP*nPasses*nTUVP*nCartOrbCompQ) 
       ELSE
          call SPcontractBasisGenQ(TmpArray1,TmpArray2,CCC,DCC,nPrimC,nPrimD,nContC,nContD,nPrimP*nPasses*nTUVP*nCartOrbCompQ) 
       ENDIF
       IF (IntPrint .GE. 25)call SPPrintIchorTensorREC(TmpArray2,nContQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ,lupri)
    ELSE !current intermediate RE(nPrimQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ) reside in TmpArray2
       ...
       !Build REC(nContQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ)
       IF(Qsegmented)THEN
          call SPcontractBasisSegQ(TmpArray2,TmpArray1,nPrimC,nPrimD,nPrimP*nPasses*nTUVP*nCartOrbCompQ) 
       ELSE
          call SPcontractBasisGenQ(TmpArray2,TmpArray1,CCC,DCC,nPrimC,nPrimD,nContC,nContD,nPrimP*nPasses*nTUVP*nCartOrbCompQ) 
       ENDIF
       IF (IntPrint .GE. 25)call SPPrintIchorTensorREC(TmpArray1,nContQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ,lupri)
    ENDIF
    TMP1 = .NOT.TMP1
    ...
    IF(SphericalTransQ)THEN
       IF(TMP1)THEN !current intermediate REC(nContQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ) reside in TmpArray1
          ...
          IF(Sph3.AND.Sph4)THEN
             call SPSphericalTransformGenQCD(TmpArray1,TmpArray2,SPH_MAT(AngmomC)%elms,SPH_MAT(AngmomD)%elms,&
                  & nCartOrbCompC,nOrbCompC,nCartOrbCompD,nOrbCompD,nContQ*nPrimP*nPasses*nTUVP)
          ELSEIF(Sph3)THEN
             call SPSphericalTransformGenQC(TmpArray1,TmpArray2,SPH_MAT(AngmomC)%elms,&
                  & nCartOrbCompC,nOrbCompC,nOrbCompD,nContQ*nPrimP*nPasses*nTUVP)
          ELSE
             call SPSphericalTransformGenQD(TmpArray1,TmpArray2,SPH_MAT(AngmomD)%elms,&
                  & nOrbCompC,nCartOrbCompD,nOrbCompD,nContQ*nPrimP*nPasses*nTUVP)
          ENDIF
          IF (IntPrint .GE. 25) call SPPrintIchorTensorRECS(TmpArray2,nContQ,nPrimP,nPasses,nTUVP,nOrbCompQ,lupri)
       ELSE !current intermediate REC(nContQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ) reside in TmpArray2
          ...
          !Build RECS(nContQ,nPrimP,nPasses,nTUVP,nOrbCompQ)
          IF(Sph3.AND.Sph4)THEN
             call SPSphericalTransformGenQCD(TmpArray2,TmpArray1,SPH_MAT(AngmomC)%elms,SPH_MAT(AngmomD)%elms,&
                  & nCartOrbCompC,nOrbCompC,nCartOrbCompD,nOrbCompD,nContQ*nPrimP*nPasses*nTUVP)
          ELSEIF(Sph3)THEN
             call SPSphericalTransformGenQC(TmpArray2,TmpArray1,SPH_MAT(AngmomC)%elms,&
                  & nCartOrbCompC,nOrbCompC,nOrbCompD,nContQ*nPrimP*nPasses*nTUVP)
          ELSE
             call SPSphericalTransformGenQD(TmpArray2,TmpArray1,SPH_MAT(AngmomD)%elms,&
                  & nOrbCompC,nCartOrbCompD,nOrbCompD,nContQ*nPrimP*nPasses*nTUVP)
          ENDIF
          IF (IntPrint .GE. 25) call SPPrintIchorTensorRECS(TmpArray1,nContQ,nPrimP,nPasses,nTUVP,nOrbCompQ,lupri)
       ENDIF
       TMP1 = .NOT.TMP1
    ENDIF
...
#endif
       call SPcontractEcoeffGenP(TmpArray1,TmpArray2,TmpArray3,nOrbCompQ,nCartOrbCompP,nTUVP,nContQ,nPrimP*nPasses)

       IF (IntPrint .GE. 25)call SPPrintIchorTensorERECS(TmpArray2,nContQ,nPrimP,nPasses,nCartOrbCompP,nOrbCompQ,lupri)
    ELSE
#ifdef VAR_DEBUGICHOR
       IF(TMParray1maxsize.LT.nContQ*nPrimP*nPasses*nCartOrbCompP*nOrbCompQ)call ichorquit('IchorTmp1G6B',-1)
#endif
       call SPcontractEcoeffGenP(TmpArray2,TmpArray1,TmpArray3,nOrbCompQ,nCartOrbCompP,nTUVP,nContQ,nPrimP*nPasses)
       IF (IntPrint .GE. 25)call SPPrintIchorTensorERECS(TmpArray1,nContQ,nPrimP,nPasses,nCartOrbCompP,nOrbCompQ,lupri)
    ENDIF
    TMP1 = .NOT.TMP1
    ...
    IF(.NOT.TMP1)THEN !current intermediate reside in TmpArray2
       ...
       IF(Psegmented)THEN
          call SPcontractBasisSegP(TmpArray2,TmpArray1,nContQ,nPasses*nCartOrbCompP*nOrbCompQ,nPrimA,nPrimB) 
       ELSE
          call SPcontractBasisGenP(TmpArray2,TmpArray1,ACC,BCC,nContQ,nPasses*nCartOrbCompP*nOrbCompQ,nPrimA,nPrimB,nContA,nContB) 
       ENDIF
       IF (IntPrint .GE. 25) call SPPrintIchorTensorCERECS(TmpArray1,nContQ,nContP,nPasses,nCartOrbCompP,nOrbCompQ,lupri)
    ELSE !current intermediate reside in TmpArray1
       ...
       !builds CERECS(nContQ,nContP,nPasses,nCartOrbCompP,nOrbCompQ)
       IF(Psegmented)THEN
          call SPcontractBasisSegP(TmpArray1,TmpArray2,nContQ,nPasses*nCartOrbCompP*nOrbCompQ,nPrimA,nPrimB) 
       ELSE
          call SPcontractBasisGenP(TmpArray1,TmpArray2,ACC,BCC,nContQ,nPasses*nCartOrbCompP*nOrbCompQ,nPrimA,nPrimB,nContA,nContB) 
       ENDIF
       IF (IntPrint .GE. 25) call SPPrintIchorTensorCERECS(TmpArray2,nContQ,nContP,nPasses,nCartOrbCompP,nOrbCompQ,lupri)
    ENDIF
    TMP1 = .NOT.TMP1
    ...
    IF(SphericalTransP)THEN
       IF(TMP1)THEN !current intermediate reside in TmpArray1
          ...
          IF(Sph1.AND.Sph2)THEN
             call SPSphericalTransformGenPAB(TmpArray1,TmpArray2,SPH_MAT(AngmomA)%elms,&
                  & SPH_MAT(AngmomB)%elms,nCartOrbCompA,nOrbCompA,nCartOrbCompB,nOrbCompB,&
                  & nContQ*nContP*nPasses,nOrbCompQ)
          ELSEIF(Sph1)THEN
             call SPSphericalTransformGenPA(TmpArray1,TmpArray2,SPH_MAT(AngmomA)%elms,&
                  & nCartOrbCompA,nOrbCompA,nOrbCompB,nContQ*nContP*nPasses,nOrbCompQ)
          ELSE
             call SPSphericalTransformGenPB(TmpArray1,TmpArray2,SPH_MAT(AngmomB)%elms,&
                  & nOrbCompA,nCartOrbCompB,nOrbCompB,nContQ*nContP*nPasses,nOrbCompQ)
          ENDIF
          IF(IntPrint.GE.25)call SPPrintIchorTensorSCERECS(TmpArray2,&
       ...
       ELSE !current intermediate reside in TmpArray2
          ...
          !builds SCEREC(nContQ,nContP,nPasses,nOrbCompP,nOrbCompQ)
          IF(Sph1.AND.Sph2)THEN
             call SPSphericalTransformGenPAB(TmpArray2,TmpArray1,SPH_MAT(AngmomA)%elms,&
                  & SPH_MAT(AngmomB)%elms,nCartOrbCompA,nOrbCompA,nCartOrbCompB,nOrbCompB,&
                  & nContQ*nContP*nPasses,nOrbCompQ)
          ELSEIF(Sph1)THEN
             call SPSphericalTransformGenPA(TmpArray2,TmpArray1,SPH_MAT(AngmomA)%elms,&
                  & nCartOrbCompA,nOrbCompA,nOrbCompB,nContQ*nContP*nPasses,nOrbCompQ)
          ELSE
             call SPSphericalTransformGenPB(TmpArray2,TmpArray1,SPH_MAT(AngmomB)%elms,&
                  & nOrbCompA,nCartOrbCompB,nOrbCompB,nContQ*nContP*nPasses,nOrbCompQ)
          ENDIF          
          IF(IntPrint.GE.25)call SPPrintIchorTensorSCERECS(TmpArray1,&
               & nContQ,nContP,nPasses,nOrbCompP,nOrbCompQ,lupri)
       ENDIF
       TMP1 = .NOT.TMP1
    ENDIF
    ...
    IF(.NOT.TMP1)THEN
       call SPreorderABCD(TmpArray2,CDAB,nContQ*nContP*nPasses,nOrbCompP*nOrbCompQ)
    ELSE
       call SPreorderABCD(TmpArray1,CDAB,nContQ*nContP*nPasses,nOrbCompP*nOrbCompQ)
    ENDIF
  end subroutine SPICI_CPU_McM_general

  subroutine SPICI_CPU_McM_general_size(TMParray1maxsize,&
...
!       TMParray1maxsize = MAX(TMParray1maxsize,6*nPasses*nPrimP+3*nPrimP)
    ...
    IF(SphericalTransP)THEN
       ...
       TMP1 = .NOT.TMP1
    ENDIF
  end subroutine SPICI_CPU_McM_general_size
  
  subroutine SPreorderABCD(SCERECS,ABCD,ndim1,ndim2)
    implicit none
    integer,intent(in) :: ndim1,ndim2
    real(reals),intent(in) :: SCERECS(ndim1,ndim2)
    real(reals),intent(inout) :: ABCD(ndim2,ndim1)
    !
    integer :: I,J
    ...
    enddo
    !$OMP END DO
  end subroutine SPReorderABCD

  subroutine SPcontractBasisGenQ(RE,REC,CCC,DCC,nPrimC,nPrimD,nContC,nContD,ndim)
    implicit none
    integer,intent(in) :: nPrimC,nPrimD,ndim
    integer,intent(in) :: nContC,nContD
    real(reals),intent(in) :: CCC(nPrimC,nContC),DCC(nPrimD,nContD)
    real(reals),intent(in) :: RE(nPrimC,nPrimD,ndim)
    real(reals),intent(inout) :: REC(nContC,nContD,ndim)
    !
    ...
    enddo
    !$OMP END DO
  end subroutine SPcontractBasisGenQ

  subroutine SPcontractBasisSegQ(RE,REC,nPrimC,nPrimD,ndim)
    implicit none
    integer,intent(in) :: nPrimC,nPrimD,ndim
    real(reals),intent(in) :: RE(nPrimC,nPrimD,ndim)
    real(reals),intent(inout) :: REC(ndim)
    !
    integer :: i,ipC,ipD
    real(reals) :: TMP
    ...
    enddo
    !$OMP END DO
  end subroutine SPcontractBasisSegQ

  subroutine SPSphericalTransformGenQCD(REC,RECS,SPHMATC,SPHMATD,ijk3,ijk3s,ijk4,ijk4s,ndim)
    implicit none
    integer,intent(in) :: ijk3,ijk3s,ijk4,ijk4s,ndim
    real(reals),intent(in) :: SPHMATC(ijk3,ijk3s),SPHMATD(ijk4,ijk4s)
    real(reals),intent(in) :: REC(ndim,ijk3,ijk4)
    real(reals),intent(inout) :: RECS(ndim,ijk3s,ijk4s)
    !
    integer :: c,d,cs,ds,i
    real(reals) :: TMP,TMPD
    ...
    enddo
    !$OMP END DO
  end subroutine SPSphericalTransformGenQCD

  subroutine SPSphericalTransformGenQC(REC,RECS,SPHMATC,ijk3,ijk3s,ijk4,ndim)
    implicit none
    integer,intent(in) :: ijk3,ijk3s,ijk4,ndim
    real(reals),intent(in) :: SPHMATC(ijk3,ijk3s)
    real(reals),intent(in) :: REC(ndim,ijk3,ijk4)
    real(reals),intent(inout) :: RECS(ndim,ijk3s,ijk4)
    !
    integer :: c,cs,ds,i
    real(reals) :: TMP
    !$OMP DO COLLAPSE(3) PRIVATE(cs,ds,i)
    ...
    enddo
    !$OMP END DO
  end subroutine SPSphericalTransformGenQC

  subroutine SPSphericalTransformGenQD(REC,RECS,SPHMATD,ijk3,ijk4,ijk4s,ndim)
    implicit none
    integer,intent(in) :: ijk3,ijk4,ijk4s,ndim
    real(reals),intent(in) :: SPHMATD(ijk4,ijk4s)
    real(reals),intent(in) :: REC(ndim*ijk3,ijk4)
    real(reals),intent(inout) :: RECS(ndim*ijk3,ijk4s)
    !
    integer :: d,ds,i
    real(reals) :: TMP
    ...
    do ds = 1,ijk4s
       ...
       enddo
    enddo
  end subroutine SPSphericalTransformGenQD

  subroutine SPcontractEcoeffGenP(IN,OUT,Ecoeff,nOrbCompQ,nCartOrbCompP,nTUVP,nContQ,nPrimPassesP)
    implicit none
    Integer,intent(IN) :: nOrbCompQ,nCartOrbCompP,nTUVP,nContQ,nPrimPassesP
    Real(reals),intent(IN)  :: IN(nContQ,nPrimPassesP,nTUVP,nOrbCompQ)
    Real(reals),intent(IN)  :: Ecoeff(nPrimPassesP,nTUVP,nCartOrbCompP)
    Real(reals),intent(OUT) :: OUT(nContQ,nPrimPassesP,nCartOrbCompP,nOrbCompQ)
    !local variables
    ...
    ELSE
       ...
       !$OMP END DO
    ENDIF
  END subroutine SPcontractEcoeffGenP
  
  subroutine SPcontractBasisGenP(ERECS,CERECS,ACC,BCC,nContQ,ndim,nPrimA,nPrimB,nContA,nContB)
    implicit none
    integer,intent(in) :: nPrimA,nPrimB,ndim,nContA,nContB,nContQ
    real(reals),intent(in) :: ACC(nPrimA,nContA),BCC(nPrimB,nContB)
    real(reals),intent(in) :: ERECS(nContQ,nPrimA,nPrimB,ndim)
    real(reals),intent(inout) :: CERECS(nContQ,nContA,nContB,ndim)
    !
    ...
    do icB = 1,nContB
       do icA = 1,nContA
          do i = 1,ndim
             do iQ = 1,nContQ
                CERECS(iQ,icA,icB,i) = 0.0E0_reals
             enddo
             do ipB = 1,nPrimB
                TMPB = BCC(ipB,icB)
                do ipA = 1,nPrimA
  ...
  end subroutine SPcontractBasisGenP

  subroutine SPcontractBasisSegP(ERECS,CERECS,nContQ,ndim,nPrimA,nPrimB)
    implicit none
    integer,intent(in) :: nPrimA,nPrimB,ndim,nContQ
    real(reals),intent(in) :: ERECS(nContQ,nPrimA,nPrimB,ndim)
    real(reals),intent(inout) :: CERECS(nContQ,ndim)
    !
    integer :: i,ipA,ipB,iQ
    !$OMP DO PRIVATE(i,ipA,ipB,iQ)
    do i = 1,ndim
       do iQ = 1,nContQ
          CERECS(iQ,i) = 0.0E0_reals
       enddo
       do ipB = 1,nPrimB
          do ipA = 1,nPrimA
             do iQ = 1,nContQ
    ...
    enddo
    !$OMP END DO
  end subroutine SPcontractBasisSegP

  subroutine SPSphericalTransformGenPAB(CERECS,SCERECS,SPHMATA,SPHMATB,&
    ...
    real(reals),intent(in) :: CERECS(ndim,ijk1,ijk2,nOrbCompQ)
    real(reals),intent(inout) :: SCERECS(ndim,ijk1s,ijk2s,nOrbCompQ)
    !
    ...
    do ijkQ = 1,nOrbCompQ
     do as = 1,ijk1s
      do bs = 1,ijk2s
       do i = 1,ndim
        SCERECS(i,as,bs,ijkQ) = 0.0E0_reals
       enddo 
       do b = 1,ijk2
        TMPB = SPHMATB(b,bs)
        do a = 1,ijk1
    ...
    enddo
    !$OMP END DO
  end subroutine SPSphericalTransformGenPAB

  subroutine SPSphericalTransformGenPA(CERECS,SCERECS,SPHMATA,&
       & ijk1,ijk1s,ijk2,ndim,nOrbCompQ)
    implicit none
    integer,intent(in) :: ijk1,ijk1s,ijk2,ndim,nOrbCompQ
    real(reals),intent(in) :: SPHMATA(ijk1,ijk1s)
    real(reals),intent(in) :: CERECS(ndim,ijk1,ijk2*nOrbCompQ)
    real(reals),intent(inout) :: SCERECS(ndim,ijk1s,ijk2*nOrbCompQ)
    !
    integer :: a,as,i,ijkQ
    real(reals) :: TMP
    ...
    enddo
    !$OMP END DO
  end subroutine SPSphericalTransformGenPA

  subroutine SPSphericalTransformGenPB(CERECS,SCERECS,SPHMATB,&
       & ijk1,ijk2,ijk2s,ndim,nOrbCompQ)
    implicit none
    integer,intent(in) :: ijk1,ijk2,ijk2s,ndim,nOrbCompQ
    real(reals),intent(in) :: SPHMATB(ijk2,ijk2s)
    real(reals),intent(in) :: CERECS(ndim*ijk1,ijk2,nOrbCompQ)
    real(reals),intent(inout) :: SCERECS(ndim*ijk1,ijk2s,nOrbCompQ)
    !
    integer :: b,bs,i,ijkQ
    real(reals) :: TMP
    ...
    enddo
    !$OMP END DO
  end subroutine SPSphericalTransformGenPB

  subroutine SPSphericalTransformGen(CEREC,SCERECS,SPHMATA,SPHMATB,&
    ...
    real(reals),intent(in) :: SPHMATA(ijk1,ijk1s),SPHMATB(ijk2,ijk2s)
    real(reals),intent(in) :: SPHMATC(ijk3,ijk3s),SPHMATD(ijk4,ijk4s)
    real(reals),intent(in) :: CEREC(ndim,ijk1,ijk2,ijk3,ijk4)
    real(reals),intent(inout) :: SCERECS(ijk1s,ijk2s,ijk3s,ijk4s,ndim)
    !
    integer :: a,b,c,d,as,bs,cs,ds,i
    real(reals) :: TMP,TMPD,TMPCD,TMPBCD
    do i = 1,ndim
     do as = 1,ijk1s
      do bs = 1,ijk2s
       do cs = 1,ijk3s
     ...
     enddo
    enddo
   end subroutine SPSphericalTransformGen

  subroutine SPPreCalciChorSPHMAT(MaxAngmom)
    implicit none
    ...
    do L=0,MaxAngmom
        ...
        call SPIchorSph_to_Cart_matrix(L,SPH_MAT(L)%elms,nLM,nXYZ,LUPRI,IPRINT)
     enddo
   end subroutine SPPreCalciChorSPHMAT

   subroutine SPFreeIchorSPHMAT()
    implicit none
    integer :: L
    ...
    enddo
    deallocate(SPH_MAT)
  end subroutine SPFreeIchorSPHMAT

  FUNCTION IchorFACULT(LUPRI,N)
    IMPLICIT NONE
    real(reals), PARAMETER :: D1=1E0_reals
    integer        :: N,I,LUPRI
    real(reals)    :: IchorFACULT
    IF (N .LT. 0) THEN
       WRITE (LUPRI,'(/,A,I10,/A)')&
  ...
  FUNCTION IchorFACUL2(LUPRI,N)
    ...
    IF (N .LT. 0) THEN
       ...
       END DO
       IF (IchorFACUL2 .EQ. 0E0_reals) THEN
          WRITE (LUPRI,'(/,A,I10,/A)')&
  ...
  subroutine SPIchorSph_to_Cart_matrix(L,SCMAT,nLM,nXYZ,LUPRI,IPRINT)
    implicit none
    INTEGER,intent(in) :: L,nLM,nXYZ,LUPRI,IPRINT
    REAL(reals),intent(inout) :: SCMAT(nXYZ,nLM)
    !
    Real(reals), parameter :: DM1 = -1.0E0_reals, D1 = 1.0E0_reals, D2 = 2.0E0_reals
    INTEGER     :: M1,MADR,MABS,V0
    REAL(reals) :: FACNRM,FAC3,FACTOR
    ...
    ELSEIF(L .EQ. 1)THEN ! P
       call SPICHOR_DZERO(SCMAT,9)
       SCMAT(1,1)=1E0_reals
       SCMAT(2,2)=1E0_reals
       SCMAT(3,3)=1E0_reals
    ELSEIF(L .GT. 1)THEN
       call SPICHOR_DZERO(SCMAT,nLM*nXYZ)
       DO M1 = 0, 2*L 
          M = M1 - L
          IF (L.EQ. 1) THEN
             IF (M .EQ. -1) MADR =  0  
       ...
       ENDDO
    ENDIF
  END subroutine SPIchorSph_to_Cart_matrix

  subroutine SPcontractBasisGen(ERE,CEREC,ACC,BCC,CCC,DCC,&
    ...
    real(reals),intent(in) :: ACC(nPrimA,nContA),BCC(nPrimB,nContB)
    real(reals),intent(in) :: CCC(nPrimC,nContC),DCC(nPrimD,nContD)
    real(reals),intent(in) :: ERE(nPrimC,nPrimD,nPrimA,nPrimB,ndim)
    real(reals),intent(inout) :: CEREC(nContC,nContD,nContA,nContB,ndim)
    !
    integer :: i, icA,icB,icC,icD,ipA,ipB,ipC,ipD
    real(reals) :: TMP,TMPB,TMPAB,TMPABD
      do i = 1,ndim
       do icB = 1,nContB
        do icA = 1,nContA
         do icd = 1,nContD
       ...
       enddo
      enddo
  end subroutine SPcontractBasisGen

subroutine SPcontractEcoeffGen(IN,OUT,Ecoeff,&
...
Real(reals),intent(IN)  :: IN(nPrimQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ)
Real(reals),intent(IN)  :: Ecoeff(nPrimP,nPasses,nTUVP,nCartOrbCompP)
Real(reals),intent(OUT) :: OUT(nPrimQ,nPrimP,nPasses,nCartOrbCompP,nCartOrbCompQ)
!local variables
...
DO ijkP = 1,nCartOrbCompP
 ...
 ENDDO
ENDDO
END subroutine SPcontractEcoeffGen

subroutine SPDirectcontractEQgen(WTUV3,OUT,nPrim,nPrimPassP,nPrimQ,nTUV,ntuvP,ntuvQ,Ecoeffs,ijkQ,l1,l2,l3,l4,MAXJ)
implicit none
Integer,intent(in) :: nPrim,nPrimQ,ntuv,ntuvP,ntuvQ,nPrimPassP
Integer,intent(in) :: l1,l2,l3,l4,MAXJ,ijkQ
Real(reals),intent(in) :: Ecoeffs(nPrimQ,ntuvQ,ijkQ)
Real(reals),intent(in) :: WTUV3(nPrimQ,nPrimPassP,ntuv)
Real(reals),intent(inout) :: OUT(nPrimQ,nPrimPassP,ntuvP,ijkQ)
!local variables
!Real(reals)     :: Ec(nPrimQ)
!Real(reals) :: maxEcont,THRESHOLD,Etmp
...
!!$ ENDDO
!!$ENDDO
END subroutine SPDirectcontractEQgen

subroutine SPbuild_Rpq(nPrimQ,nPassP,nPrimP,Qcent,Pcent,Rpq,&
     & IatomApass,IatomBpass,MaxPasses,nAtomsA,nAtomsB)
  implicit none
  integer,intent(in) :: nPrimQ,nPassP,nPrimP,MaxPasses,nAtomsA,nAtomsB
  integer,intent(in) :: IatomApass(MaxPasses),IatomBpass(MaxPasses)
  real(reals),intent(in) :: Qcent(3,nPrimQ),Pcent(3,nPrimP,nAtomsA,nAtomsB)
  real(reals),intent(inout) :: Rpq(nPrimQ*nPrimP*nPassP,3)
  !
...
!$OMP DO PRIVATE(iP,iPrimQ,iPassP,iPrimP,iAtomA,iAtomB)
  ...
  ENDDO
!$OMP END DO
end subroutine SPbuild_Rpq

subroutine SPbuildRJ000_general(nPasses,nPrimQ,nPrimP,nTABFJW1,nTABFJW2,reducedExponents,&
  ...
  REAL(reals),intent(in)     :: TABFJW(0:nTABFJW1,0:nTABFJW2)
  real(reals),intent(in)     :: Rpq(nPrimQ*nPrimP*nPasses,3)     
  REAL(reals),intent(inout)  :: RJ000(0:Jmax,nPrimQ*nPrimP*nPasses)
  !local variables
  REAL(reals)     :: D2JP36,WVAL
  REAL(reals),PARAMETER :: HALF =0.5E0_reals,D1=1E0_reals,D2 = 2E0_reals, D4 = 4E0_reals, D100=100E0_reals
  Real(reals),parameter :: D12 = 12E0_reals, TENTH = 0.01E0_reals
  REAL(reals), PARAMETER :: COEF2 = HALF,  COEF3 = - D1/6E0_reals, COEF4 = D1/24E0_reals
  REAL(reals), PARAMETER :: COEF5 = - D1/120E0_reals, COEF6 = D1/720E0_reals
  Real(reals) :: WDIFF,RWVAL,REXPW,PREF,D2MALPHA
  ...
  Real(reals) :: W2,W3,R,pqx,pqy,pqz
  ...
  ENDDO
  !$OMP END DO
END subroutine SPbuildRJ000_general
...
subroutine SPbuildGJ000_general(nPasses,nPrimQ,nPrimP,reducedExponents,&
  ...
  REAL(reals),intent(in)     :: integralPrefactor(nPrimQ*nPrimP)
  real(reals),intent(in)     :: Rpq(nPrimQ*nPrimP*nPasses,3)     
  REAL(reals),intent(inout)  :: GJ000(0:Jmax,nPrimQ*nPrimP*nPasses)
  !local variables
  Real(reals),PARAMETER :: One = 1.0E0_reals, MinusTwo = -2.0E0_reals
  REAL(reals),PARAMETER :: PI=3.14159265358979323846E0_reals
  REAL(reals),PARAMETER :: OneOverTwoPi=0.159154943091895335768883763372514E0_reals
  INTEGER :: iP,iPrimQP,K,J
  REAL(reals) :: DumFact,Ooor,RhoTilde,TEMP3,TEMP1,TEMP2,R2
  ...
  ENDDO
  !$OMP END DO
END subroutine SPbuildGJ000_general
...
subroutine SPbuildGJ000Grad_general(nPasses,nPrimQ,nPrimP,reducedExponents,&
     & GJ000,JMAX,integralPrefactor,Rpq)
  IMPLICIT NONE
  INTEGER,intent(in)         :: nPrimQ,nPrimP,Jmax,nPasses
  REAL(reals),intent(in)     :: reducedExponents(nPrimQ*nPrimP)
  ...
  REAL(reals),intent(inout)  :: GJ000(0:Jmax,nPrimQ*nPrimP*nPasses)
  !local variables
  Real(reals),PARAMETER :: TwoSqrtPi=3.54490770181103205459633496668229E0_reals
  REAL(reals),PARAMETER :: Anderthalb = 1.5E0_reals, One = 1.0E0_reals
  REAL(reals),PARAMETER :: MinusTwo = -2.0E0_reals
  INTEGER :: iP,iPrimQP,K,J
  REAL(reals) :: DumFact,Ooor,RhoTilde,TEMP3,TEMP1,TEMP2,R2
  real(reals) :: InvGGemexponent,RhoHat
  ...
  ENDDO
  !$OMP END DO
END subroutine SPbuildGJ000Grad_general
...
subroutine SPbuildGJ000Coulomb_general(nPasses,nPrimQ,nPrimP,nTABFJW1,nTABFJW2,reducedExponents,&
  ...
  REAL(reals),intent(inout)  :: GJ000(0:Jmax,nPrimQ*nPrimP*nPasses)
  !local variables
  REAL(reals),PARAMETER :: HALF =0.5E0_reals,D1=1E0_reals,D2 = 2E0_reals, D4 = 4E0_reals
  REAL(reals),PARAMETER :: D100=100E0_reals
  Real(reals),parameter :: D12 = 12E0_reals, TENTH = 0.01E0_reals
  REAL(reals), PARAMETER :: COEF2 = HALF,  COEF3 = - D1/6E0_reals, COEF4 = D1/24E0_reals
  REAL(reals), PARAMETER :: COEF5 = - D1/120E0_reals, COEF6 = D1/720E0_reals
  REAL(reals), PARAMETER :: GFAC0 =  D2*0.4999489092E0_reals
  ...
  REAL(reals), PARAMETER :: PID4 = PI/D4, PID4I = D4/PI
  REAL(reals), PARAMETER :: SMALL = 1E-15_reals
  Real(reals), parameter :: One = 1.0E0_reals, MinusTwo = -2E0_reals, MinusHalf = -0.5E0_reals
  Real(reals) :: WDIFF,RWVAL,REXPW,PREF,D2MALPHA
  Real(reals) :: W2,W3,R,Ooor,RhoHat,RhoTilde,TEMP1,TEMP2,TEMP3,R2
  REAL(reals)     :: D2JP36,WVAL,RJ000(0:Jmax)
  ...
  ENDDO
  !$OMP END DO
END subroutine SPbuildGJ000Coulomb_general

subroutine SPPrintRJ000(RJ000,AngmomPQ,nPrim,nPasses,lupri)
  implicit none
  integer,intent(in) :: AngmomPQ,nPrim,nPasses,lupri
  real(reals),intent(in) :: RJ000(0:AngmomPQ,nPrim,nPasses)
  !
  integer :: I,J,iPass
  ...
  DO iPass=1,nPasses
     ...
     ENDDO
  ENDDO
end subroutine SPPrintRJ000

subroutine SPPrintIchorTensorRE(RE,nPrimQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ,lupri)
implicit none
integer,intent(in) :: nTUVP,nCartOrbCompQ,nPrimQ,nPrimP,nPasses,lupri
real(reals),intent(in) :: RE(nPrimQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ)
...
do iQ = 1,nPrimQ
 ...
 enddo
enddo
end subroutine SPPrintIchorTensorRE

subroutine SPPrintIchorTensorREC(REC,nContQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ,lupri)
implicit none
integer,intent(in) :: nTUVP,nCartOrbCompQ,nContQ,nPrimP,nPasses,lupri
real(reals),intent(in) :: REC(nContQ,nPrimP,nPasses,nTUVP,nCartOrbCompQ)
integer :: iTUV,iPass,iP,ijkQ,iQ
iTUV=0
...
do iQ = 1,nContQ
 ...
 enddo
enddo
end subroutine SPPrintIchorTensorREC

subroutine SPPrintIchorTensorRECS(RECS,nContQ,nPrimP,nPasses,nTUVP,nOrbCompQ,lupri)
implicit none
integer,intent(in) :: nTUVP,nOrbCompQ,nContQ,nPrimP,nPasses,lupri
real(reals),intent(in) :: RECS(nContQ,nPrimP,nPasses,nTUVP,nOrbCompQ)
integer :: iTUV,iPass,iP,ijkQ,iQ
iTUV=0
...
do iQ = 1,nContQ
 ...
 enddo
enddo
end subroutine SPPrintIchorTensorRECS

subroutine SPPrintIchorTensorERECS(ERECS,nContQ,nPrimP,nPasses,nCartOrbCompP,nOrbCompQ,lupri)
implicit none
integer,intent(in) :: nOrbCompQ,nCartOrbCompP,nContQ,nPrimP,nPasses,lupri
real(reals),intent(in) :: ERECS(nContQ,nPrimP,nPasses,nCartOrbCompP,nOrbCompQ)
!
...
do iQ = 1,nContQ
 ...
 enddo
enddo
end subroutine SPPrintIchorTensorERECS

subroutine SPPrintIchorTensorCERECS(CERECS,nContQ,nContP,nPasses,nCartOrbCompP,nOrbCompQ,lupri)
implicit none
integer,intent(in) :: nOrbCompQ,nCartOrbCompP,nContQ,nContP,nPasses,lupri
real(reals),intent(in) :: CERECS(nContQ,nContP,nPasses,nCartOrbCompP,nOrbCompQ)
!
integer :: iPass,iP,ijkQ,ijkP,iQ
...
do iQ = 1,nContQ
 ...
 enddo
enddo
end subroutine SPPrintIchorTensorCERECS

subroutine SPPrintIchorTensorSCERECS(SCERECS,nContQ,nContP,nPasses,nOrbCompP,nOrbCompQ,lupri)
implicit none
integer,intent(in) :: nOrbCompQ,nOrbCompP,nContQ,nContP,nPasses,lupri
real(reals),intent(in) :: SCERECS(nContQ,nContP,nPasses,nOrbCompP,nOrbCompQ)
!
integer :: iPass,iP,ijkQ,ijkP,iQ
...
do iQ = 1,nContQ
 ...
 enddo
enddo
end subroutine SPPrintIchorTensorSCERECS

end module SPIchorEriCoulombintegralCPUMcMGeneralMod
