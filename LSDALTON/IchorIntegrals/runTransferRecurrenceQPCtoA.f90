  ...
  subroutine PASSsub
    ...
    DO JMAX=2,MaxAngmomQP
       DO JP = 1, JMAX
          ...
          WRITE(*,'(A)')'  integer :: iPassQ,iPrimP,iPrimQ,IP,iTUVQ,iPrimB,iPrimD'
...
!          WRITE(*,'(A)')'     iPrimC = iPrimQ - ((iPrimQ-1)/nPrimC)*nPrimC'
          ...
          WRITE(*,'(A)')'     facX = -(expBX+Dexp(iPrimD)*Xcd)*invexpP'
          WRITE(*,'(A)')'     facY = -(expBY+Dexp(iPrimD)*Ycd)*invexpP'
          WRITE(*,'(A)')'     facZ = -(expBZ+Dexp(iPrimD)*Zcd)*invexpP'
