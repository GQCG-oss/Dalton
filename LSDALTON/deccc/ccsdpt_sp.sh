...
list="ccsdpt_kernels"
  ...
  for i in $list
	 ...
	 sed "s/call ijk_loop/call sp_ijk_loop/g" $pwd_dir/tmp1 > $pwd_dir/tmp2
         ...
         sed "s/call ccsdpt_driver/call sp_ccsdpt_driver/g" $pwd_dir/tmp2 > $pwd_dir/tmp1
         sed "s/call trip_generator/call sp_trip_generator/g" $pwd_dir/tmp1 > $pwd_dir/tmp2
...
list="ccsdpt_dec"
  ...
  for i in $list
     do
         ...
         sed "s/call ccsdpt_contract_/call sp_ccsdpt_contract_/g" $pwd_dir/tmp1 > $pwd_dir/tmp2
         sed "s/call ls_dgemm_acc/call ls_sgemm_acc/g" $pwd_dir/tmp2 > $pwd_dir/tmp1
         sed "s/call array_reorder_3d/call array_reorder_3d_sp/g" $pwd_dir/tmp1 > $pwd_dir/tmp2
         sed "s/call array_reorder_3d_acc/call array_reorder_3d_acc_sp/g" $pwd_dir/tmp2 > $pwd_dir/tmp1
         sed "s/public :: ccsdpt/public :: sp_ccsdpt/g" $pwd_dir/tmp1 > $pwd_dir/tmp2
