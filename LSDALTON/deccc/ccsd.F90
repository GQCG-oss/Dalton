...
function precondition_doubles_memory(omega2,ppfock,qqfock) result(prec)
  ...
  subroutine get_ccsd_residual_integral_driven(ccmodel,deltafock,omega2,t2,fock,govov,no,nv,&
     ...
     integer(kind=ls_mpik) :: Hadw,t2_dw,Gbiw,sio4w,gvvoow,gvoovw
     ...
     if(master.and.DECinfo%PL>1) then
        ...
        ActuallyUsed=get_min_mem_req(no,nv,nb,MaxActualDimAlpha,MaxActualDimGamma,iter,4,scheme,.true.)
     endif
...
!#endif
        ...
        BatchAlpha: do while(alphaB<=nbatchesAlpha) ! AO batches
           ...
           if ( Ccmodel > MODEL_CC2 ) then
              ...
              if(scheme==4)then
                 ...
                 call time_start_phase(PHASE_COMM, at = time_intloop_work)
                 call array_add(gvvooa,1.0E0_realk,w2%d,wrk=w0%d,iwrk=w0%n)
                 call time_start_phase(PHASE_WORK, at = time_intloop_comm)
#endif
              endif
              call lsmpi_poke()
           ...
           if(iter==1)then
              ...
              if(scheme==4)then
                 ...
                 ! i a j b
#ifdef VAR_MPI
                 if( lock_outside .and. .not. restart )call arr_lock_wins(govov,'s',mode)
                 call dgemm('t','n',no,v2o,la,1.0E0_realk,xo(fa),nb,w1%d,la,0.0E0_realk,w2%d,no)
                 call time_start_phase(PHASE_COMM, at = time_intloop_work)
                 if( .not. restart )call array_add(govov,1.0E0_realk,w2%d,order=[1,4,2,3],wrk=w3%d,iwrk=w3%n)
                 if( restart )call array_add(govov,1.0E0_realk,w2%d,order=[1,4,2,3] )
                 call time_start_phase(PHASE_WORK, at = time_intloop_comm)
#endif
              endif
              call lsmpi_poke()
           ...
           if ( Ccmodel > MODEL_CC2 .and. ( iter/=1.or.restart ) ) then
              ...
              if(scheme==4)then
                 ...
                 call array_add(gvoova,1.0E0_realk,w2%d)
                 call time_start_phase(PHASE_WORK, at = time_intloop_comm)
#endif
              endif
              call lsmpi_poke()
           ...
           if(scheme==2)then
              ...
              call time_start_phase(PHASE_COMM, at = time_intloop_work )
              call array_add(omega2,1.0E0_realk,w2%d,wrk=w0%d,iwrk=w0%n)
              call time_start_phase(PHASE_WORK, at = time_intloop_comm )
#endif
           else
     ...
     if(Ccmodel>MODEL_CC2)then
        ...
        !***********************
        print *,infpar%lg_mynum,"start cnd"
        call lsmpi_barrier(infpar%lg_comm)
        ...
        print *,infpar%lg_mynum,"done cnd"
        call lsmpi_barrier(infpar%lg_comm)
     ...
     if(s==4.or.s==3.or.s==0)then
        ...
        if(.not.lock_outside.and.traf1)then
           ...
           if(traf1)then
              ...
              call time_start_phase(PHASE_COMM, at = tw)
              call array_two_dim_1batch(omega2,[1,2,3,4],'a',w1,3,fai1,tl1,.false.,debug=.true.)
              call time_start_phase(PHASE_WORK, at = tc)
           endif
        endif
     ...
     if(traf.or.me==0)then
        ...
        if(s==4)then
           ...
           if(lock_outside)then
              ...
              print *,infpar%lg_mynum,"get first"
              call lsmpi_barrier(infpar%lg_comm)

              !call arr_lock_wins(gvvoo,'s',mode)
              call array_two_dim_1batch(gvvoo,[1,3,4,2],'g',w2,2,fai,tl,.false.,debug=.true.)
              !call arr_unlock_wins(gvvoo,.true.)

              print *,infpar%lg_mynum,"done first"
              call lsmpi_barrier(infpar%lg_comm)
        ...
        if(s==2.and.lock_outside)then
           ...
           print *,infpar%lg_mynum,"get second"
           call lsmpi_barrier(infpar%lg_comm)
           ...
           print *,infpar%lg_mynum,"done second"
           call lsmpi_barrier(infpar%lg_comm)
        ...
        if(s==4)then
           ...
           if(lock_outside)then
              ...
              print *,infpar%lg_mynum,"get third",nv,nv/2
              call lsmpi_barrier(infpar%lg_comm)
              ...
              print *,infpar%lg_mynum,"done third"
              call lsmpi_barrier(infpar%lg_comm)
        ...
        if(s==4)then
           ...
           print *,infpar%lg_mynum,"get fourth"
           call lsmpi_barrier(infpar%lg_comm)
           ...
           print *,infpar%lg_mynum,"done fourth"
           call lsmpi_barrier(infpar%lg_comm)
        ...
        if(s==4)then
           ...
           call array_two_dim_1batch(gvoov,[1,4,2,3],'g',w2,2,fai,tl,.false.,debug=.true.)
           call array_two_dim_1batch(gvvoo,[1,3,2,4],'g',w3,2,fai,tl,.false.,debug=.true.)
        ...
        if(s==4)then
           ...
           call array_two_dim_1batch(u2,[2,3,4,1],'g',w3,2,fai,tl,.false.,&
              &mem=MemFree,debug=.true.)!,wrk=w3(o2v2+1:w3size),iwrk=(w3size-o2v2))
...
!> \author Patrick Ettenhuber
!> \date January 2012
  function get_min_mem_req(no,nv,nb,nba,nbg,iter,choice,s,print_stuff) result (memrq)
    implicit none
    integer, intent(in) :: no,nv,nb
    ...
    w0size = get_wsize_for_ccsd_int_direct(0,no,nv,nb,nba,nbg,s)
    w1size = get_wsize_for_ccsd_int_direct(1,no,nv,nb,nba,nbg,s)
    w2size = get_wsize_for_ccsd_int_direct(2,no,nv,nb,nba,nbg,s)
    w3size = get_wsize_for_ccsd_int_direct(3,no,nv,nb,nba,nbg,s)
    !w0
  ...
  subroutine get_mo_ccsd_residual(ccmodel,pgmo_diag,pgmo_up,t1,omega1,t2,omega2, &
    ...
    type(array), intent(inout) :: omega1
    type(array), intent(inout) :: t2
    type(array), intent(inout) :: omega2
    ...
    type(array), intent(inout) :: qqfock
    !> occupied-virtual block of the t1-fock matrix
    type(array), intent(inout) :: pqfock
    !> virtual-occupied block of the t1-fock matrix
    type(array), intent(inout) :: qpfock
   ...
   if (.not.local) then
    ...
    if(.not.local)then
      ...
      omega2%access_type = MASTER_ACCESS
      if (iter==1) then
        call array_mv_dense2tiled(govov,.true.)
      else
  ...
  subroutine wrapper_get_C2_and_D2(tmp0,tmp1,tmp2,t2,u2,govov,gvoov,gvvoo,no,nv,omega2)
    ...
    type(array) :: u2a, govova, gvvooa, gvoova
  ...
  subroutine ccsd_debug_print(ccmodel,print_nr,master,local,&
     ...
     type(mpi_realk) :: w1
     type(array) :: omega2,govov,gvvooa,gvoova
     character(ARR_MSG_LEN) :: msg
  ...
  function get_wsize_for_ccsd_int_direct(wnr,no,nv,nb,nba,nbg,s) result(wsize)
     implicit none
     integer, intent(in) :: wnr,no,nv,nb,nba,nbg,s
...
subroutine ccsd_data_preparation()
  ...
  use tensor_interface_module, only: array_ainit,array_free,&
      &memory_allocate_array_dense,memory_deallocate_array_dense,&
