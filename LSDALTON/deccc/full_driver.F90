...
module full 
  ...
  public :: full_driver, full_canonical_rimp2, full_canonical_mp2, full_canonical_mp2B
  private
       ...
       elseif(DECinfo%ccModel==MODEL_RIMP2)then
          ...
          if(DECinfo%ccModel==MODEL_MP2) then
             if(DECinfo%use_canonical ) then
                !simple conventional MP2 calculation
                !only works for canonical orbitals
                !no amplitudes stored. 
...
!       & numnodes,DECinfo%output)
  ...
  !for now 
  !1. First chose nOccBatchdimI and (thereby nrownodes) as big as possible (recalc integrals)
  !   chose nOccBatchdimI so that (nb*dimAlphaMPI*dimGammaMPI*nOccBatchDimI) can fit in mem!
  !2. chose ncolnodes so that nodes = nrownodes*ncolnodes

  !tmp1(dimAlphaMPI,dimGammaMPI,nb,nb)                 Integral
  !tmp2(dimAlphaMPI,dimGammaMPI,nb,nOccBatchDimI)      dgemm
  !tmp3(nb,nOccBatchDimI,dimAlphaMPI,dimGammaMPI)      reorder 
  !tmp4(nvirt,nOccBatchDimI,dimAlphaMPI,dimGammaMPI)   dgemm
  !tmp5(nvirt*nOccBatchDimI,dimAlphaMPI*nOccBatchDimJ) Bcast/dgemm 
  !tmp7(dimAlpha,nOccBatchDimJ,nvirt,nOccBatchDimI)    reorder
  !VOVO(nvirt,nOccBatchDimJ,nvirt,nOccBatchDimI)       dgemm
  ...
  nOccBatchDimImax = CEILING(1.0E0_realk*nocc/nrownodes) 
...
subroutine full_canonical_mp2_slave
  use full,only: full_canonical_mp2
  use infpar_module !infpar
  use lsmpi_type,only:ls_mpiInitBuffer,ls_mpiFinalizeBuffer,&
       & LSMPIBROADCAST,MPI_COMM_LSDALTON 
  use lsmpi_op,only: mpicopy_lsitem
