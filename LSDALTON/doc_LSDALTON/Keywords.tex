...
\item **WAVE FUNCTION contains info about the wave function (e.g. HF/DFT) and settings
...
For the most effcient orbital localization for large molecular systems \cite{hoyvik:3L}, the .ARH, .START/TRILEVEL  and .LCM options are recommended for use as described in Section \ref{subsec:densopt}.  
...
Should only be used in combination with a .STARTINGUESS/TRILEVEL option (see Section \ref{}) and the .LCM keyword. The keyword is used to skip orbital localization after the second level in the three level procedure. Not recommended for general use, but can be used to save  computational time if only the virtual space is to be localized.  
