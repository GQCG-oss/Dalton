...
of the DALTON.INP file. In general, DALTON.INP is divided into the following sections under
the headlines:
\begin{itemize}
...
\end{itemize}
Each of these sections may contain subsections, indicated by a single asterisk. DALTON.INP
...
\item[\Key{DENSFIT}] Use density-fitting. When using this keyword, an auxiliary
basis set has to be specified in \mol\ file.
...
(how many excitation energies,
threshold for the response solver etc.)
...
The test cases may be found in the {\bf test/} directory.
...
\item[\Key{*SOLVER}] \verb| | \newline
.\verb|<Solver information>|\newline
...
\item[\Key{COMPLEX}] \verb| | \newline
\verb|<Damping parmeter gamma>|\newline
The $\gamma$ parmeter is treated as an imaginary component of a frequency of intrest and describes a broadening of
a spectrum.
\item[\Key{OLSEN}] \verb| | \newline
...
Damped two-photon absorption where both individual photons
...
\end{enumerate}
{\bf Test case}: LSresponse/LSresponse\_mcd, LSresponse/LSresponse\_mcd2\newline
LSresponse/LSresponse\_mcd3  
\begin{description}
...
Restart unfinished DEC calculation. Requires that .info files are present in the folder where the calculation is restarted.
...
\begin{itemize}
\item
\textit{fullmp2}: Calculate conventional canonical MP2 energy.
\end{itemize}
...
density matrix elements or molecular orbital coefficients. The .plt files can then be generated by running a new calculation where the DALTON.INP file has been modified as exemplified below.
...
insert the following section in the DALTON.INP file used to generate the  \emph{dens.restart} file in the first place:
 \newline
**PLT \newline
.INPUT \newline
dens.restart \newline
...
insert the following section in the DALTON.INP file used to generate the  \emph{lcm\_orbitals.u} file in the first place::
 \newline
**PLT \newline
.INPUT \newline
lcm\_orbitals.u \newline
