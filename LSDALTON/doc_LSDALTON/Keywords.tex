...
The methods differ in the way approximate Hessians are built and updated.
\Key{INIMOD}, \Key{INIRED} and \Key{INITEV} provide different approximations for the initial Hessian, 
...
The default setting specifies the minimization in internal coordinates \cite{DaltonOpt}, with model Hessian \cite{ModelHess} updated with BFGS formula. 
...
The default value of $\epsilon$ is $10^{-5}$ (\Key{VLOOSE}, \Key{LOOSE}, 
\Key{TIGHT} and \Key{VTIGHT} correspond to other predefined values for the convergence criteria).
\newline
...
with BFGS update of initially unit Hessian.
\item
\textit{geomopt/Excited\_state\_opt}:
performs a geometry optimization of an excited state in redundant internal coordinates 
with B3LYP.
\item
\textit{geomopt/redint\_bfgs}:
performs a geometry optimization in redundant internal coordinates
...
and McWeenie's purification used for constructing initial guess.
\item
\textit{geomopt/geoopt\_constrain}:
shows a constrained optimization of ethane,
keeping the C-C bond distance fixed
(coordinate \#1 as determined from geoopt\_constrain).
\item
\textit{geomopt/cartes\_trilevel\_newconv}:
performs a geometry optimization in Cartesian coordinates
with ``trilevel'' starting guess and a default convergence criteria,
which is set to *loose* DFP Hessian update.
\end{itemize}
