...
This keyword will distribute the memory of matrices among the MPI processes. This keyword will also use the SCALAPACK library 
to parallize the matrix operations like matrix multiplication. 
\item[\Key{SCALAPACKBLOCKSIZE}] 
\verb| | \newline
\verb|<ScalapackBlockSize>|\newline
The blocksize used in SCALAPACK matrices
\item[\Key{SCALAPACKNODES}] 
\verb| | \newline
\verb|<ScalapackNodes>|\newline
The number of MPI processes you wish to use for the SCALAPACK format. This must be a subset of the total MPI processes used. 
It may be benficial to chose a subset of the total MPI processes because the matrix operations scale differently with MPI nodes than the 
Integral evaluation. 
...
\item[\Key{ADMM}] Use the Auxiliary Density Matrix Method (ADMM)~\cite{ADMM:2010,ADMM:2014} 
...
\item[\Key{ADMMS}] Use the ADMMS approximation of Ref.~\cite{ADMM:2014}.
Reccomended.

\item[\Key{ADMMP}] Use the ADMMP approximation of Ref.~\cite{ADMM:2014}.

\item[\Key{ADMMQ}] Use the ADMMQ approximation of Ref.~\cite{ADMM:2014}.
Not reccomended.
