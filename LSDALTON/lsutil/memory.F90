...
!END SUBROUTINE print_mem
  ...
  !> \date April 2013
  !> Printout modified February 2014 /SR
...
integer(kind=MPI_ADDRESS_KIND) :: mpi_intlen,lb,bytes
   ...
   if (.not.ASSOCIATED(A).or..not.c_associated(cip)) then
      print *,'Memory previously released!!'
      call memory_error_quit('ERROR(mpi_deallocate_iV): memory previously released',nsize)
   endif
...
IF (IERR.NE. 0) THEN
   ...
   CALL MEMORY_ERROR_QUIT('Error in CCORBITAL_allocate_1dim',nsize)
ENDIF
...
integer (kind=long) :: nsize
   ...
   if (.not.ASSOCIATED(CCORBITALITEM)) then
      print *,'Memory previously released!!'
      call memory_error_quit('Error in CCORBITAL_deallocate_1dim - memory previously released',nsize)
   endif
   DEALLOCATE(CCORBITALITEM,STAT = IERR)
   IF (IERR.NE. 0) THEN
      write(*,*) 'Error in CCORBITAL_deallocate_1dim',IERR
      CALL MEMORY_ERROR_QUIT('Error in CCORBITAL_deallocate_1dim',nsize)
   ENDIF
   NULLIFY(CCORBITALITEM)
...
IF (IERR.NE. 0) THEN
   ...
   CALL MEMORY_ERROR_QUIT('Error in CCATOM_allocate_1dim',nsize)
ENDIF
...
integer (kind=long) :: nsize
   ...
   if (.not.ASSOCIATED(CCATOMITEM)) then
      print *,'Memory previously released!!'
      call memory_error_quit('Error in CCATOM_deallocate_1dim - memory previously released',nsize)
   endif
   DEALLOCATE(CCATOMITEM,STAT = IERR)
   IF (IERR.NE. 0) THEN
      write(*,*) 'Error in CCATOM_deallocate_1dim',IERR
      CALL MEMORY_ERROR_QUIT('Error in CCATOM_deallocate_1dim',nsize)
   ENDIF
   NULLIFY(CCATOMITEM)
   ...
   subroutine mem_deallocated_mem_CCORBITAL(nsize)
     ...
     IF(mem_InsideOMPsection)THEN!we add to thread private variables
        ...
        if (mem_tp_allocated_CCORBITAL < 0) then
           call memory_error_quit('Error in mem_tp_deallocated_mem_tp_CCORBITAL - probably integer overflow!',nsize)
        endif
        !Count also the total memory:
        mem_tp_allocated_global = mem_tp_allocated_global - nsize
        if (mem_tp_allocated_global < 0) then
     ...
     ELSE
        ...
        if (mem_allocated_CCORBITAL < 0) then
           call memory_error_quit('Error in mem_deallocated_mem_CCORBITAL - probably integer overflow!',nsize)
        endif
        !Count also the total memory:
        mem_allocated_global = mem_allocated_global - nsize
        if (mem_allocated_global < 0) then
   ...
   subroutine mem_deallocated_mem_CCATOM(nsize)
     ...
     IF(mem_InsideOMPsection)THEN!we add to thread private variables
        ...
        if (mem_tp_allocated_CCATOM < 0) then
           call memory_error_quit('Error in mem_tp_deallocated_mem_tp_CCATOM - probably integer overflow!',nsize)
        endif
        !Count also the total memory:
        mem_tp_allocated_global = mem_tp_allocated_global - nsize
        if (mem_tp_allocated_global < 0) then
     ...
     ELSE
        ...
        if (mem_allocated_CCATOM < 0) then
           call memory_error_quit('Error in mem_deallocated_mem_CCATOM - probably integer overflow!',nsize)
        endif
        !Count also the total memory:
        mem_allocated_global = mem_allocated_global - nsize
        if (mem_allocated_global < 0) then
