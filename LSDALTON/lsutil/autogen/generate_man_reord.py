...
def main():
  ...
  if(not os.path.exists(lsutildir+"manual_reorderings.F90")):
    ...
    if(not os.path.exists(lsutildir+"manual_utils.F90")):
      writenew = True
...
##################################################################################################
##################################################################################################
def produce_files(lsutildir,args):
   maxr = 4
   minr = 2
   f=open(lsutildir+"manual_reorderings.F90",'w')
   foo=open(lsutildir+"manual_utils.F90",'w')
   #GET COMMAND LINE ARGUMENTS
   debug_loops = args[0]
   nocollapse  = args[1]
    
    
   # WRITE HEADER AND GET NOW 
   now = datetime.datetime.now()
   write_module_header_r(f,now,args,lsutildir)
   write_module_header_u(foo,now,args,lsutildir)
   
   
   write_testing_framework(f,minr,maxr)

   forfoo = ["_full2tile","_tile2full"]
   ...
   for modes in range(maxr,minr-1,-1):
     ...
     for perm in all_permutations:
       ...
       for  addition in forfoo:
         ...
         #END THE SUBROUTINE
         foo.write("  end subroutine "+subroutinename+"\n\n")
       if doreord :
         # WRITE THE SUBROUTINE HEADER AND GET ITS NAME
         subroutinename = ""
   ...
   foo.write("end module manual_utils_module")
   foo.close()
...
def write_subroutine_header(f,idxarr,perm,now,modes,sname,ad):
  ...
  sname =  "manual_"+reordstr1+"_reordering"+ad
...
def write_module_header_u(f,now,args,lsutildir):
   ...
   f.write("module manual_utils_module\n")
   f.write("  use precision\n")
   f.write("  contains\n")

def write_module_header_r(f,now,args,lsutildir):
   ...
   f.write("  use manual_utils_module\n")
