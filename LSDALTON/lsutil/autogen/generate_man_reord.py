...
def main():
  ...
  args = []
  args.append(("VAR_LSDEBUG" in sys.argv))
  args.append(("nocollapse" in sys.argv))
  ...
  if(not os.path.exists(lsutildir+"manual_reorderings.F90")):
    ...
    for line in c:
      ...
      if "!VAR DEBUG:" in line:
        writenew = (not line.split()[-1] == str(args[0]))
        if writenew:
          break

      if "!VAR NOCOLLAPSE:" in line:
...
def produce_file(lsutildir,args):
   ...
   f.write("!VAR DEBUG: "+str(debug_loops)+"\n")
   f.write("!VAR NOCOLLAPSE: "+str(nocollapse)+"\n")
