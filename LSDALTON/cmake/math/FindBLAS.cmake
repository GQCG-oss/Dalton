...
macro(find_mkl)
	...
	if(${CMAKE_HOST_SYSTEM_PROCESSOR} STREQUAL "x86_64")
		...
		if(ENABLE_SCALAPACK)	
		    if(ENABLE_OMP)
                         set(blas_libs mkl_scalapack_ilp64 mkl_core mkl_intel_ilp64 mkl_intel_thread mkl_blacs_intelmpi_ilp64 guide pthread m)
	            else()
                         set(blas_libs mkl_scalapack_ilp64 mkl_core mkl_intel_ilp64 mkl_sequential mkl_blacs_intelmpi_ilp64 guide pthread m)
		    endif()		    
                else()
		    if(ENABLE_OMP)
                         set(blas_libs mkl_core mkl_intel_ilp64 mkl_intel_thread guide pthread m)
		     else()
		...
		if(ENABLE_SCALAPACK)	
		    if(ENABLE_OMP)
                        set(blas_libs mkl_scalapack_lp64 mkl_core mkl_intel_lp64 mkl_intel_thread mkl_blacs_intelmpi_lp64 guide pthread m)
                    else()
                        set(blas_libs mkl_scalapack_lp64 mkl_core mkl_intel_lp64 mkl_sequential mkl_blacs_intelmpi_lp64 guide pthread m)
      		    endif()
                else()
		    if(ENABLE_OMP)
                        set(blas_libs mkl_core mkl_intel_lp64 mkl_intel_thread guide pthread m)
                    else()
		...
		if(ENABLE_SCALAPACK)	
		    if(ENABLE_OMP)
   		       set(blas_libs mkl_scalapack mkl_core mkl_intel mkl_intel_thread mkl_blacs_intelmpi guide pthread m)
                    else()
   		       set(blas_libs mkl_scalapack mkl_core mkl_intel mkl_sequential mkl_blacs_intelmpi guide pthread m)
      		    endif()
                else()
		    if(ENABLE_OMP)
		       set(blas_libs mkl_core mkl_intel mkl_intel_thread guide pthread m)
                    else()
