...
!         real(tensor_real), pointer, contiguous, private:: elems(:)=>NULL() !tensor elements (1:*)
         real(tensor_real), pointer, contiguous :: elems(:)=>NULL() !tensor elements (1:*)
        end type tens_loc_t
 !Tensor argument:
 ...
 !User-level complete tensor contraction specification:
        type, public:: dil_tens_contr_t
         ...
         real(tensor_real), private:: alpha              !multiplication prefactor (can be explicit)
         real(tensor_real), private:: beta               !scaling factor for the destination tensor (always implicit)
...
#ifdef VAR_PGF90
         real(tensor_real), pointer, contiguous :: buffer(:)=>NULL() !work buffer
#else
         real(tensor_real), pointer, contiguous, private:: buffer(:)=>NULL() !work buffer
#endif
...
#ifdef VAR_PGF90
         real(tensor_real), pointer, contiguous :: buf_ptr(:)=>NULL() !buffer pointer
#else
         real(tensor_real), pointer, contiguous, private:: buf_ptr(:)=>NULL() !buffer pointer
#endif
        end type arg_buf_t
 !Device buffers:
...
!INTERFACES:
 !CPU pointer memory allocation:
        interface cpu_ptr_alloc
         module procedure cpu_ptr_alloc_r !deafult real kind (tensor_real)
        end interface cpu_ptr_alloc
 !CPU pointer memory deallocation:
        interface cpu_ptr_free
         module procedure cpu_ptr_free_r !deafult real kind (tensor_real)
...
!This subroutine sets up a tensor argument: either "LOCAL" or "DISTRIBUTED".
        ...
        real(tensor_real), intent(in), target, optional:: tens_elems(1:*) !in: tensor elements (for local tensors only)
...
!This user-level subroutine cleans a full tensor contraction specification.
        ...
        tcontr%alpha=1E0_tensor_real; tcontr%beta=0E0_tensor_real
...
!----------------------------------------------
        ...
        if(ndev.ge.1.and.ndev.le.MAX_DEVS) then
         ...
         dil_get_min_buf_size=max(dil_get_min_buf_size,sz)
         dil_get_min_buf_size=(int(real(dil_get_min_buf_size,8)*scl,INTL)/ALIGNMENT+1_INTL)*ALIGNMENT*BUFS_PER_DEV*ndev*tensor_real
...
!This subroutine prepares an internal work buffer for a giving tensor contraction.
        ...
        integer(INTD), intent(inout):: ierr                      !out: error code (0:success)
        real(tensor_real), intent(in), target, optional:: ext_buf(1:)  !in: external buffer space
        integer(INTD), intent(in), optional:: alloc_type         !in: allocation type
        integer(INTL):: nelems,sreal
        real(tensor_real):: val
        ...
        if(ierr.eq.0) then
         ...
         if(mem_lim.ge.MIN_BUF_MEM) then
          val=0E0_tensor_real; sreal=int(sizeof(val),INTL)
...
!Note that the tensor arguments must have been set already (prior to this call)!
        ...
        real(tensor_real), intent(in), optional:: alpha        !in: alpha prefactor
 ...
 !Other stuff:
          if(present(alpha)) then; tcontr%alpha=alpha; else; tcontr%alpha=1E0_tensor_real; endif
          if(tcontr%dest_arg%store_type.eq.'l'.or.tcontr%dest_arg%store_type.eq.'L') then
           tcontr%beta=1E0_tensor_real !local destination arguments use accumulation in GEMM
          else
           tcontr%beta=0E0_tensor_real !distributed destination arguments use assignment in GEMM (accumulated via MPI)
          endif
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(i0) SCHEDULE(GUIDED)
            do i0=i1,i1+ivol-1_INTL
             arg_buf%buf_ptr(i0)=0E0_tensor_real
            enddo
!$OMP END PARALLEL DO
           else
            ierr=1
...
!This subroutine allocates memory for a 1d pointer array (default real).
        implicit none
        real(tensor_real), pointer, contiguous, intent(inout):: arr(:) !out: 1d array
        ...
        type(C_PTR):: caddr
        real(tensor_real), pointer, contiguous:: fptr(:)
        real(tensor_real):: val
 ...
 !Pinned malloc:
         case(DIL_ALLOC_PINNED)
          val=0E0_tensor_real; csize=int(nelems*sizeof(val),C_SIZE_T); caddr=C_NULL_PTR
          !`Write (call C wrapper for cudaMallocHost)
          call c_f_pointer(caddr,fptr,[nelems]); arr(bs:)=>fptr; nullify(fptr)
 !MPI malloc:
         case(DIL_ALLOC_MPI)
          caddr=C_NULL_PTR
          val=0E0_tensor_real; mpi_size=int(nelems*sizeof(val),MPI_ADDRESS_KIND)
...
!If not, it may NOT work correctly!!!
        implicit none
        real(tensor_real), pointer, contiguous, intent(inout):: arr(:) !inout: 1d array
        integer(INTD), intent(inout):: ierr                      !out: error code (0:success)
        integer(INTD), intent(in), optional:: attr               !in: attributes (pinned, etc.)
        type(C_PTR):: caddr
        integer(INTD):: flags
...
!This subroutine extracts a slice from a tensor.
        implicit none
!-------------------------------------------------
        integer(INTD), parameter:: real_kind=tensor_real
!-------------------------------------------------
...
!This subroutine starts fetching all tiles necessary for constructing a given tensor part.
        ...
        type(tensor), intent(in):: tens_arr     !in: tensor stored distributively in terms of tiles
        type(subtens_t), intent(in):: tens_part !in: tensor part specification
        real(tensor_real), intent(inout):: bufi(1:*)  !out: local buffer where the tiles will be put in
        integer(INTD), intent(inout):: ierr     !out: error code (0:success)
        logical, intent(in), optional:: locked  !in: if .TRUE., MPI windows are assumed already locked
        integer(INTD):: i,k,tile_host,signa(1:MAX_TENSOR_RANK),tile_dims(1:MAX_TENSOR_RANK)
        integer(INTL):: tile_vol,buf_end
...
!This subroutine finishes tile fetching and unpacks the tiles into a dense tensor slice.
        ...
        type(tensor), intent(in):: tens_arr     !in: tensor stored distributively in terms of tiles
        type(subtens_t), intent(in):: tens_part !in: tensor part specification
        real(tensor_real), intent(inout):: bufi(1:*)  !in: local buffer containing the tiles
        real(tensor_real), intent(inout):: bufo(1:*)  !out: local buffer that will contain the dense tensor slice
        integer(INTD), intent(inout):: ierr     !out: error code (0:success)
        logical, intent(in), optional:: locked  !in: if .TRUE., MPI windows are assumed already locked
...
!This subroutine packs a dense tensor slice into tiles and starts uploading them.
        ...
        type(tensor), intent(inout):: tens_arr  !inout: tensor stored distributively in terms of tiles
        type(subtens_t), intent(in):: tens_part !in: tensor part specification
        real(tensor_real), intent(in):: bufi(1:*)     !in: local buffer containing the dense tensor slice
        real(tensor_real), intent(inout):: bufo(1:*)  !tmp: temporary buffer from where the tiles will be uploaded
        integer(INTD), intent(inout):: ierr     !out: error code (0:success)
        logical, intent(in), optional:: locked  !in: if .TRUE., MPI windows are assumed already locked
        integer(INTD):: i,k,n,tile_host,signa(1:MAX_TENSOR_RANK),tile_dims(1:MAX_TENSOR_RANK)
        integer(INTL):: tile_vol,buf_end
...
!This subroutine completes upload of all tiles constituting a given dense tensor slice.
        ...
        type(tensor), intent(inout):: tens_arr  !inout: tensor stored distributively in terms of tiles
        type(subtens_t), intent(in):: tens_part !in: tensor part specification
        real(tensor_real), intent(inout):: bufo(1:*)  !in: local buffer containing the tiles to upload
        integer(INTD), intent(inout):: ierr     !out: error code (0:success)
        logical, intent(in), optional:: locked  !in: if .TRUE., MPI windows are assumed already locked
        integer(INTD):: i,k,tile_host,signa(1:MAX_TENSOR_RANK),tile_dims(1:MAX_TENSOR_RANK)
        integer:: tile_num,tile_win
...
!   but postponed for later: lasync(1:nasync) will contain the needed information.
        ...
        real(tensor_real), intent(in):: alpha                           !in: tensor contraction prefactor (GEMM alpha)
        real(tensor_real), intent(in):: beta                            !in: GEMM beta
        integer(INTL), intent(in):: mem_lim                       !in: local memory limit (bytes): buffer space
        real(tensor_real), intent(inout), target, contiguous:: hbuf(1:) !inout: existing external buffer (to avoid mem allocations)
        integer(INTD), intent(inout):: ierr                       !out: error code (0:success)
...
!Init:
        val=0E0_tensor_real; size_of_real=sizeof(val)
         ...
         if(arg_keep(1:1).ne.'R') then
          ...
          elseif(darg%store_type.eq.'l'.or.darg%store_type.eq.'L') then !local tensor
           ...
           if(DIL_DEBUG) then
            ...
            &jtm,dble(2_INTL*jvol*tensor_real)/(jtm*1024d0*1024d0*1024d0),je
           endif
           if(je.ne.0) then; errc=4; return; endif
          else
           errc=5; return
...
!Allocate the work buffer, if needed:
        if(tcontr%alloc_type.eq.DIL_ALLOC_NOT) then
         ...
         if(associated(tcontr%buffer)) then
          ...
          if(tcbv*tensor_real.ge.mem_lim) then
...
!-------------------------------------------------------
        real(tensor_real) function dil_tensor_norm1(tens,ierr) !PARALLEL (MPI+OMP)
!This function computes 1-norm of a tensor.
        implicit none
        type(tensor), intent(in):: tens               !in: tensor
        integer(INTD), intent(inout), optional:: ierr !out: error code (0:success)
        integer(INTL):: i,l
        real(tensor_real):: nrm1
        integer(INTD):: mpi_dtyp,errc

        errc=0; dil_tensor_norm1=0E0_tensor_real; nrm1=0E0_tensor_real
...
!--------------------------------------------------------------
        real(tensor_real) function dil_array_norm1(arr,arr_size,ierr) !PARALLEL (OMP)
        implicit none
        real(tensor_real), intent(in):: arr(1:*)            !in: array
        ...
        errc=0; dil_array_norm1=0E0_tensor_real
...
!Each MPI process initializes its own tiles.
        ...
        real(tensor_real), intent(in), optional:: val
        integer(INTD):: lti
        real(tensor_real):: vlu
 !get the slaves here
 ...
 !loop over local tiles and zero them individually
        vlu=0E0_tensor_real; if(present(val)) vlu=val
        do lti=1,a%nlti
!$OMP WORKSHARE
         a%ti(lti)%t=vlu
!$OMP END WORKSHARE
...
!------------------------------------------
        ...
        implicit none
        real(tensor_real), intent(inout):: a(1:*)     !out: array (contiguous)
        integer(INTL), intent(in):: sz          !in: number of elements to initialize
        real(tensor_real), intent(in), optional:: val !in: initialization value (defaults to zero)
        integer(INTL):: i
        real(tensor_real):: v

        if(present(val)) then; v=val; else; v=0E0_tensor_real; endif
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(i) SCHEDULE(GUIDED)
        do i=1,sz
         a(i)=v
        enddo
...
!This subroutine prints a real array into a file, with an optional ordering done to it.
        implicit none
        real(tensor_real), intent(in), target:: arr(1:)         !inout: array to print
        integer(INTD), intent(inout), optional:: ierr     !out: error code (0:success)
        ...
        real(tensor_real), pointer, contiguous:: sar(:)
...
!=============================================
        ...
        real(tensor_real), pointer, contiguous:: darr(:),larr(:),rarr(:),barr(:)
        type(dil_tens_contr_t):: tcr
        character(128):: tcs
        real(tensor_real):: val0,val1,val2
        real(8):: tmb,tm
