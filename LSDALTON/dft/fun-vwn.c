...
static integer  vwn_read(const char* conf_line, real *hfweight);
...
static integer
vwn_read(const char* conf_line, real *hfweight)
{
    return 1;
}
            ...
            const struct vwn_params* ferro)
{
    ...
    if(rhoa<VWN_ZERO) rhoa = 1e-35;
    if(rhob<VWN_ZERO) rhob = 1e-35;
    rho = rhoa + rhob;
    vwn_en_pot(ep_p, rho, 0, para);
...
{
    printf("vwni_third not implemented."); exit(1);
}
