...
*/
/* fun-cam_compx.c:
Complementary (i.e. (Slater+B88X)-camx) X-only part of cam...
*/
...
/* INTERFACE PART */
static integer cam_compx_isgga(void) { return 1; }
static integer cam_compx_read(const char *conf_line, real *hfweight);
static void cam_compx_report(integer lupri);
static real cam_compx_energy(const FunDensProp* dp);
static void cam_compx_first(FunFirstFuncDrv *ds,   real factor,
                           const FunDensProp* dp);
static void cam_compx_second(FunSecondFuncDrv *ds, real factor,
                            const FunDensProp* dp);
static void cam_compx_third(FunThirdFuncDrv *ds, real factor,
                           const FunDensProp* dp);
static void cam_compx_fourth(FunFourthFuncDrv *ds, real factor,
                            const FunDensProp* dp);

Functional CamcompxFunctional = {
  "Camcompx",       /* name */
  cam_compx_isgga,   /* gga-corrected */
  cam_compx_read,
  cam_compx_report,
  cam_compx_energy,
  cam_compx_first,
...
static integer
cam_compx_read(const char *conf_line, real *hfweight)
{
...
static void
cam_compx_report(integer lupri)
{
 ...
 * =================================================================== */
static real
cam_compx_b_energy_small(real a)
{
    real res;
...
static real
cam_compx_b_first_small(real a)
{
    real res;
    a = 2*a; /* the expension derived for different a; correct for this. */
...
static real
cam_compx_b_energy_large(real a)
{
    real res, ac, a2;
    integer i;
...
static real
cam_compx_b_first_large(real a)
{
    real tmp;
    real ac, a2;
    integer i;
 ...
 * =================================================================== */
static real
cam_compx_b_energy_medium(real a)
{
...
/* tested version */
static real
cam_compx_b_first_medium(real a)
{
    real t1 = 1/a;
    real t2 = a*a;
    real t3 = 1/t2;
...
static real
cam_compx_b_second_medium(real a)
{
...
static real
cam_compx_b_third_medium(real a)
{
...
static real
cam_compx_b_fourth_medium(real a)
{
...
#define EVALUATOR(a,type) \
  ((a<0.14) ? cam_compx_b_ ## type ## _small(a)  : \
  ((a<4.25) ? cam_compx_b_ ## type ## _medium(a) : \
   cam_compx_b_ ## type ## _large(a)))

static real
cam_compx_energy_sigma(real rho, real ex)
{
    real a       = fun_a(rho, ex);
...
static real
cam_compx_energy(const FunDensProp *dp)
{
    real res, ea, eb, ex;
    FunDensProp dsigma;
    ...
    ea = ex*(1-ALPHA) - BETA*cam_compx_energy_sigma(dp->rhoa, ex);
	...
	dsigma.gradab = dsigma.grada*dsigma.gradb;
        ex = 0.5*(SlaterFunctional.func(&dsigma) +
                  BeckeFunctional.func(&dsigma)*BECKE88_CORR_WEIGHT);
        eb = ex*(1-ALPHA) - BETA*cam_compx_energy_sigma(dp->rhob, ex);
    } else eb = ea;
    res = -(ea + eb);

    res +=
        SlaterFunctional.func(dp) +
        BeckeFunctional.func(dp);

    return res;
}

static void
cam_compx_first_sigma(real rho, real ex,
...
static void
cam_compx_first(FunFirstFuncDrv *ds, real factor, const FunDensProp *dp)
{
    RGFirstDrv res, dfun;
    FunDensProp dsigma;
    real ex, ex_weight;
    ...
    cam_compx_first_sigma(dp->rhoa, ex, &dfun, &res);
    ex_weight = (1-ALPHA);
    ...
    if(dp->rhob>10e-13) {
	    ...
	    dsigma.gradab = dsigma.grada*dsigma.gradb;
            ...
            cam_compx_first_sigma(dp->rhob, ex, &dfun, &res);
        }
    ...
    BeckeFunctional.first(ds, factor, dp);

}

static void
cam_compx_second_sigma(real rho, real ex, RGSecondDrv *f2,
                      RGSecondDrv *res)
{
    ...
    b_second = BETA*cam_compx_b_second_medium(a);
...
static void
cam_compx_second(FunSecondFuncDrv *ds, real factor, const FunDensProp* dp)
{
    FunSecondFuncDrv f2;
    ...
    dfun.df11 = f2.df1010; 
    cam_compx_second_sigma(dp->rhoa, ex, &dfun, &res);
	...
	dsigma.gradab = dsigma.grada*dsigma.gradb;
        ...
        dfun.df11 = f2.df0101;
        cam_compx_second_sigma(dp->rhob, ex, &dfun, &res);
    }
    ...
    BeckeFunctional.second(ds, factor, dp);
...
static void
cam_compx_third_sigma(real rho, real ex, RGThirdDrv *f3,
                     RGThirdDrv *res)
{
    ...
    b_first  = BETA*EVALUATOR(a, first);
    b_second = BETA*cam_compx_b_second_medium(a);
    b_third  = BETA*cam_compx_b_third_medium(a);
...
static void
cam_compx_third(FunThirdFuncDrv *ds, real factor, const FunDensProp* dp)
{
    FunThirdFuncDrv f3;
    RGThirdDrv res, dfun;
    FunDensProp dsigma;
    ...
    cam_compx_third_sigma(dp->rhoa, ex, &dfun, &res);
	...
	dsigma.gradab = dsigma.grada*dsigma.gradb;
        ...
        cam_compx_third_sigma(dp->rhob, ex, &dfun, &res);
    }
    ...
    BeckeFunctional.third(ds, factor, dp);
...
/* ===================================================================
   ...
   =================================================================== */
static void
cam_compx_fourth_sigma(real rho, real ex, RGFourthDrv *f4,
		      RGFourthDrv *res)
{
    ...
    b_second = BETA*cam_compx_b_second_medium(a_);
    b_third  = BETA*cam_compx_b_third_medium(a_);
    b_fourth = BETA*cam_compx_b_fourth_medium(a_);
...
static void
cam_compx_fourth(FunFourthFuncDrv *ds, real factor,
		const FunDensProp* dp)
{
    FunFourthFuncDrv f4;
    ...
    cam_compx_fourth_sigma(dp->rhoa, ex, &dfun, &res);
	...
	dfun.df13 = f4.df0103; dfun.df04 = f4.df0004;
        cam_compx_fourth_sigma(dp->rhob, ex, &dfun, &res);
    }
    ...
    BeckeFunctional.fourth(ds, factor, dp);
...
integer main(integer argc, char *argv[])
{
    ...
    for(a=0.1; a<10; a += 0.1) {
        bfactor_first  = cam_compx_b_second_medium(a*FAC);//EVALUATOR(a, second);
