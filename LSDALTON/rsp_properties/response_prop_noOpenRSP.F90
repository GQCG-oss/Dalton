...
!        call LSTIMER('START',t1,t2,config%LUPRI)
!        call NucSpecNMRshieldresponse_noOpenRSP(molcfg,F,D,S)
!        call LSTIMER('NucSpecNMRshield',t1,t2,config%LUPRI)
     endif
...
!4. 
!5. Alternative expression for RHS using hderiv not Bderiv
!    Seem simple 1. RHS = -P_anti(hkDS)
!    Dk = [D,Xk]_S     D0b = -DSbD   
!    Tr(D*hkb) + TrDk[hb + Gb(D) + 1/2G(D0b)] + TrD0b[hk+1/2G(Dk)]
!    I need to be able to call call II_get_prop for a specific nuclei. 

! TODO
!1.  Read JCP 115 page 10344                           
!2.  Read JCP 134, 074102 (2011) + JCP 128, 221102
!3.  write up the theory. Sx term seem to be missing?  Turn over rule.  
!4.  There is a discrepency - I think it is related to nummerical noise - 
!    so try to fist add Fock matrix contrbutions together and then do the 
!    dot product. See if that changes things
!5  .DFT to fix the signs and factors in front of DFT contributions. 
!
!6.  Include convergence test on the quantity Tr([D,X]*B) not X itself - make 
!    call to (1) in rsp_solver 
!    Add the convergence criteria also requires the full derivative matrix to test convergence
!    add a nuclear specific for prop 'NST    '

!7.  Reduce memory usage 
!8.  Test Speed
!9.  Test solving the response equations together 
!10. try .CSR
!11. test sparsity of the matrices
!12. how local is the RHS? 
!13. big ass calculation
subroutine NucSpecNMRshieldresponse_noOpenRSP(molcfg,F,D,S)
  implicit none
  type(rsp_molcfg), intent(inout) :: molcfg
  type(Matrix),intent(in) :: F(1),D(1),S
  !
  ...
  WRITE(LUPRI,*) '=========================================================='
  WRITE(LUPRI,*) ' Nuclei specific NMR shielding                         ST '
  WRITE(LUPRI,*) '=========================================================='

  !#############################################################################
  !##      Calc RHS of eq 70 JCP 115 page 10344                               ## 
  !##      Note that we have used the notation: FX = hX+GX(D)                 ##
  !##                                                                         ## 
  !##      Using First formula                                                ##
  !##                                                                         ## 
  ...
  call mat_init(hX(1),nbast,nbast)
  call mat_init(hX(2),nbast,nbast)
  call mat_init(hX(3),nbast,nbast)                                     !# Matrices Allocated 3
  call II_get_PSO_spec(LUPRI,LUERR,molcfg%SETTING,hX,1)
  call mat_init(tempm1,nbast,nbast)                                    !# Matrices Allocated 4
  do icoor = 1,3 
     call mat_mul(hX(icoor),D(1),'n','n',1.0E0_realk,0.0E0_realk,tempm1)
     call mat_init(RHS(icoor),nbast,nbast)
     call mat_mul(tempm1,S,'n','n',1.0E0_realk,0.0E0_realk,RHS(icoor))

     call mat_mul(D(1),hX(icoor),'n','n',1.0E0_realk,0.0E0_realk,tempm1)
     call mat_mul(S,tempm1,'n','n',1.0E0_realk,-1.0E0_realk,RHS(icoor))


!     call mat_free(hX(icoor))
  enddo                
  call mat_free(tempm1)
...
!  enddo
  ...
  do icoor = 1,3 
     ...
     !############################################################################
     !##      STEP 2: Make D^k=[D_0,X^b]_s                                      ## 
     !############################################################################   
     call mat_init(Dx(icoor),nbast,nbast)
     call ABCcommutator(nbast,D(1),Xx(1),S,Dx(icoor))
     call mat_free(Xx(1))
     call mat_scal(-4.0d0,Dx(icoor))
  enddo !B-field komponen
  
  !# Matrices Allocated 3 (DX = D^k)

  call mem_alloc(NMST,3,3)
  ...
  do icoor=1,3
!     do jcoor=1,3*natoms  ! magnetic moment koordinate
     do jcoor=1,3  ! magnetic moment koordinate
        NMST(jcoor,icoor) = 2.0E0_realk*expval(icoor,jcoor)
        WRITE(lupri,*)'A NMST(',jcoor,',',icoor,') = ',2.0E0_realk*Factor*expval(icoor,jcoor)
     enddo
  enddo
  call mem_dealloc(expval)
  ...
  call II_get_prop(LUPRI,LUERR,molcfg%SETTING,FX,3,'MAGMOM ')
  do icoor=1,3     ! magnetic
     do jcoor=1,3  ! magnetic moment koordinate
        NMST(jcoor,icoor) = NMST(jcoor,icoor) - mat_dotproduct(Dx(jcoor),FX(icoor))        
        WRITE(lupri,*)'B NMST(',jcoor,',',icoor,') = ',-Factor*mat_dotproduct(Dx(jcoor),FX(icoor))
     enddo
  enddo
  call mat_free(FX(1))
  call mat_free(FX(2))
  ...
  call mat_init(FX(3),nbast,nbast)                                     
  !  Calculate the two electron Magnetic derivative Coulomb matrix contribution to RHS
  call II_get_magderivJ(LUPRI,LUERR,molcfg%SETTING,nbast,D,FX)
  do icoor=1,3     ! magnetic
     do jcoor=1,3  ! magnetic moment koordinate
        NMST(jcoor,icoor) = NMST(jcoor,icoor) - mat_dotproduct(Dx(jcoor),FX(icoor))        
        WRITE(lupri,*)'C NMST(',jcoor,',',icoor,') = ',-Factor*mat_dotproduct(Dx(jcoor),FX(icoor))
     enddo
  enddo
  !  Calculate the two electron Magnetic derivative Exchange matrix contribution to RHS
  call II_get_magderivK(LUPRI,LUERR,molcfg%SETTING,nbast,D,FX)
  do icoor=1,3     ! magnetic
     do jcoor=1,3  ! magnetic moment koordinate
        NMST(jcoor,icoor) = NMST(jcoor,icoor) - mat_dotproduct(Dx(jcoor),FX(icoor))        
        WRITE(lupri,*)'D NMST(',jcoor,',',icoor,') = ',-Factor*mat_dotproduct(Dx(jcoor),FX(icoor))
     enddo
  enddo
  if(molcfg%setting%do_dft)then
     !  Adding DFT contribution to G^x(D)  
     call II_get_xc_magderiv_kohnsham_mat(LUPRI,LUERR,molcfg%SETTING,nbast,D(1),FX)
     do icoor=1,3     ! magnetic
        do jcoor=1,3  ! magnetic moment koordinate
           NMST(jcoor,icoor) = NMST(jcoor,icoor) - mat_dotproduct(Dx(jcoor),FX(icoor))        
           WRITE(lupri,*)'E NMST(',jcoor,',',icoor,') = ',-Factor*mat_dotproduct(Dx(jcoor),FX(icoor))
        enddo
     enddo
  endif
  call mat_free(FX(1))
  ...
  call mat_init(tempm1,nbast,nbast)
  do icoor = 1,3 
     call mat_mul(SX(icoor),D(1),'n','n',1E0_realk,0.E0_realk,tempm1)     !tempm1 = SB*D
     call mat_free(SX(icoor))                    
     call mat_init(DB(icoor),nbast,nbast)        
     call mat_mul(D(1),tempm1,'n','n',-1.0E0_realk,0.E0_realk,DB(icoor))  !D0^b = -D*SB*D
  enddo
  call mat_free(tempm1)
  ! G(D0^b) = J(D0^b) + K(D0^b)
  call mat_init(FX(1),nbast,nbast)
  call mat_init(FX(2),nbast,nbast)
  call mat_init(FX(3),nbast,nbast)                                     
  call di_GET_GbDs(lupri,luerr,DB,FX,3,molcfg%setting)
  do icoor=1,3     ! magnetic
     do jcoor=1,3  ! magnetic moment koordinate
        NMST(jcoor,icoor) = NMST(jcoor,icoor) - mat_dotproduct(Dx(jcoor),FX(icoor))        
        WRITE(lupri,*)'F NMST(',jcoor,',',icoor,') = ',-Factor*mat_dotproduct(Dx(jcoor),FX(icoor))
     enddo
  enddo
  IF(molcfg%setting%do_dft)THEN
     call II_get_xc_linrsp(lupri,luerr,molcfg%setting,nbast,DB,D(1),FX,3) 
     do icoor=1,3     ! magnetic
        do jcoor=1,3  ! magnetic moment koordinate
           NMST(jcoor,icoor) = NMST(jcoor,icoor) - mat_dotproduct(Dx(jcoor),FX(icoor))        
           WRITE(lupri,*)'G NMST(',jcoor,',',icoor,') = ',-Factor*mat_dotproduct(Dx(jcoor),FX(icoor))
        enddo
     enddo
  ENDIF
  call mat_free(FX(1))
  ...
  do icoor=1,3     ! magnetic
     do jcoor=1,3  ! magnetic moment koordinate
        NMST(jcoor,icoor) = NMST(jcoor,icoor) - 2.0E0_realk*mat_dotproduct(hx(jcoor),DB(icoor))        
        WRITE(lupri,*)'H NMST(',jcoor,',',icoor,') = ',-Factor*mat_dotproduct(hx(jcoor),DB(icoor))
     enddo
  enddo
  call mat_free(hX(1))
  call mat_free(hX(2))
...
!!$  call mat_init(FX(2),nbast,nbast)
!!$  call mat_init(FX(3),nbast,nbast)                                     
!!$  call di_GET_GbDs(lupri,luerr,DX,FX,3,molcfg%setting)
!!$  do icoor=1,3     ! magnetic
!!$     do jcoor=1,3  ! magnetic moment koordinate
!!$        NMST(jcoor,icoor) = NMST(jcoor,icoor) + 2.0E0_realk*mat_dotproduct(Fx(jcoor),DB(icoor))        
!!$        WRITE(lupri,*)'I NMST(',jcoor,',',icoor,') = ',2.0E0_realk*Factor*mat_dotproduct(Fx(jcoor),DB(icoor))
!!$     enddo
!!$  enddo
!!$  IF(molcfg%setting%do_dft)THEN
!!$     call II_get_xc_linrsp(lupri,luerr,molcfg%setting,nbast,DX,D(1),FX,3) 
!!$     do icoor=1,3     ! magnetic
!!$        do jcoor=1,3  ! magnetic moment koordinate
!!$           NMST(jcoor,icoor) = NMST(jcoor,icoor) + 2.0E0_realk*mat_dotproduct(Fx(jcoor),DB(icoor))        
!!$           WRITE(lupri,*)'J NMST(',jcoor,',',icoor,') = ',2.0E0_realk*Factor*mat_dotproduct(Fx(jcoor),DB(icoor))
!!$        enddo
!!$     enddo
!!$  ENDIF
...
!!$  call mat_free(FX(3))
  ...
  jcoor = 1
  WRITE (LUPRI,'(20X,3(A,13X),/)') 'Bx', 'By', 'Bz'
  WRITE (LUPRI,'(2X,A8,3F15.8)')  atomname(jcoor)//CHRXYZ(1),(Factor*NMST(1,K),K=1,3)
  WRITE (LUPRI,'(2X,A8,3F15.8)')  atomname(jcoor)//CHRXYZ(2),(Factor*NMST(2,K),K=1,3)
  WRITE (LUPRI,'(2X,A8,3F15.8)')  atomname(jcoor)//CHRXYZ(3),(Factor*NMST(3,K),K=1,3)

  WRITE(LUPRI,*) "      Absolute chemical shift"
  WRITE (LUPRI,'(2X,A8,f15.8,A8)')  atomname(jcoor), &
       & 1.0E0_realk/3.0E0_realk*(Factor*NMST(1,1)+Factor*NMST(2,2)+Factor*NMST(3,3))
  deallocate(atomname)
  call mem_dealloc(NMST)
  WRITE(LUPRI,*) " Done with shielding tensor calculation"

end subroutine NucSpecNMRshieldresponse_noOpenRSP
